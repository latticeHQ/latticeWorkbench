...
The README screenshots in `static/img/*.webp` are **not manually captured** — they are
...
| `static/img/*.webp`                                       | Output screenshot assets                   |
...
- Writes to `static/img/<outputFile>`
...
for f in code-review agent-status git-status plan-mermaid auto-mode costs-tab context-management mobile-server-mode orchestrate-agents; do
  bun -e "const s = require('sharp'); const m = await s('static/img/${f}.webp').metadata(); console.log('${f}:', m.width + 'x' + m.height)"
done

# Stage and commit
git add static/img/*.webp
...
Intentional — README now uses `lattice-demo.gif` as the hero media. The README
screenshot pipeline covers only the 9 `static/img/*.webp` assets that remain in
