---
name: tbench
description: Terminal-Bench integration for Lattice agent benchmarking and failure analysis
---

# Terminal-Bench Integration

This directory contains the lattice agent adapter for [Terminal-Bench 2.0](https://tbench.ai/), using [Harbor](https://harborframework.com/) as the evaluation harness.
...
- `LATTICE_RUN_ARGS`: CLI flags passed directly to `lattice run` inside the container (e.g., `--thinking high --use-1m --budget 5.00`). This is the primary mechanism for all `lattice run` flags — avoids per-flag plumbing.
...
All other `lattice run` CLI flags (thinking level, mode, runtime, budget, etc.) are passed via `LATTICE_RUN_ARGS` — no per-flag plumbing needed.
...
gh workflow run terminal-bench.yml \
  ...
  -f lattice_run_args="--thinking xhigh --use-1m"

# Run with budget cap
gh workflow run terminal-bench.yml \
  -f model_name=anthropic/claude-opus-4-6 \
  -f lattice_run_args="--thinking high --budget 5.00"
```

**Local runs:**

```bash
# Pass flags via LATTICE_RUN_ARGS env var
LATTICE_RUN_ARGS="--thinking high --use-1m" make benchmark-terminal
...
Lattice Terminal-Bench results are uploaded to BigQuery after CI runs. Query via `bq` CLI after authenticating with `gcloud auth login` and setting project to `lattice-benchmarks`.

**Table:** `lattice-benchmarks.benchmarks.tbench_results`
...
To submit lattice results to the [Terminal-Bench 2.0 leaderboard](https://tbench.ai/leaderboard/terminal-bench/2.0):
...
```
submissions/terminal-bench/2.0/Lattice__<model>/
...
api.upload_folder(
    ...
    repo_type="dataset",
    create_pr=True,
    commit_message="Add Lattice + <Model> submission",
    commit_description="- Agent: Lattice (Lattice)\n- Model: <model>\n- <N> tasks × <K> attempts",
)
```
...
- `lattice_agent.py`: Main agent adapter implementing Harbor's `BaseInstalledAgent` interface
- `lattice-run.sh`: Shell script that sets up environment and invokes lattice CLI
- `lattice_payload.py`: Helper to package lattice app for containerized execution
- `lattice_setup.sh.j2`: Jinja2 template for agent installation script
...
When investigating why Lattice fails on a task more than other agents, consider this workflow:

### 1. Identify High-Priority Failures

```bash
# Find tasks where Lattice underperforms (high M/O ratio = Lattice fails more than others)
...
To identify where Lattice underperforms relative to other top agents, use the analysis script:

```bash
# Run analysis (requires bq CLI for Lattice results, git for leaderboard data)
...
# Filter to specific Lattice model
python benchmarks/terminal_bench/analyze_failure_rates.py --lattice-model sonnet
...
Tasks with **high M/O ratio** are where Lattice underperforms relative to competitors—these represent the best optimization opportunities.
...
================================================================================
Task ID                                   Lattice Fail%  Avg Other%  M/O Ratio Agent
--------------------------------------------------------------------------------
some-difficult-task                         100.0%       10.0%       9.09 Lattice__Claude-Sonnet-4.5
another-task                                 80.0%       20.0%       3.64 Lattice__Claude-Sonnet-4.5
...

================================================================================
SUMMARY
================================================================================
Total tasks with Lattice failures: 42
