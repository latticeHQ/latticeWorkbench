...
_Generated with `lattice` • Model: `<modelString>` • Thinking: `<thinkingLevel>` • Cost: `$<costs>`_

<!-- lattice-attribution: model=<modelString> thinking=<thinkingLevel> costs=<costs> -->
```

Always check `$LATTICE_MODEL_STRING`, `$LATTICE_THINKING_LEVEL`, and `$LATTICE_COSTS_USD` via bash before creating or updating PRs—include them in the footer if set.
