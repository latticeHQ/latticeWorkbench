...
export async function startSSHServer(): Promise<SSHServerConfig> {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-ssh-test-"));
  ...
  try {
    ...
    await execCommand("docker", ["build", "-t", "lattice-ssh-test", dockerfilePath]);

    // Generate unique container name to avoid conflicts
    const containerName = `lattice-ssh-test-${crypto.randomBytes(8).toString("hex")}`;
