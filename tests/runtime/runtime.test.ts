...
describeIntegration("Runtime integration tests", () => {
    ...
    ({ type }) => {
      ...
      const createRuntime = (): Runtime =>
        createTestRuntime(
          ...
          sshConfig,
          type === "docker"
            ? { image: "lattice-ssh-test", containerName: sshConfig!.containerId }
            : undefined
        );
      ...
      describe("ensureReady() - Runtime readiness", () => {
        ...
        testDockerOnly(
          ...
          async () => {
            ...
            const containerName = `lattice-docker-ready-test-${Date.now()}`;

            // Start a fresh container (no --rm so we can stop/start it)
            execSync(`docker run -d --name ${containerName} lattice-ssh-test sleep infinity`, {
              timeout: 60000,
            });
        ...
        testDockerOnly("returns error for non-existent container", async () => {
          ...
          const runtime = new DockerRuntime({
            image: "ubuntu:22.04",
            containerName: "lattice-nonexistent-container-12345",
          });
      ...
      describe("resolvePath() - Path resolution", () => {
        ...
        testForRuntime("expands ~/path by prefixing the home directory", async () => {
          ...
          const resolved = await runtime.resolvePath("~/lattice");

          expect(resolved).toBe(`${home}/lattice`);
        });
      });
    ...
    describe("initWorkspace skips setup for running containers (fork scenario)", () => {
      testForDocker(
        ...
        async () => {
          ...
          const runtime = new DockerRuntime({ image: "lattice-ssh-test", containerName });
          ...
          try {
            ...
            await dockerCommand(
              `docker run -d --name ${containerName} lattice-ssh-test sleep infinity`
            );
      ...
      testForDocker(
        ...
        async () => {
          ...
          try {
            ...
            await dockerCommand(`docker exec ${containerName} mkdir -p /src/.lattice`);
            await dockerCommand(
            ...
            await dockerCommand(
              `docker exec ${containerName} bash -c "echo '#!/bin/bash\nexit 1' > /src/.lattice/init && chmod +x /src/.lattice/init"`
            );
            await dockerCommand(
   ...
   *
   * Tests Lattice-specific behavior like fork config updates.
