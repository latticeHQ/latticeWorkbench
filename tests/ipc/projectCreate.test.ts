 ...
 *
 * Tests:
 * - Bare project names resolve to ~/.lattice/projects/<name>
 * - Tilde expansion in project paths (home + ~/.lattice)
...
import { getLatticeHome, getLatticeProjectsDir } from "../../src/common/constants/paths";
...
describeIntegration("PROJECT_CREATE IPC Handler", () => {
  test.concurrent("should resolve bare project name to lattice projects dir", async () => {
    const env = await createTestEnvironment();
    const bareName = `lattice-test-bare-${Date.now()}`;
    const expectedPath = path.join(getLatticeProjectsDir(), bareName);
  ...
  test.concurrent("should expand ~/.lattice paths to lattice home", async () => {
    const env = await createTestEnvironment();
    const tildeSubpath = `lattice-test-tilde-${Date.now()}`;
    const tildeProjectPath = `~/.lattice/test-projects/${tildeSubpath}`;
    const expectedPath = path.join(getLatticeHome(), "test-projects", tildeSubpath);
    const client = resolveOrpcClient(env);
  ...
  test.concurrent("should expand windows-style lattice tilde paths", async () => {
    const env = await createTestEnvironment();
    const tildeSubpath = `lattice-test-tilde-win-${Date.now()}`;
    const tildeProjectPath = `~\\.lattice\\test-projects\\${tildeSubpath}`;
    const expectedPath = path.join(getLatticeHome(), "test-projects", tildeSubpath);
    const client = resolveOrpcClient(env);
  ...
  test.concurrent("should reject duplicate bare project name", async () => {
    const env = await createTestEnvironment();
    const bareName = `lattice-test-dup-${Date.now()}`;
    const expectedPath = path.join(getLatticeProjectsDir(), bareName);
    const client = resolveOrpcClient(env);
    ...
    try {
      ...
      expect(result2.error).toBe("Headquarter already exists");
    } finally {
      await fs.rm(expectedPath, { recursive: true, force: true });
      await cleanupTestEnvironment(env);
    }
  ...
  test.concurrent("should reject empty project path", async () => {
    ...
    try {
      ...
      expect(result.error).toBe("Headquarter path cannot be empty");
    } finally {
      await cleanupTestEnvironment(env);
    }
  });
  ...
  test.concurrent("should expand tilde in project path and create project", async () => {
    const env = await createTestEnvironment();
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-project-test-"));
    // Create a test directory in home directory
    const testDirName = `lattice-test-tilde-${Date.now()}`;
  ...
  test.concurrent("should create non-existent project path", async () => {
    const env = await createTestEnvironment();
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-project-test-"));
    const newProjectPath = path.join(tempProjectDir, `lattice-created-${Date.now()}`);
    const client = resolveOrpcClient(env);
  ...
  test.concurrent("should create non-existent tilde path", async () => {
    const env = await createTestEnvironment();
    const tempProjectDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-project-test-"));
    const testDirName = `lattice-tilde-create-${Date.now()}`;
    const tildeProjectPath = `~/${testDirName}`;
