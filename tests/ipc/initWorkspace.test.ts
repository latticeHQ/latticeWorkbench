...
/**
 * Create a temp git repo with a .lattice/init hook that writes to stdout/stderr and exits with a given code
 */
async function createTempGitRepoWithInitHook(options: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-test-init-hook-"));
  ...
  const latticeDir = path.join(tempDir, ".lattice");
...
describeIntegration("Workspace init hook", () => {
    ...
    async () => {
      ...
      try {
        ...
        if (startEvent && isInitStart(startEvent)) {
          // Hook path should be the project path (where .lattice/init exists)
  ...
  test.concurrent(
    ...
    async () => {
      const env = await createTestEnvironment();
      // Create repo without .lattice/init hook
      const execAsync = promisify(exec);

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-test-no-hook-"));
  ...
  test.concurrent(
    ...
    async () => {
      ...
      try {
        ...
        expect(status.hookPath).toBeTruthy(); // Headquarter path where hook exists
