...
describe("ACP user message translation for agent skills", () => {
  it("suppresses synthetic agent skill snapshot messages", async () => {
    ...
    await forwardEvents(translator, [
      makeUserMessage('<agent-skill name="lattice-docs" scope="built-in">...</agent-skill>', {
        metadata: {
          synthetic: true,
          agentSkillSnapshot: {
            skillName: "lattice-docs",
            scope: "built-in",
            sha256: "abc123",
          },
        },
  ...
  it("suppresses live transformed skill invocation text", async () => {
    ...
    await forwardEvents(translator, [
      makeCaughtUp("live"),
      makeUserMessage("Using skill lattice-docs: what is lattice?", {
        metadata: {
          latticeMetadata: {
            type: "agent-skill",
            rawCommand: "/lattice-docs what is lattice?",
            skillName: "lattice-docs",
            scope: "built-in",
          },
        },
  ...
  it("replays the original slash command for agent-skill history", async () => {
    ...
    expect(getUserTextChunks(sessionUpdates)).toEqual(["/lattice-docs what is lattice?"]);
  });
