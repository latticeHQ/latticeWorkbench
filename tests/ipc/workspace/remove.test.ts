...
async function executeBash(
  ...
  const result = await env.orpc.minion.executeBash({ minionId, script: command });

  if (!result.success || !result.data) {
    const errorMessage = "error" in result ? result.error : "unknown error";
    throw new Error(
      `Bash execution failed for "${command}" (workspace=${minionId}): ${errorMessage}`
    );
  }
 ...
 */
async function workspaceExists(env: TestEnvironment, minionId: string): Promise<boolean> {
  try {
    // Try to execute a simple command in the workspace
    // If workspace doesn't exist, this will fail
    const result = await executeBash(env, minionId, `pwd`);
 ...
 */
async function makeWorkspaceDirty(env: TestEnvironment, minionId: string): Promise<void> {
...
/**
 * Create a temporary git repo with a committed .lattice/init hook.
 *
...
async function createTempGitRepoWithInitHook(customScript: string): Promise<string> {
  ...
  const latticeDir = path.join(repoPath, ".lattice");
  await fs.mkdir(latticeDir, { recursive: true });

  const hookPath = path.join(latticeDir, "init");
    ...
    ({ type }) => {
      ...
      runTest(
        ...
        async () => {
          ...
          try {
            ...
            const { minionId, workspacePath } = await createWorkspaceWithInit(
              env,
              tempGitRepo,
              branchName,
              runtimeConfig,
            ...
            const existsBefore = await workspaceExists(env, minionId);
            if (!existsBefore) {
              console.error(`Workspace ${minionId} does not exist after creation`);
              console.error(`workspacePath from metadata: ${workspacePath}`);
            }
            expect(existsBefore).toBe(true);

            // Delete the workspace
            const deleteResult = await env.orpc.minion.remove({ minionId });
            ...
            if (project) {
              const stillInConfig = project.minions.some((w) => w.id === minionId);
      ...
      runTest(
        ...
        async () => {
          ...
          const markerPath =
            type === "ssh"
              ? `/home/testuser/lattice-init-cancel-${branchName}`
              : path.join(os.tmpdir(), `lattice-init-cancel-${branchName}`);
          ...
          try {
            ...
            const runtimeConfig = getRuntimeConfig(branchName);
            const { minionId } = await createWorkspaceWithInit(
              env,
              tempGitRepo,
              branchName,
              runtimeConfig,
            ...
            try {
              ...
              expect(await markerFileExists(type, markerPath)).toBe(false);
              expect(await workspaceExists(env, minionId)).toBe(false);
            } finally {
              collector.stop();
            }
          } finally {
      ...
      runTest(
        ...
        async () => {
          ...
          try {
            ...
            const deleteResult = await env.orpc.minion.remove({
              minionId: "non-existent-workspace-id",
            });
      ...
      runTest(
        ...
        async () => {
          ...
          try {
            ...
            const runtimeConfig = getRuntimeConfig(branchName);
            const { minionId, workspacePath: _workspacePath } = await createWorkspaceWithInit(
              env,
              tempGitRepo,
              branchName,
              runtimeConfig,
            ...
            await executeBash(env, minionId, 'cd .. && rm -rf "$(basename "$PWD")"');

            // Verify it's gone (note: workspace is deleted, so we can't use executeBash on minionId anymore)
      ...
      runTest(
        ...
        async () => {
          ...
          try {
            ...
            await makeWorkspaceDirty(env, minionId);
            ...
            const stillExists = await workspaceExists(env, minionId);
            expect(stillExists).toBe(true);

            // Cleanup: force delete for cleanup
            await env.orpc.minion.remove({ minionId, options: { force: true } });
          } finally {
            await cleanupTestEnvironment(env);
            await cleanupTempGitRepo(tempGitRepo);
          }
        ...
        runTest(
          ...
          async () => {
            ...
            try {
              ...
              const forceDeleteResult = await env.orpc.minion.remove({
                minionId,
                options: { force: true },
              });
    ...
    test.concurrent(
      ...
      async () => {
        ...
        try {
          ...
          const { minionId: ws1Id } = await createWorkspaceWithInit(
            env,
            tempGitRepo,
            "local-ws-1",
            runtimeConfig,
          ...
          const { minionId: ws2Id } = await createWorkspaceWithInit(
            env,
            tempGitRepo,
            "local-ws-2",
            runtimeConfig,
          ...
          const { minionId: ws3Id } = await createWorkspaceWithInit(
            env,
            tempGitRepo,
            "local-ws-3",
            runtimeConfig,
          ...
          expect(project?.minions.length).toBe(3);
          expect(project?.minions.some((w) => w.id === ws1Id)).toBe(true);
          expect(project?.minions.some((w) => w.id === ws2Id)).toBe(true);
          expect(project?.minions.some((w) => w.id === ws3Id)).toBe(true);

          // Delete workspace 2
          const deleteResult = await env.orpc.minion.remove({ minionId: ws2Id });
          expect(deleteResult.success).toBe(true);
          ...
          expect(project?.minions.length).toBe(2);
          expect(project?.minions.some((w) => w.id === ws1Id)).toBe(true);
          expect(project?.minions.some((w) => w.id === ws2Id)).toBe(false); // deleted
          expect(project?.minions.some((w) => w.id === ws3Id)).toBe(true);

          // Cleanup remaining workspaces
          await env.orpc.minion.remove({ minionId: ws1Id });
          await env.orpc.minion.remove({ minionId: ws3Id });
        } finally {
          await cleanupTestEnvironment(env);
          await cleanupTempGitRepo(tempGitRepo);
        }
    ...
    runSshTest(
      ...
      async () => {
        ...
        try {
          ...
          await executeBash(env, minionId, 'git config user.email "test@example.com"');
          await executeBash(env, minionId, 'git config user.name "Test User"');
          ...
          await executeBash(env, minionId, 'echo "new content" > newfile.txt');
          await executeBash(env, minionId, "git add newfile.txt");
          await executeBash(env, minionId, 'git commit -m "Unpushed commit"');

          // Verify commit was created and working tree is clean
          const statusResult = await executeBash(env, minionId, "git status --porcelain");
    ...
    runSshTest.skip(
      ...
      async () => {
        ...
        try {
          ...
          await executeBash(env, minionId, 'echo "1" > file1.txt');
          await executeBash(env, minionId, "git add file1.txt");
          await executeBash(env, minionId, 'git commit -m "First commit"');

          await executeBash(env, minionId, 'echo "2" > file2.txt');
          await executeBash(env, minionId, "git add file2.txt");
          await executeBash(env, minionId, 'git commit -m "Second commit"');
    ...
    runSshTest.skip(
      ...
      async () => {
        ...
        try {
          ...
          const pwdResult = await executeBash(env, minionId, "pwd");
          ...
          await executeBash(env, minionId, `git clone --bare . "${originPath}"`);
          ...
          await executeBash(env, minionId, 'echo "feature1" > feature.txt');
          await executeBash(env, minionId, "git add feature.txt");
          await executeBash(env, minionId, 'git commit -m "Feature commit 1"');

          await executeBash(env, minionId, 'echo "feature2" >> feature.txt');
          await executeBash(env, minionId, "git add feature.txt");
          await executeBash(env, minionId, 'git commit -m "Feature commit 2"');

          // Get the feature branch's final file content
          const featureContent = await executeBash(env, minionId, "cat feature.txt");
          ...
          await executeBash(env, minionId, `rm -rf "${tempCheckoutPath}"`);

          // Fetch the updated origin in the workspace
          await executeBash(env, minionId, "git fetch origin");
    ...
    runSshTest(
      ...
      async () => {
        ...
        try {
          ...
          await executeBash(env, minionId, 'echo "unique-unmerged-content" > unique.txt');
          await executeBash(env, minionId, "git add unique.txt");
          await executeBash(env, minionId, 'git commit -m "Unique commit"');
