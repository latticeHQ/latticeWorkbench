...
async function createInitHook(repoPath: string, hookContent: string): Promise<void> {
  const latticeDir = path.join(repoPath, LATTICE_DIR);
  await fs.mkdir(latticeDir, { recursive: true });
  const initHookPath = path.join(latticeDir, INIT_HOOK_FILENAME);
  ...
  describe.each<{ type: "local" | "ssh" }>([{ type: "local" }, { type: "ssh" }])(
    ...
    ({ type }) => {
      ...
      describe("Init hook execution", () => {
        runTest(
          "executes .lattice/init hook when present and streams logs",
          async () => {
            const env = await createTestEnvironment();
            const tempGitRepo = await createTempGitRepo();
      ...
      async () => {
        ...
        try {
          ...
          try {
            ...
            expect(remoteUrl).not.toContain(".lattice-bundle");
          } finally {
            await cleanup();
          }
        } finally {
