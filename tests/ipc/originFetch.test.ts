...
async function createRepoWithAheadOrigin(): Promise<{
  ...
  // Create local repo
  const repoPath = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-test-repo-"));
  ...
  const originPath = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-test-origin-"));
  ...
  const tempClone = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-test-clone-"));
