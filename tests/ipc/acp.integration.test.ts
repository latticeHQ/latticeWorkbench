...
async function createAcpClient(options: CreateAcpClientOptions = {}): Promise<AcpTestClient> {
  ...
  const latticeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-acp-test-root-"));
...
describeIntegration("ACP built CLI integration", () => {
  ...
  test(
    "initialize returns lattice agent info",
    async () => {
      ...
      try {
        ...
        expect(initializeResponse.agentInfo?.name).toBe("lattice");
      } finally {
  ...
  test(
    ...
    async () => {
      ...
      const logDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-acp-log-file-"));
