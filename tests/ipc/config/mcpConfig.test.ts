...
describeIntegration("MCP global configuration", () => {
  test.concurrent("add, list, and remove MCP servers", async () => {
    ...
    try {
      ...
      // Add server (writes to global <latticeHome>/mcp.jsonc)
  ...
  test.concurrent("repo .lattice/mcp.jsonc overrides global servers by name", async () => {
    const env = await createTestEnvironment();
    const repoPath = await createTempGitRepo();
    const client = resolveOrpcClient(env);
    ...
    try {
      ...
      const overridePath = path.join(repoPath, ".lattice", "mcp.jsonc");
