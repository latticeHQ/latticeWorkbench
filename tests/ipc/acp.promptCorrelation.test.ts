...
describe("ACP prompt stream correlation", () => {
  it("ignores unrelated stream-start/end pairs while waiting for this prompt turn", async () => {
    ...
    const latticeMetadata = firstSend.options["latticeMetadata"];
    if (!isRecord(latticeMetadata)) {
      throw new Error("Expected prompt send options to include latticeMetadata record");
    }

    const promptCorrelationId = latticeMetadata["acpPromptId"];
  ...
  it("attaches delegated tool metadata when local runtime and editor capabilities allow delegation", async () => {
    ...
    expect(latticeMetadata["acpDelegatedTools"]).toEqual([
      "file_read",
      "file_write",
  ...
  it("interrupts active turns on ACP disconnect to unblock delegated tool waits", async () => {
    ...
    await expect(promptPromise).rejects.toThrow("Lattice ACP connection closed");
  ...
  it("interrupts every active turn before disconnect session eviction runs", async () => {
    ...
    expect(firstPromptResult.message).toContain("Lattice ACP connection closed");
    expect(secondPromptResult.message).toContain("Lattice ACP connection closed");
  ...
  it("continues disconnect cleanup when interruptStream reports backend failure", async () => {
    ...
    expect(promptResult.message).toContain("Lattice ACP connection closed");
    await waitForCondition(() => harness.interruptCalls.length === 1);
