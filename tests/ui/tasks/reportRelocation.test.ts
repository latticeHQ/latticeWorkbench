...
async function seedHistory(historyService: HistoryService, workspaceId: string): Promise<void> {
  ...
  const userMessage = createLatticeMessage("user-1", "user", "Spawn a background task");

  const taskToolMessage = createLatticeMessage("assistant-task", "assistant", "", undefined, [
    {
  ...
  const taskAwaitToolMessage = createLatticeMessage(
