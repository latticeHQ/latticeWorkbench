...
async function createAgentFile(
  ...
  const agentsDir = path.join(workspacePath, ".lattice", "agents");
...
async function removeAgentFile(workspacePath: string, agentId: string): Promise<void> {
  const filePath = path.join(workspacePath, ".lattice", "agents", `${agentId}.md`);
  try {
    ...
    try {
      ...
      const projectRow = await waitFor(
        () => {
          ...
          if (!el) throw new Error("Headquarter not found");
      ...
      await waitFor(
        () => {
          ...
          if (!textarea) throw new Error("Headquarter page not rendered");
        },
        { timeout: 5_000 }
      );
