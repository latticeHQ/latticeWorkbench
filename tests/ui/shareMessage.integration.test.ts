/**
 * Integration tests for the openagent.md message sharing feature.
 *
 * Tests cover:
 * - End-to-end encrypted upload to openagent.md
...
import { uploadToLatticeMd, deleteFromLatticeMd } from "../../src/common/lib/latticeMd";

const describeIntegration = shouldRunIntegrationTests() ? describe : describe.skip;

// ═══════════════════════════════════════════════════════════════════════════════
// OPENAGENT.MD UPLOAD TESTS
// ═══════════════════════════════════════════════════════════════════════════════

describeIntegration("openagent.md sharing (upload integration)", () => {
  test("should upload encrypted content and return valid URL", async () => {
    ...
    // Verify the URL format: https://openagent.md/{id}#{key}
    expect(result.url).toMatch(/^https:\/\/openagent\.md\/[A-Za-z0-9]+#[A-Za-z0-9_-]+$/);
  ...
  test("should generate unique URLs for each upload", async () => {
    ...
    const result1 = await uploadToLatticeMd(content, {
      name: "message.md",
      type: "text/markdown",
      size: content.length,
    });

    const result2 = await uploadToLatticeMd(content, {
      name: "message.md",
      type: "text/markdown",
      size: content.length,
    });
...
`;
  ...
  test("should successfully delete uploaded content using mutateKey", async () => {
    ...
    const response = await fetch(`https://openagent.md/${result.id}`);
