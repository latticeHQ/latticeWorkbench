/**
 * Integration tests for WorkbenchPanel dock-lite behavior.
 *
 * Tests cover:
...
// WorkbenchPanelLayoutState used for initial setup via persisted-state helpers - acceptable for test fixtures
...
describeIntegration("WorkbenchPanel (UI)", () => {
      ...
      if (workspaceId) {
        ...
        await Promise.all(
          sessions.map(async (s) => {
            try {
              await env.orpc.terminal.close({ sessionId: s.sessionId });
            } catch {
              // Best-effort cleanup.
            }
          })
  ...
  beforeEach(async () => {
    ...
    updatePersistedState(WORKBENCH_PANEL_TAB_KEY, null);
    updatePersistedState(WORKBENCH_PANEL_COLLAPSED_KEY, null);
    updatePersistedState(WORKBENCH_PANEL_WIDTH_KEY, null);
    updatePersistedState(getWorkbenchPanelLayoutKey(workspaceId), null);
  ...
  test("sidebar collapse and expand via button", async () => {
    ...
    // Start expanded
    updatePersistedState(WORKBENCH_PANEL_COLLAPSED_KEY, false);
  ...
  test("tab selection persists across workspace navigation", async () => {
    ...
    const initialLayout: WorkbenchPanelLayoutState = {
      version: 1,
      nextId: 2,
    ...
    updatePersistedState(getWorkbenchPanelLayoutKey(workspaceId), initialLayout);
  ...
  test("sidebar cannot be resized beyond available width", async () => {
    ...
    // Force a narrow viewport so the workbench panel max clamp is exercised.
  ...
  test("split layout renders multiple panes with separate tablists", async () => {
    ...
    const splitLayout: WorkbenchPanelLayoutState = {
      version: 1,
      nextId: 10,
      root: {
        type: "split",
    ...
    updatePersistedState(getWorkbenchPanelLayoutKey(workspaceId), splitLayout);
