...
describe("Unread indicator (mock AI router)", () => {
  ...
  describe("basic unread tracking", () => {
    ...
    test("workspace with missing lastRead key is not unread by default", async () => {
      ...
      await waitFor(() => {
        const latticeHelpButton = app.view.container.querySelector(
          'button[aria-label="Open Chat with Lattice"]'
        ) as HTMLButtonElement | null;
        expect(latticeHelpButton).not.toBeNull();
        const unreadDot = latticeHelpButton?.querySelector('[aria-label="Unread messages"]');
