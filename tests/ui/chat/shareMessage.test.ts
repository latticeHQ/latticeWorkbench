/**
 * Integration tests for the lattice.md message sharing feature.
 *
 * Tests cover:
 * - End-to-end encrypted upload to lattice.md
...
import { uploadToLatticeMd, deleteFromLatticeMd, getLatticeMdBaseUrl } from "../../../src/common/lib/latticeMd";
...
// ═══════════════════════════════════════════════════════════════════════════════
// LATTICE.MD UPLOAD TESTS
// ═══════════════════════════════════════════════════════════════════════════════

describeIntegration("lattice.md sharing (upload integration)", () => {
  test("should upload encrypted content and return valid URL", async () => {
    const testContent =
      "# Test Message\n\nThis is a test message for sharing.\n\n```typescript\nconst x = 42;\n```";

    const result = await uploadToLatticeMd(testContent, {
      name: "message.md",
      type: "text/markdown",
    ...
    // Verify the URL format: https://lattice.md/{id}#{key}
    expect(result.url).toMatch(/^https:\/\/lattice\.md\/[A-Za-z0-9]+#[A-Za-z0-9_-]+$/);
  ...
  test("should generate unique URLs for each upload", async () => {
    ...
    // from sequential network latency to the external lattice.md service.
    const [result1, result2] = await Promise.all([
      uploadToLatticeMd(content, {
        name: "message.md",
        type: "text/markdown",
        size: content.length,
      }),
...
`;
    ...
    const result = await uploadToLatticeMd(contentWithSpecialChars, {
      name: "special.md",
      type: "text/markdown",
  ...
  test("should include model metadata in upload", async () => {
    ...
    const result = await uploadToLatticeMd(content, {
      name: "message.md",
      type: "text/markdown",
      size: content.length,
  ...
  test("should successfully delete uploaded content using mutateKey", async () => {
    ...
    await expect(deleteFromLatticeMd(result.id, result.mutateKey)).resolves.not.toThrow();

    // Verify the content is no longer accessible (fetch returns 404).
    // Use getLatticeMdBaseUrl() to respect the LATTICE_MD_URL_OVERRIDE env var,
    ...
    const response = await fetch(`${getLatticeMdBaseUrl()}/${result.id}`);
