...
const defaultTestRoot = path.join(appRoot, "tests", "e2e", "tmp", "lattice-root");
const BASE_DEV_SERVER_PORT = Number(process.env.LATTICE_E2E_DEVSERVER_PORT_BASE ?? "5173");
const shouldLoadDist = process.env.LATTICE_E2E_LOAD_DIST === "1";
...
function shouldSkipBuild(): boolean {
  return process.env.LATTICE_E2E_SKIP_BUILD === "1";
}
...
export const electronTest = base.extend<ElectronFixtures>({
  workspace: async ({}, use, testInfo) => {
    ...
    //
    // Even when LATTICE_ROOT is set (often for debugging), Playwright runs tests in parallel.
    ...
    // Ensure the Electron process sees our per-test LATTICE_ROOT even if Playwright's electron.launch({ env }) is ignored.
    //
    // This is especially important when running Playwright under Bun.
    process.env.LATTICE_ROOT = testRoot;
  ...
  app: async ({ workspace }, use, testInfo) => {
    ...
    if (shouldStartDevServer) {
      devServer = spawn("make", ["dev"], {
        ...
        env: {
          ...
          LATTICE_VITE_PORT: String(devServerPort),
        },
      });
    ...
    try {
      ...
      if (shouldStartDevServer) {
        devHost = process.env.LATTICE_DEVSERVER_HOST ?? "127.0.0.1";
      ...
      if (shouldStartDevServer) {
        electronEnv.LATTICE_DEVSERVER_PORT = String(devServerPort);
        electronEnv.LATTICE_DEVSERVER_HOST = devHost;
      ...
      // Sanity check: ensure we are not accidentally reading/writing the developer's real ~/.lattice state.
          ...
          try {
            ...
            if (electronApp && videoFiles.length) {
              ...
              const baseName = sanitizeForPath(
                testInfo.testId || testInfo.title || "lattice-e2e-video"
              );
