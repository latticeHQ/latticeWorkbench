"""Autogenerated OpenBB /derivatives/futures Module."""
...
class ROUTER_derivatives_futures(Container):
    ...
    def curve(
        self,
        symbol: Annotated[
            Union[str, Literal['VX_AM', 'VX_EOD'], Literal['BTC', 'ETH', 'PAXG']],
            OpenBBField(
                description=(
                    'Symbol to get data for.;     Symbol to get data'
                    "for.Default is 'VX_EOD'. Entered dates return the"
                    "data nearest to the entered date.     'VX_AM' ="
                    "Mid-Morning TWAP Levels     'VX_EOD' = 4PM Eastern"
                    'Time Levels (provider: cboe);     Symbol to get'
                    "data for. Default is 'btc' Supported symbols are:"
                    "['btc', 'eth', 'paxg'] (provider: deribit) Choices"
                    "for deribit: 'BTC', 'ETH', 'PAXG' Choices for"
                    "cboe: 'VX_AM', 'VX_EOD'"
                )
            )
        ],
        date: Annotated[
            ...
            OpenBBField(
                description=(
                    ...
                    'separated items allowed for provider(s): cboe,'
                    'yfinance.'
                )
            )
        ] = None,
        provider: Annotated[
            ...
            OpenBBField(
                description=(
                    ...
                    'Default priority: cboe, deribit, yfinance.'
                )
            )
        ] = None,
        **kwargs
...
hours_ago : int | list[int] | str | None
...
>>> obb.derivatives.futures.curve(symbol='NG')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'deribit': {'choices': ['BTC', 'ETH', 'PAXG']}, 'cboe': {'choices': ['VX_AM', 'VX_EOD']}}, 'date': {'cboe': {'multiple_items_allowed': True, 'choices': None}, 'yfinance': {'multiple_items_allowed': True, 'choices': None}}, 'hours_ago': {'deribit': {'multiple_items_allowed': True, 'choices': None}}},
            )
        )
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    ...
                    'items allowed for provider(s): deribit, yfinance.'
                )
            )
        ],
        ...
        provider: Annotated[
            ...
            OpenBBField(
                description=(
                    'The provider to use, by default None. If None, the'
                    'priority list configured in the settings is used.'
                    'Default priority: deribit, yfinance.'
                )
            )
        ] = None,
        **kwargs
...
>>> obb.derivatives.futures.historical(symbol='ES', expiration='2025-12')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'deribit': {'multiple_items_allowed': True, 'choices': None}, 'yfinance': {'multiple_items_allowed': True, 'choices': None}}, 'interval': {'deribit': {'multiple_items_allowed': False, 'choices': ['1m', '3m', '5m', '10m', '15m', '30m', '1h', '2h', '3h', '6h', '12h', '1d']}, 'yfinance': {'choices': ['1m', '2m', '5m', '15m', '30m', '60m', '90m', '1h', '1d', '5d', '1W', '1M', '1Q']}}},
            )
        )
...
--------
>>> from openbb import obb
>>> obb.derivatives.futures.info(symbol='BTC')
>>> obb.derivatives.futures.info(symbol='SOLUSDC')
>>> obb.derivatives.futures.info(symbol='SOL_USDC-PERPETUAL')
>>> obb.derivatives.futures.info(symbol='BTC,ETH')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'deribit': {'multiple_items_allowed': True, 'choices': None}}},
            )
        )
