        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    ...
                    'items allowed for provider(s): fmp, tmx.'
                )
            )
        ],
        provider: Annotated[
...
symbol : str | list[str]
    Symbol to get data for. Multiple comma separated items allowed for provider(s): fmp, tmx.
use_cache : bool
...
>>> obb.etf.countries(symbol='VT')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'fmp': {'multiple_items_allowed': True, 'choices': None}, 'tmx': {'multiple_items_allowed': True, 'choices': None}}},
            )
        )

    @property
    def discovery(self):
        """Router for /etf/discovery."""
        ...
        provider: Annotated[
            ...
            OpenBBField(
                description=(
                    'The provider to use, by default None. If None, the'
                    'priority list configured in the settings is used.'
                    'Default priority: fmp, intrinio, tmx.'
                )
            )
        ] = None,
        **kwargs
...
country : str | None
    The country or region of the holding. (provider: intrinio, tmx)
        ...
        provider: Annotated[
            ...
            OpenBBField(
                description=(
                    'The provider to use, by default None. If None, the'
                    'priority list configured in the settings is used.'
                    'Default priority: fmp, intrinio, tmx, yfinance.'
                )
            )
        ] = None,
        **kwargs
...
>>> obb.etf.info(symbol='SPY,IWM,QQQ,DJIA')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'fmp': {'multiple_items_allowed': True, 'choices': None}, 'intrinio': {'multiple_items_allowed': True, 'choices': None}, 'tmx': {'multiple_items_allowed': True, 'choices': None}, 'yfinance': {'multiple_items_allowed': True, 'choices': None}}},
            )
        )
        ...
        provider: Annotated[
            ...
            OpenBBField(
                description=(
                    'The provider to use, by default None. If None, the'
                    'priority list configured in the settings is used.'
                    'Default priority: finviz, fmp, intrinio.'
                )
            )
        ] = None,
        **kwargs
...
>>> obb.etf.price_performance(symbol='SPY,QQQ,IWM,DJIA')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'finviz': {'multiple_items_allowed': True, 'choices': None}, 'fmp': {'multiple_items_allowed': True, 'choices': None}, 'intrinio': {'multiple_items_allowed': True, 'choices': None}}, 'return_type': {'intrinio': {'choices': ['trailing', 'calendar']}}, 'adjustment': {'intrinio': {'choices': ['splits_only', 'splits_and_dividends']}}},
            )
        )
...
exchange : str | None
    ...
    (provider: intrinio)
        ...
        Choices: 'xnas', 'arcx', 'bats', 'xnys', 'bvmf', 'xshg', 'xshe', 'xhkg', 'xbom', 'xnse' 'xidx', 'tase', 'xkrx', 'xkls', 'xmex', 'xses', 'roco', 'xtai', 'xbkk', 'xist'
country : openbb_core.provider.utils.country_utils.Country | None
    Filter by country. Accepts ISO 3166-1 alpha-2 codes (e.g., 'US', 'DE'), alpha-3 codes (e.g., 'USA'), or country names (e.g., 'United States', 'united_states'). (provider: fmp)
div_freq : str
    The dividend payment frequency. (provider: tmx)
    Choices for tmx: 'monthly', 'annually', 'quarterly'
sort_by : str
    The column to sort by. (provider: tmx)
    Choices for tmx: 'aum', 'return_1m', 'return_3m', 'return_6m', 'return_1y', 'return_3y' 'return_ytd', 'beta_1y', 'volume_avg_daily', 'management_fee' 'distribution_yield', 'pb_ratio', 'pe_ratio'
use_cache : bool
    Whether to use a cached request. All ETF data comes from a single JSON file that is updated daily. To bypass, set to False. If True, the data will be cached for 4 hours. (provider: tmx)
    ...
    def sectors(
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    'Symbol to get data for. (ETF) Multiple comma'
                    'separated items allowed for provider(s): fmp, tmx.'
                )
            )
        ],
        provider: Annotated[
