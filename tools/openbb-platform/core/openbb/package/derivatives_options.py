...
class ROUTER_derivatives_options(Container):
    ...
    def chains(
        ...
        symbol: Annotated[
            ...
            OpenBBField(
                description=(
                    'Symbol to get data for. Choices for deribit:'
                    "'BTC', 'ETH', 'SOL', 'XRP', 'BNB', 'PAXG'"
                )
            )
        ],
        provider: Annotated[
            ...
            OpenBBField(
                description=(
                    ...
                    'Default priority: cboe, deribit, intrinio, tmx,'
                    'tradier, yfinance.'
                )
            )
        ] = None,
...
>>> obb.derivatives.options.chains(symbol='AAPL', date='2023-01-25')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'symbol': {'deribit': {'multiple_items_allowed': False, 'choices': ['BTC', 'ETH', 'SOL', 'XRP', 'BNB', 'PAXG']}}, 'delay': {'intrinio': {'multiple_items_allowed': False, 'choices': ['eod', 'realtime', 'delayed']}}, 'option_type': {'intrinio': {'multiple_items_allowed': False, 'choices': ['call', 'put']}}, 'moneyness': {'intrinio': {'multiple_items_allowed': False, 'choices': ['otm', 'itm', 'all']}}, 'model': {'intrinio': {'multiple_items_allowed': False, 'choices': ['black_scholes', 'bjerk']}}},
            )
        )
...
>>> obb.derivatives.options.snapshots()
    ...
    def surface(
        ...
        target: Annotated[
            ...
            OpenBBField(
                description=(
                    'The field to use as the z-axis. Default is'
                    '"implied_volatility".'
                )
            )
        ] = 'implied_volatility',
        ...
        dte_min: Annotated[
            ...
            OpenBBField(
                description=(
                    'Minimum days to expiration (DTE) to filter'
                    'options.'
                )
            )
        ] = None,
        dte_max: Annotated[
            ...
            OpenBBField(
                description=(
                    'Maximum days to expiration (DTE) to filter'
                    'options.'
                )
            )
        ] = None,
        ...
        strike_min: Annotated[
            float | None,
            OpenBBField(
                description='Minimum strike price to filter options.'
            )
        ] = None,
        strike_max: Annotated[
            float | None,
            OpenBBField(
                description='Maximum strike price to filter options.'
            )
        ] = None,
        oi: Annotated[
            bool,
            OpenBBField(
                description=(
                    'Filter for only options that have open interest.'
                    'Default is False.'
                )
            )
        ] = False,
        volume: Annotated[
            ...
            OpenBBField(
                description=(
                    'Filter for only options that have trading volume.'
                    'Default is False.'
                )
            )
        ] = False,
...
>>> obb.derivatives.options.unusual(symbol='TSLA')
        ...
        return self._run(
            ...
            **filter_inputs(
                ...
                extra_params=kwargs,
                info={'trade_type': {'intrinio': {'choices': ['block', 'sweep', 'large']}}, 'sentiment': {'intrinio': {'choices': ['bullish', 'bearish', 'neutral']}}, 'source': {'intrinio': {'choices': ['delayed', 'realtime']}}},
            )
        )
