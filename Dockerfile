# lattice server Docker image
# Multi-stage build with esbuild bundling for minimal runtime image
#
# Build:   docker build -t lattice-server .
# Run:     docker run -p 3000:3000 -v ~/.lattice:/root/.lattice lattice-server
#
...
# Create lattice data directory
RUN mkdir -p /root/.lattice

# Default environment variables
ENV NODE_ENV=production
ENV LATTICE_HOME=/root/.lattice
