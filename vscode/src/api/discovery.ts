...
const SERVER_AUTH_TOKEN_SECRET_KEY = "lattice.serverAuthToken";
...
export async function discoverServerConfig(
  ...
  const envBaseUrl = process.env.LATTICE_SERVER_URL?.trim();
  ...
  const envToken = (process.env.LATTICE_SERVER_AUTH_TOKEN ?? process.env.LATTICE_SERVER_AUTH_TOKEN)?.trim();
