...
import { getLatticeHome } from "lattice/common/constants/paths";
import { ServerLockfile } from "lattice/node/services/serverLockfile";

import { getAllWorkspacesFromApi } from "../latticeConfig";
...
integrationTestOrSkip(
  "connects to lattice oRPC server (via lockfile discovery) and lists workspaces",
  async () => {
    ...
    assert(
      lock,
      `No running lattice server found (missing/stale lockfile at ${lockfile.getLockPath()}). ` +
        `Start lattice and re-run with TEST_INTEGRATION=1.`
    );
