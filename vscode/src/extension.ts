...
import { formatRelativeTime } from "lattice/browser/utils/ui/dateTime";
import {
...
const PENDING_AUTO_SELECT_STATE_KEY = "lattice.pendingAutoSelectWorkspace";
const SELECTED_WORKSPACE_STATE_KEY = "lattice.selectedWorkspaceId";
...
let latticeLogChannel: vscode.LogOutputChannel | undefined;

function getLatticeLogChannel(): vscode.LogOutputChannel {
  if (!latticeLogChannel) {
    latticeLogChannel = vscode.window.createOutputChannel("Lattice", { log: true });
  }

  return latticeLogChannel;
}
...
function latticeLog(level: "debug" | "info" | "warn" | "error", message: string, data?: unknown): void {
  const channel = getLatticeLogChannel();
...
function latticeLogDebug(message: string, data?: unknown): void {
  latticeLog("debug", message, data);
}

function latticeLogInfo(message: string, data?: unknown): void {
  latticeLog("info", message, data);
}

function latticeLogWarn(message: string, data?: unknown): void {
  latticeLog("warn", message, data);
}

function latticeLogError(message: string, data?: unknown): void {
  latticeLog("error", message, data);
}
...
async function revealChatView(): Promise<void> {
  try {
    await vscode.commands.executeCommand("workbench.view.extension.latticeSecondary");
  } catch {
...
function getWarningSuffix(failure: ApiConnectionFailure): string {
  if (failure.kind === "unauthorized") {
    return "Using local file access while lattice is running can cause inconsistencies.";
  }
...
async function tryGetApiClient(
  ...
  latticeLogDebug("lattice: tryGetApiClient start");

  try {
    const discovery = await discoverServerConfig(context);

    latticeLogDebug("lattice: discovered server config", {
    ...
    latticeLogDebug("lattice: server reachable check", reachable);
    ...
    latticeLogDebug("lattice: auth check", auth);
    ...
    latticeLogDebug("lattice: tryGetApiClient success", { baseUrl: discovery.baseUrl });
    ...
    latticeLogError("lattice: tryGetApiClient threw", { error: formatError(error) });
...
async function getWorkspacesForCommand(
  ...
  if (modeSetting === "server-only") {
    const selection = await vscode.window.showErrorMessage(
      `lattice: ${describeFailure(failure)}. (${failure.error})`,
  ...
  const selection = await vscode.window.showWarningMessage(
    `lattice: ${describeFailure(failure)}. ${getWarningSuffix(failure)}`,
    ACTION_FIX_CONNECTION_CONFIG,
  ...
  sessionPreferredMode = "file";
  void vscode.window.showWarningMessage(
    `lattice: ${describeFailure(retry.failure)}. Falling back to local file access. (${retry.failure.error})`
  );
  return getAllWorkspacesFromFiles();
}
...
/**
 * Command: Open a lattice workspace
 */
async function openWorkspaceCommand(
  context: vscode.ExtensionContext,
  options?: {
    chatViewProvider?: LatticeChatViewProvider;
  }
): Promise<void> {
...
async function configureConnectionCommand(context: vscode.ExtensionContext): Promise<void> {
  ...
  for (;;) {
    ...
    const hasToken = (await context.secrets.get("lattice.serverAuthToken")) !== undefined;
...
async function debugConnectionCommand(context: vscode.ExtensionContext): Promise<void> {
  ...
  const output = getLatticeLogChannel();
  output.show(true);

  latticeLogInfo("lattice: debugConnection start");

  let discovery: Awaited<ReturnType<typeof discoverServerConfig>>;
  try {
    discovery = await discoverServerConfig(context);
  } catch (error) {
    latticeLogError("lattice: debugConnection discovery failed", { error: formatError(error) });
    void vscode.window.showErrorMessage(
      `lattice: Failed to discover server config. (${formatError(error)})`
    );
    return;
  }

  latticeLogInfo("lattice: debugConnection discovered server config", {
    baseUrl: discovery.baseUrl,
    baseUrlSource: discovery.baseUrlSource,
    authTokenSource: discovery.authTokenSource,
    hasAuthToken: Boolean(discovery.authToken),
  ...
  latticeLogInfo("lattice: debugConnection server reachable", reachable);

  if (reachable.status !== "ok") {
    void vscode.window.showErrorMessage(
      `lattice: Server not reachable at ${discovery.baseUrl}. (${reachable.error})`
    );
    return;
  }
  ...
  latticeLogInfo("lattice: debugConnection auth", auth);
  ...
  if (auth.status !== "ok") {
    ...
    void vscode.window.showErrorMessage(
      `lattice: Failed to authenticate at ${discovery.baseUrl}. (${auth.error})${hint}`
    );
    return;
  }
  ...
  try {
    ...
    latticeLogInfo("lattice: debugConnection listed workspaces", { count: workspaceCount });
  } catch (error) {
    latticeLogWarn("lattice: debugConnection list workspaces failed", { error: formatError(error) });
  }

  void vscode.window.showInformationMessage(
    workspaceCount === null
      ? `lattice: Connected to ${discovery.baseUrl} (auth ok).`
      : `lattice: Connected to ${discovery.baseUrl} (auth ok). Workspaces: ${workspaceCount}.`
  );
}
...
async function getWorkspacesForSidebar(
  ...
  const modeSetting: ConnectionMode = getConnectionModeSetting();
  latticeLogDebug("lattice: getWorkspacesForSidebar", { modeSetting });
  ...
  if (modeSetting === "file-only") {
    ...
    if ("error" in fileResult) {
      return {
        ...
        status: {
          mode: "file",
          error: `Failed to read lattice workspaces from local files. (${fileResult.error})`,
        },
      };
    }
  ...
  try {
    ...
    if (modeSetting === "server-only") {
      return {
        ...
        status: {
          mode: "api",
          baseUrl: api.baseUrl,
          error: `Failed to list lattice workspaces from server. (${apiError})`,
        },
      };
    }
    ...
    if ("error" in fileResult) {
      return {
        ...
        status: {
          mode: "api",
          baseUrl: api.baseUrl,
          error: `Failed to list lattice workspaces from server. (${apiError}). Additionally, reading local workspaces failed. (${fileResult.error})`,
        },
      };
    }
    ...
    return {
      ...
      status: {
        mode: "api",
        baseUrl: api.baseUrl,
        error: `Failed to list lattice workspaces from server; falling back to local file access. (${apiError})`,
      },
    };
  }
}
...
function renderChatViewHtml(
  ...
  const scriptUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "out", "latticeChatView.js")
  );
  const styleUri = webview.asWebviewUri(
    vscode.Uri.joinPath(extensionUri, "out", "latticeChatView.css")
  );
  ...
  const csp = [
    ...
    // Many lattice components use inline styles (e.g., FileIcon).
...
<html lang="en">
  ...
  <body data-lattice-trace-id="${traceId}">
    <div id="root"></div>
  ...
  latticeLogDebug("lattice.chatView: renderChatViewHtml", {
    traceId,
...
class LatticeChatViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  dispose(): void {
    ...
    for (const [streamId, stream] of this.activeOrpcStreams.entries()) {
      ...
      void stream.iterator.return?.().catch((error) => {
        latticeLogWarn("lattice.chatView: stream iterator return failed during dispose", {
          streamId,
          error: formatError(error),
        });
      });
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    latticeLogDebug("lattice.chatView: resolveWebviewView", { visible: view.visible });

    // New view instance; clear any previous timers.
    this.clearReadyProbeInterval();

    this.traceId = randomBytes(8).toString("hex");
    latticeLogDebug("lattice.chatView: traceId assigned", { traceId: this.traceId });
    ...
    try {
      ...
      latticeLogDebug("lattice.chatView: webview.options set", {
      ...
      const visibilityDisposable = view.onDidChangeVisibility(async () => {
        latticeLogDebug("lattice.chatView: view visibility changed", { visible: view.visible });
      ...
      const messageDisposable = view.webview.onDidReceiveMessage((msg: unknown) => {
        ...
        const meta =
          typeof msg === "object" && msg !== null && "__latticeMeta" in msg
            ? (msg as { __latticeMeta?: unknown }).__latticeMeta
            : undefined;

        latticeLogDebug("lattice.chatView: <- webview message", {
        ...
        void this.onWebviewMessage(msg).catch((error) => {
          latticeLogError("lattice.chatView: error handling webview message", {
            error: formatError(error),
          });
          console.error("lattice.chatView: error handling webview message", error);
          this.postMessage({
            type: "uiNotice",
            level: "error",
      ...
      view.onDidDispose(() => {
        latticeLogDebug("lattice.chatView: disposed");
      ...
      assert(
        ...
        "lattice.chatView: traceId must be set before rendering webview"
      );
      ...
      this.readyProbeInterval = setInterval(() => {
        if (this.view !== view) {
          latticeLogDebug("lattice.chatView: stopping debugProbe (view changed)");
          this.clearReadyProbeInterval();
          return;
        }

        if (this.isWebviewReady) {
          latticeLogDebug("lattice.chatView: stopping debugProbe (ready received)");
          this.clearReadyProbeInterval();
          return;
        }
        ...
        void view.webview.postMessage({ type: "debugProbe", attempt, sentAtMs }).then(
          (delivered) => {
            latticeLogDebug("lattice.chatView: -> debugProbe", {
              traceId: this.traceId,
              attempt,
              delivered,
            });
          ...
          (error) => {
            latticeLogWarn("lattice.chatView: debugProbe postMessage failed", {
              traceId: this.traceId,
              attempt,
              error: formatError(error),
            });
        ...
        if (attempt >= 15) {
          latticeLogWarn("lattice.chatView: stopping debugProbe after max attempts", {
            maxAttempts: attempt,
          });
          this.clearReadyProbeInterval();
        }
      ...
      const readyWarnTimeout = setTimeout(() => {
        ...
        latticeLogWarn("lattice.chatView: webview has not sent ready after 2s", {
          traceId: this.traceId,
          visible: view.visible,
          cspSource: view.webview.cspSource,
          hint: "Open Webview Developer Tools and look for CSP/script errors; also check Output > Lattice.",
        });
      }, 2_000);
      ...
      const readyErrorTimeout = setTimeout(() => {
        ...
        latticeLogError("lattice.chatView: webview has not sent ready after 10s", {
          traceId: this.traceId,
          visible: view.visible,
          cspSource: view.webview.cspSource,
          hint: "Open Webview Developer Tools and look for CSP/script errors; also check Output > Lattice.",
      ...
      latticeLogError("lattice.chatView: resolveWebviewView failed", {
        traceId: this.traceId,
        error: formatError(error),
      });
      ...
      try {
        view.webview.html =
          '<!DOCTYPE html><html lang="en"><body><h3>Failed to load lattice chat view</h3><p>Check Output > Lattice for details.</p></body></html>';
      } catch {
  ...
  private postMessage(message: ExtensionToWebviewMessage): void {
    ...
    if (!this.view) {
      if (shouldLog) {
        latticeLogDebug("lattice.chatView: -> drop postMessage (no view)", {
          traceId: this.traceId,
          type: message.type,
        });
      }
    ...
    if (!this.isWebviewReady) {
      if (shouldLog) {
        latticeLogDebug("lattice.chatView: -> drop postMessage (webview not ready)", {
          traceId: this.traceId,
          type: message.type,
        });
      }
    ...
    const envelope: Record<string, unknown> = { __latticeMeta: meta, ...message };

    void this.view.webview.postMessage(envelope).then(
      (delivered) => {
        if (shouldLog) {
          latticeLogDebug("lattice.chatView: -> postMessage", {
            traceId: this.traceId,
            seq,
            type: message.type,
            delivered,
      ...
      (error) => {
        latticeLogWarn("lattice.chatView: postMessage failed", {
          traceId: this.traceId,
          seq,
          type: message.type,
          error: formatError(error),
  ...
  private async onWebviewMessage(raw: unknown): Promise<void> {
    ...
    switch (msg.type) {
      case "debugLog":
        latticeLogDebug(`lattice.chatView(webview): ${msg.message}`, msg.data);
        return;

      case "copyDebugLog": {
        const text = msg.text;
        latticeLogInfo("lattice.chatView: copyDebugLog requested", {
          traceId: this.traceId,
          length: text.length,
        });
        ...
        this.postMessage({
          type: "uiNotice",
          level: "info",
          message: "Copied lattice debug log to clipboard.",
        });
        return;
      }
      ...
      case "ready":
        latticeLogDebug("lattice.chatView: ready handshake received", { traceId: this.traceId });
  ...
  private async refreshWorkspaces(): Promise<void> {
    ...
    latticeLogDebug("lattice.chatView: refreshWorkspaces start", { traceId: this.traceId, generation });

    try {
      const result = await getWorkspacesForSidebar(this.context);

      if (generation !== this.refreshWorkspacesGeneration) {
        latticeLogDebug("lattice.chatView: refreshWorkspaces stale result discarded", {
          traceId: this.traceId,
          generation,
        });
        return;
      ...
      latticeLogDebug("lattice.chatView: refreshWorkspaces done", {
        traceId: this.traceId,
      ...
      latticeLogError("lattice.chatView: refreshWorkspaces failed", {
        traceId: this.traceId,
        durationMs: Date.now() - startedAt,
        error: formatError(error),
      });

      const message = `Failed to load lattice workspaces. (${formatError(error)})`;
  ...
  private handleOrpcStreamCancel(streamId: string): void {
    ...
    void stream.iterator.return?.().catch((error) => {
      latticeLogWarn("lattice.chatView: stream iterator return failed", {
        streamId,
        error: formatError(error),
      });
    });
...
async function maybeAutoRevealChatViewFromPendingSelection(
  context: vscode.ExtensionContext,
  provider: LatticeChatViewProvider
): Promise<void> {
