import { Config } from "lattice/node/config";
import assert from "node:assert";

import type { FrontendWorkspaceMetadata, WorkspaceActivitySnapshot } from "lattice/common/types/workspace";
import { type ExtensionMetadata, readExtensionMetadata } from "lattice/node/utils/extensionMetadata";
import { createRuntime } from "lattice/node/runtime/runtimeFactory";
...
export async function getAllWorkspacesFromFiles(options?: {
  ...
  const workspaces = await promiseWithTimeout(
    ...
    timeoutMs,
    "Read lattice workspaces from files"
  );
...
export async function getAllWorkspacesFromApi(
  ...
  const [workspaces, activityById] = await Promise.all([
    promiseWithTimeout(client.workspace.list(), timeoutMs, "lattice API workspace.list"),
    promiseWithTimeout(
      client.workspace.activity.list() as Promise<Record<string, WorkspaceActivitySnapshot>>,
      timeoutMs,
      "lattice API workspace.activity.list"
    ),
  ]);
