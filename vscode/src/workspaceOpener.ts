...
import { WorkspaceWithContext, getWorkspacePath } from "./latticeConfig";
...
export async function openWorkspace(workspace: WorkspaceWithContext) {
  ...
  if (!isRemoteSshInstalled()) {
    const selection = await vscode.window.showErrorMessage(
      'lattice: The "Remote - SSH" extension is required to open SSH workspaces. ' +
  ...
  if (workspace.runtimeConfig.type !== "ssh") {
    ...
    vscode.window.showErrorMessage("lattice: Unknown workspace runtime type.");
    return;
  }
  ...
  try {
    ...
    const selection = await vscode.window.showErrorMessage(
      `lattice: Failed to open SSH workspace on host "${host}". ` +
