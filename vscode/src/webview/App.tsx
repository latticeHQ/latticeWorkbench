...
import type { WorkspaceChatMessage } from "lattice/common/orpc/types";
import type { DisplayedMessage } from "lattice/common/types/message";
import { createClient } from "lattice/common/orpc/client";

import { ProviderOptionsProvider } from "lattice/browser/contexts/ProviderOptionsContext";
import { SettingsProvider } from "lattice/browser/contexts/SettingsContext";
import { APIProvider } from "lattice/browser/contexts/API";
import { ThemeProvider } from "lattice/browser/contexts/ThemeContext";
import { ChatHostContextProvider } from "lattice/browser/contexts/ChatHostContext";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "lattice/browser/components/ui/tooltip";
import { Button } from "lattice/browser/components/ui/button";
import { matchesKeybind, KEYBINDS } from "lattice/browser/utils/ui/keybinds";
import { readPersistedState } from "lattice/browser/hooks/usePersistedState";
import { VIM_ENABLED_KEY } from "lattice/common/constants/storage";
import { useAutoScroll } from "lattice/browser/hooks/useAutoScroll";
import { applyWorkspaceChatEventToAggregator } from "lattice/browser/utils/messages/applyWorkspaceChatEventToAggregator";
import { StreamingMessageAggregator } from "lattice/browser/utils/messages/StreamingMessageAggregator";
            ...
            const forceCatchUp = () => {
              ...
              if (!replayState.didWarnBufferOverflow) {
                ...
                pushNotice({
                  level: "error",
                  message:
                    "Lattice chat did not finish loading (missing caught-up). Showing a partial transcript; try Refresh if messages look incomplete.",
                });
              }
            ...
            >
              <div ref={innerRef}>
                ...
                {!selectedWorkspaceId && notices.length === 0 ? (
                  <div className="text-muted text-sm">Select a lattice workspace to view messages.</div>
                ) : null}
              </div>
            </div>
            ...
            <div className="border-t border-border bg-background-secondary p-3">
              {selectedWorkspaceId ? (
                <ChatComposer
                  ...
                  workspaceId={selectedWorkspaceId}
                  disabled={!canChat}
                  disabledReason={canChat ? undefined : "Chat requires lattice server connection."}
                  aggregator={aggregatorRef.current}
                  onSendComplete={jumpToBottom}
                  onNotice={pushNotice}
                />
              ) : (
                <div className="text-muted text-sm">Select a lattice workspace to chat.</div>
              )}
            </div>
          </div>
            </TooltipProvider>
