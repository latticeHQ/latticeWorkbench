...
import type { StreamingMessageAggregator } from "lattice/browser/utils/messages/StreamingMessageAggregator";
import { getSendOptionsFromStorage } from "lattice/browser/utils/messages/sendOptions";

import { matchesKeybind, formatKeybind, KEYBINDS } from "lattice/browser/utils/ui/keybinds";
import { useAPI } from "lattice/browser/contexts/API";
import { AgentProvider, useAgent } from "lattice/browser/contexts/AgentContext";
import { ThinkingProvider } from "lattice/browser/contexts/ThinkingContext";
import { useThinkingLevel } from "lattice/browser/hooks/useThinkingLevel";
import { usePersistedState } from "lattice/browser/hooks/usePersistedState";
import { useModelsFromSettings } from "lattice/browser/hooks/useModelsFromSettings";
import { migrateGatewayModel } from "lattice/browser/hooks/useGatewayModels";
import { useProviderOptions } from "lattice/browser/hooks/useProviderOptions";
import { useAutoCompactionSettings } from "lattice/browser/hooks/useAutoCompactionSettings";

import { VimTextArea } from "lattice/browser/components/VimTextArea";
import { ModelSelector } from "lattice/browser/components/ModelSelector";
import { ThinkingSliderComponent } from "lattice/browser/components/ThinkingSlider";
import { ContextUsageIndicatorButton } from "lattice/browser/components/ContextUsageIndicatorButton";
import { Tooltip, TooltipTrigger, TooltipContent } from "lattice/browser/components/ui/tooltip";

import type { AgentId } from "lattice/common/orpc/schemas";

import { calculateTokenMeterData } from "lattice/common/utils/tokens/tokenMeterUtils";
import { createDisplayUsage } from "lattice/common/utils/tokens/displayUsage";
import type { ChatUsageDisplay } from "lattice/common/utils/tokens/usageAggregator";
import { enforceThinkingPolicy } from "lattice/common/utils/thinking/policy";
import { cn } from "lattice/common/lib/utils";
import { VIM_ENABLED_KEY, getInputKey, getModelKey } from "lattice/common/constants/storage";
...
function ChatComposerInner(props: {
  ...
  const onSend = async () => {
    ...
    if (!api) {
      props.onNotice({ level: "error", message: "Not connected to lattice server." });
      return;
    }
