import assert from "lattice/common/utils/assert";
...
export function getVscodeBridge(): VscodeBridge {
  ...
  const traceId =
    (document.body && document.body.dataset && document.body.dataset.latticeTraceId) || "unknown";
  ...
  try {
    ...
    throw new Error(`lattice webview: acquireVsCodeApi failed: ${String(error)}`);
  }
  ...
  window.addEventListener("message", (event: MessageEvent) => {
    for (const handler of listeners) {
      try {
        ...
        console.error("lattice webview: message handler crashed", error);
      }
    }
  });
  ...
  const postMessage = (payload: WebviewToExtensionMessage): void => {
    ...
    const envelope: Record<string, unknown> = { __latticeMeta: meta, ...payload };
  ...
  const debugLog = (message: string, data?: unknown): void => {
    ...
    // eslint-disable-next-line no-console
    console.log(`[lattice-webview ${traceId}] ${message}`, data);
