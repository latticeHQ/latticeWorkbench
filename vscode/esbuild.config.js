...
function resolveLatticeImport(subpath) {
  ...
  // Support directory imports (e.g. lattice/foo -> src/foo/index.tsx).
...
// Plugin to resolve lattice/* imports from parent directory.
const latticeResolverPlugin = {
  name: "lattice-resolver",
  setup(build) {
    build.onResolve({ filter: /^lattice\// }, (args) => {
      const subpath = args.path.replace(/^lattice\//, "");
      const resolved = resolveLatticeImport(subpath);
      if (!resolved) {
        return null;
      }
...
function buildWebviewCss() {
  ...
  webviewCssBuildPromise = (async () => {
    ...
    const outputPath = path.resolve(__dirname, "out", "latticeChatView.css");
...
// Support Vite-style SVG React imports ("*.svg?react") used by lattice UI.
...
// The lattice markdown renderer imports KaTeX CSS.
...
const webviewBuild = {
  entryPoints: {
    latticeChatView: "src/webview/index.tsx",
  },
  bundle: true,
  outdir: "out",
...
async function main() {
  ...
  if (isWatch) {
    ...
    console.log("lattice VS Code extension: watching for changes...");
    return;
  }
