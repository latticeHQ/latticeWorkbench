// VS Code webview script for the lattice secondary sidebar chat view.
//
...
(function () {
  ...
  const traceId = (document.body && document.body.dataset && document.body.dataset.latticeTraceId) || "unknown";
  ...
  function appendDebug(message, data) {
    ...
    try {
      ...
      console.log("[lattice-webview]", line);
    } catch {
      // ignore
    }
  }
  ...
  setStatus(`lattice webview: js loaded (trace ${traceId})`);
  appendDebug("boot", {
    traceId,
  ...
  try {
    ...
    setStatus(`lattice webview: acquireVsCodeApi failed: ${String(error)}`);
    return;
  }
  ...
  function postToExtension(payload) {
    ...
    const message = Object.assign({ __latticeMeta: meta }, payload);
  ...
  function updateControls() {
    ...
    if (inputEl) {
      ...
      inputEl.placeholder = canChat
        ? "Message lattice…"
        : hasSelection
          ? "Chat requires lattice server connection."
          : "Select a lattice workspace to chat.";
    }
  }
  ...
  function setConnectionStatus(status) {
    ...
    const parts = [];
    if (status.mode === "api") {
      parts.push("Connected to lattice server");
      if (status.baseUrl) {
  ...
  window.addEventListener("message", (ev) => {
    ...
    if (typeof msg.type === "string" && msg.type !== "chatEvent") {
      appendDebug(`rx ${msg.type}`, msg.__latticeMeta);
    }

    if (msg.type === "debugProbe") {
      appendDebug("rx debugProbe", msg);
      setStatus(`lattice webview: received debugProbe #${String(msg.attempt || "?")}`);
