...
done < <(grep -oE "https://docs\.lattice\.com[^[:space:])\"']*" README.md 2>/dev/null || true)
...
done < <(grep -rn --include="*.ts" --include="*.tsx" -oE "https://docs\.lattice\.com[^[:space:])\"']*" src/ 2>/dev/null || true)
