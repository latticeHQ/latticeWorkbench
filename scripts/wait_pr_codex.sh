...
REACTIONS_SCAN_CACHE_FILE="${LATTICE_REACTIONS_SCAN_CACHE_FILE:-}"
...
if [[ -n "$PR_DATA_FILE" ]]; then
  if [ ! -e "$PR_DATA_FILE" ] && ! : >"$PR_DATA_FILE"; then
    echo "❌ assertion failed: unable to create LATTICE_PR_DATA_FILE at '$PR_DATA_FILE'" >&2
    exit 1
  fi

  if [ ! -w "$PR_DATA_FILE" ]; then
    echo "❌ assertion failed: LATTICE_PR_DATA_FILE is not writable: '$PR_DATA_FILE'" >&2
    exit 1
  fi
fi
