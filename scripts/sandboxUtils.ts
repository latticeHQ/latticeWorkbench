...
export function chooseSeedLatticeRoot(): string | null {
  if (process.env.SEED_LATTICE_ROOT) {
    const explicit = expandTilde(process.env.SEED_LATTICE_ROOT);
    if (!dirExists(explicit)) {
      throw new Error(`SEED_LATTICE_ROOT does not exist or is not a directory: ${explicit}`);
    }
    return explicit;
  }

  const candidates = [
    process.env.LATTICE_ROOT ? expandTilde(process.env.LATTICE_ROOT) : null,
    path.join(os.homedir(), ".lattice-dev"),
