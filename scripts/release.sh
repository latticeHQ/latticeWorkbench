...
# ── Pull latest submodules (latticeInference, etc.) ─────────────────────────
info "Pulling latest submodules..."
git submodule update --init --recursive
if [[ -d vendor/latticeInference ]]; then
  PREV_COMMIT=$(cd vendor/latticeInference && git rev-parse --short HEAD)
  (cd vendor/latticeInference && git checkout main && git pull origin main)
  NEW_COMMIT=$(cd vendor/latticeInference && git rev-parse --short HEAD)
  if [[ "$PREV_COMMIT" != "$NEW_COMMIT" ]]; then
    info "latticeInference updated: $PREV_COMMIT -> $NEW_COMMIT"
  else
    info "latticeInference already at latest ($NEW_COMMIT)"
  fi
fi
...
# Set RELEASE_TAG so generate-version.sh stamps a clean version (no -dirty)
export RELEASE_TAG="v${NEW_VERSION}"

if [[ "$SKIP_BUILD" == "false" ]]; then
  info "Building application (RELEASE_TAG=$RELEASE_TAG)..."
  make build
else
...
for candidate in \
  ...
  if [[ -f "$candidate/Casks/lattice-workbench.rb" ]]; then
    HOMEBREW_TAP_DIR="$candidate"
    break
  fi
done

if [[ -n "$HOMEBREW_TAP_DIR" ]]; then
  info "Updating Homebrew cask at $HOMEBREW_TAP_DIR..."

  # Compute SHA256 for the zip files
  ARM64_ZIP="release/Lattice-${NEW_VERSION}-arm64.zip"
  X64_ZIP="release/Lattice-${NEW_VERSION}-x64.zip"
  CASK_FILE="$HOMEBREW_TAP_DIR/Casks/lattice-workbench.rb"

  if [[ -f "$ARM64_ZIP" && -f "$X64_ZIP" ]]; then
    ARM64_SHA=$(shasum -a 256 "$ARM64_ZIP" | awk '{print $1}')
    X64_SHA=$(shasum -a 256 "$X64_ZIP" | awk '{print $1}')

    # Update version and hashes in the cask file using sed
    sed -i '' "s/version \".*\"/version \"${NEW_VERSION}\"/" "$CASK_FILE"
    # arm64 hash is the first sha256 line, x64 is the second
    sed -i '' "0,/sha256 \"[a-f0-9]*\"/s/sha256 \"[a-f0-9]*\"/sha256 \"${ARM64_SHA}\"/" "$CASK_FILE"
    sed -i '' "/on_intel/,/end/{s/sha256 \"[a-f0-9]*\"/sha256 \"${X64_SHA}\"/;}" "$CASK_FILE"

    info "  Cask updated: v${NEW_VERSION} arm64=$ARM64_SHA x64=$X64_SHA"

    # Commit and push
    (cd "$HOMEBREW_TAP_DIR" && \
      git add Casks/lattice-workbench.rb && \
      git commit -m "Update Lattice Workbench to v${NEW_VERSION}" && \
      git push origin main) && \
    info "  Homebrew tap pushed" || \
    warn "  Failed to push Homebrew tap — update manually"
  else
    warn "  Zip files not found — skipping Homebrew cask update"
  fi
else
  warn "Homebrew tap not found locally — update Casks/lattice-workbench.rb manually"
fi
...
if ! git diff --cached --quiet; then
  ...
  git push origin main
  info "Pushed release commit"
else
  info "No uncommitted changes to push"
fi

info "Done! Lattice v${NEW_VERSION} released."
info "  Release: $RELEASE_URL"
info "  Install: brew install --cask latticehq/lattice/lattice-workbench"
