...
const SCREENSHOTS_DIR = path.resolve(import.meta.dirname, "..", "static", "img");
...
const STORIES: StoryDef[] = [
  // README hero uses lattice-demo.gif; screenshot captures start at code-review.webp.
  {
  ...
  },
  {
    ...
    playInteraction: async (page: Page) => {
      ...
      const projectRow = page
        .locator('[data-project-path="/home/user/projects/lattice"][aria-controls]')
        .first();
...
async function main() {
  ...
  for (const story of storiesToCapture) {
    const outputPath = path.join(SCREENSHOTS_DIR, story.outputFile);
