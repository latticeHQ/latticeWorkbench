...
import type { LatticeMessage, LatticeTextPart, LatticeFilePart } from "@/common/types/message";
import type { DynamicToolPart } from "@/common/types/toolParts";
import type { WorkspaceChatMessage } from "@/common/orpc/types";
import { isLatticeMessage } from "@/common/orpc/types";
 ...
 * This is derived from WorkspaceChatMessage excluding LatticeMessage (which uses `role`).
 *
 ...
 */
type TypedEventType =
  Exclude<WorkspaceChatMessage, LatticeMessage> extends infer T
...
/**
 * Transform LatticeMessage into DisplayedMessage array.
 * Handles merging adjacent text/reasoning parts and extracting tool calls.
 */
function transformLatticeToDisplayed(message: LatticeMessage): DisplayedMessage[] {
  ...
  if (message.role === "user") {
    const content = message.parts
      .filter((p): p is LatticeTextPart => p.type === "text")
      .map((p) => p.text)
      .join("");
...
export function createChatEventExpander(): ChatEventExpander {
  ...
  const emitDisplayedMessages = (
    ...
    const displayed = transformLatticeToDisplayed(message);
  ...
  const expandSingle = (payload: IncomingEvent | undefined): WorkspaceChatEvent[] => {
    ...
    if (isObject(payload)) {
      ...
      if (isLatticeMessage(candidate)) {
        const latticeMessage: LatticeMessage = candidate;
        const historySequence = latticeMessage.metadata?.historySequence;

        if (typeof historySequence !== "number" || !Number.isFinite(historySequence)) {
          console.warn(`${DEBUG_TAG} Dropping lattice message without historySequence`, {
            id: latticeMessage.id,
        ...
        processor.handleEvent(latticeMessage as WorkspaceChatMessage);
        const displayed = transformLatticeToDisplayed(latticeMessage);
