...
const STORAGE_KEY_BASE_URL = "com.lattice.lattice.app-settings.baseUrl";
const STORAGE_KEY_AUTH_TOKEN = "com.lattice.lattice.app-settings.authToken";
const DEFAULT_BASE_URL = "http://localhost:3000";
const URL_SCHEME_REGEX = /^[a-z][a-z0-9+.-]*:\/\//i;

interface ExpoLatticeExtra {
  baseUrl?: string;
  authToken?: string;
}
...
function readExpoLatticeExtra(): ExpoLatticeExtra {
  const extra = Constants.expoConfig?.extra;
  if (!extra || typeof extra !== "object") {
    return {};
  }
  const latticeExtra = (extra as { lattice?: unknown }).lattice;
  if (!latticeExtra || typeof latticeExtra !== "object" || Array.isArray(latticeExtra)) {
    return {};
  }
  const record = latticeExtra as Record<string, unknown>;
  return {
...
export function AppConfigProvider({ children }: { children: ReactNode }): JSX.Element {
  const expoDefaults = useMemo(() => readExpoLatticeExtra(), []);
