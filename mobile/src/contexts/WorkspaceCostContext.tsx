...
import type { LatticeMessage } from "@/common/types/message.ts";
import type { WorkspaceChatMessage } from "@/common/orpc/types";
import { isLatticeMessage, isStreamEnd } from "@/common/orpc/types";
...
function extractMessagesFromReplay(events: WorkspaceChatMessage[]): LatticeMessage[] {
  const messages: LatticeMessage[] = [];
  for (const event of events) {
    if (isLatticeMessage(event)) {
      messages.push(event as unknown as LatticeMessage);
    }
  }
  return messages;
}

function getLastModel(messages: LatticeMessage[]): string | undefined {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        for (const event of events) {
          if (isLatticeMessage(event as unknown as WorkspaceChatMessage)) {
            const message = event as unknown as LatticeMessage;
