...
Expo React Native app for lattice - connects to lattice server via ORPC over HTTP with SSE streaming.
...
Start the lattice server with auth (optional):

```bash
# In the main lattice repo
LATTICE_SERVER_AUTH_TOKEN=your_token make dev-server BACKEND_HOST=0.0.0.0 BACKEND_PORT=3000
```
...
**Connection refused**: Make sure the lattice server is running and accessible from your device (use your machine's Tailscale IP or local network IP, not `localhost`).
