...
const devServerHost = process.env.LATTICE_VITE_HOST ?? "127.0.0.1"; // Secure by default
const devServerPort = Number(process.env.LATTICE_VITE_PORT ?? "5173");

const devServerAllowedHosts = (() => {
  const raw = process.env.LATTICE_VITE_ALLOWED_HOSTS?.trim();
  if (raw) {
    if (raw === "true" || raw === "all") {
      return true;
    }
  ...
  // Default to localhost-only. For remote access, set LATTICE_VITE_ALLOWED_HOSTS (or
...
const previewPort = Number(process.env.LATTICE_VITE_PREVIEW_PORT ?? "4173");
...
const backendProxyHost = process.env.LATTICE_BACKEND_HOST ?? "127.0.0.1";
const backendProxyPort = Number(process.env.LATTICE_BACKEND_PORT ?? "3000");
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      host: devServerHost, // Configurable via LATTICE_VITE_HOST (defaults to 127.0.0.1 for security)
      port: devServerPort,
      strictPort: true,
