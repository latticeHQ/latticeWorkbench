...
const isE2ETest = process.env.LATTICE_E2E === "1";
const forceDistLoad = process.env.LATTICE_E2E_LOAD_DIST === "1";

if (isE2ETest) {
  // For e2e tests, use a test-specific userData directory
  // Note: getLatticeHome() already respects LATTICE_ROOT for test isolation
  const e2eUserData = path.join(getLatticeHome(), "user-data");
  try {
...
const devServerPort = process.env.LATTICE_DEVSERVER_PORT ?? "5173";

console.log(
  `Lattice starting - version: ${(VERSION as { git?: string; buildTime?: string }).git ?? "(dev)"} (built: ${(VERSION as { git?: string; buildTime?: string }).buildTime ?? "dev-mode"})`
);
console.log("Main process starting...");

// Debug: abort immediately if LATTICE_DEBUG_START_TIME is set
// This is used to measure baseline startup time without full initialization
if (process.env.LATTICE_DEBUG_START_TIME === "1") {
  console.log("LATTICE_DEBUG_START_TIME is set - aborting immediately");
  process.exit(0);
}
...
// Single instance lock (can be disabled for development with CLATTICE_ALLOW_MULTIPLE_INSTANCES=1)
const allowMultipleInstances = process.env.CLATTICE_ALLOW_MULTIPLE_INSTANCES === "1";
...
async function loadServices(): Promise<void> {
  ...
  const authToken = process.env.LATTICE_SERVER_AUTH_TOKEN ?? process.env.LATTICE_SERVER_AUTH_TOKEN ?? randomBytes(32).toString("hex");
  ...
  electronIpcMain.handle("lattice:get-is-windows-wsl-shell", async () => {
  ...
  if (process.env.LATTICE_NO_API_SERVER !== "1") {
    ...
    if (existing) {
      ...
      try {
        ...
        const envPortRaw = process.env.LATTICE_SERVER_PORT
...
function createWindow() {
  ...
  if ((isE2ETest && !forceDistLoad) || (!app.isPackaged && !forceDistLoad)) {
    ...
    const devHost = process.env.LATTICE_DEVSERVER_HOST ?? "127.0.0.1";
