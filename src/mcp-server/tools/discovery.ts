...
const CATEGORIES: Record<string, string> = {
  ...
  browser: "[Minion-scoped] Full-strength headless browser via agent-browser (46 tools): navigate, snapshot (accessibility tree with refs), screenshot, annotated screenshot, screenshot element, screenshot diff, snapshot diff, click, fill, type, press, hover, scroll (directional, pixel, element), find (semantic locators), wait, eval (JS), console logs, PDF export, viewport/device emulation, geolocation, permissions, tabs, dialogs, cookies (CRUD), network requests, set headers, intercept network, offline mode, drag, select, save/restore state (AES-256-GCM), localStorage/sessionStorage CRUD, session recording, cloud providers (Browserbase/Browserless/BrowserUse/Kernel), session config (headed/proxy/userAgent). Live WebSocket streaming for interactive use. Each minion gets its own isolated browser. Parent: minion.",
  "code-execution": "[Global] Code execution: run TypeScript with pre-imported Lattice SDK (execute_code), read SDK module source (read_sdk_module), and manage reusable skills (save_skill, list_skills, run_skill, delete_skill). Use this for complex multi-step workflows instead of chaining individual tool calls.",
