...
- "run-e2e-tests" — test execution with retry logic`,
    {
      ...
      instructions: z
        .string()
        .optional()
        .describe("SKILL.md content — usage instructions, requirements, examples"),
      tags: z
        .array(z.string())
        .optional()
        .describe("Tags for discovery (e.g., ['deploy', 'vercel', 'nextjs'])"),
      author: z.string().optional().describe("Who created this (agent ID or 'human')"),
      requirements: z
        .array(z.string())
        .optional()
        .describe("Prerequisites (e.g., ['vercel CLI installed', 'VERCEL_TOKEN set'])"),
      inputSchema: z
        .record(z.string(), z.string())
        .optional()
        .describe("Input parameters: { paramName: 'type description' }"),
    },
  ...
  server.tool(
    ...
    async ({ tag, query }) => {
      ...
      if (skills.length === 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: "No skills found. Use skills_save to create your first skill.",
            },
          ],
        };
      }
      ...
      for (const s of skills) {
        ...
        lines.push(
          `   Tags: ${s.metadata.tags.join(", ") || "none"} | Used: ${s.metadata.useCount}x | Last: ${lastUsed}`
        );
  ...
  server.tool(
    ...
    "Read a skill's full code, instructions, and metadata. Use this to understand what a skill does before executing it.",
    {
      ...
      part: z
        .enum(["all", "code", "instructions", "metadata"])
        .optional()
        .describe("Which part to read (default: all)"),
    },
    async ({ name, part }) => {
      const slug = slugify(name);
...
The agent will receive the skill code and instructions, then execute it in the workspace context.`,
    {
      ...
      inputs: z
        .record(z.string(), z.string())
        .optional()
        .describe("Input values to pass to the skill"),
...
Execute this skill code in the workspace. Follow the instructions and use the provided inputs. Report results clearly.`;
        ...
        if (!sendResult.success) {
          return {
            content: [
              { type: "text" as const, text: `Failed to dispatch skill: ${sendResult.error}` },
            ],
            isError: true,
          };
        }
  ...
  server.tool("skills_delete", "Delete a saved skill.", { name: z.string() }, async ({ name }) => {
    const slug = slugify(name);
    const skillDir = path.join(getSkillsDir(), slug);
  ...
  server.tool(
    ...
    async ({ sourceName, newName, newDescription, newCode, newTags }) => {
      ...
      if (!fs.existsSync(sourceDir)) {
        return {
          content: [{ type: "text" as const, text: `Source skill "${sourceSlug}" not found.` }],
          isError: true,
        };
      }
  ...
  server.tool(
    ...
    "Extract and save a skill from a workspace's chat history. Useful when an agent figures out a workflow — capture it as a reusable skill.",
    {
      ...
      messageIndex: z
        .number()
        .optional()
        .describe("Index of the message containing the code (default: last assistant message)"),
    ...
    async ({ workspaceId, name, description, messageIndex, tags }) => {
      try {
        ...
        if (!targetMessage) {
          return {
            content: [{ type: "text" as const, text: "No message found to extract code from." }],
            isError: true,
          };
        }
        ...
        if (codeBlocks.length === 0) {
          ...
          const metadata: SkillMetadata = {
            ...
            description,
            tags: tags ?? [],
            author: `extracted from ${workspaceId}`,
            version: 1,
            createdAt: Date.now(),
            updatedAt: Date.now(),
            useCount: 0,
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `✅ Saved skill "${slug}" (instructions only, no code blocks found)`,
              },
            ],
          };
        }
