  ...
  return {
      ...
      },
      {
        ...
        params: [
          ...
          { name: "message", type: "string", required: true, description: "Message to send" },
          {
            ...
            type: "number",
            required: false,
            description: "Max wait time (default: 120000)",
          },
        ],
        returnType: "string",
      },
    ...
    project: [
      ...
      },
      {
        ...
        category: "project",
        params: [
          { name: "projectPath", type: "string", required: true, description: "Absolute path" },
        ],
        returnType: "{ success: boolean }",
      },
      {
        ...
        category: "project",
        params: [
          { name: "projectPath", type: "string", required: true, description: "Headquarter path" },
        ],
    ...
    channel: [
      ...
      },
      {
        ...
        category: "channel",
        params: [
          { name: "accountId", type: "string", required: true, description: "Channel account ID" },
        ],
        returnType: "{ success: boolean }",
      },
      {
    ...
    system: [
      ...
      },
      {
        ...
        params: [],
        returnType:
          "{ hostname: string; cpus: number; totalMemoryGB: number; freeMemoryGB: number }",
      },
      {
        name: "processes",
    ...
    swarm: [
      {
        ...
        params: [
          ...
          {
            name: "agents",
            type: "Array<{role, systemPrompt?, title?}>",
            required: true,
            description: "Specialists to spawn",
          },
        ],
      ...
      },
      {
        ...
        category: "swarm",
        params: [
          { name: "key", type: "string", required: false, description: "Key (omit for all)" },
        ],
...
function generateToolFile(tool: ToolDef): string {
  ...
  const funcParams = tool.params.length > 0 ? `input: ${capitalize(tool.name)}Input` : "";
...
`;
  ...
  for (const [category, tools] of Object.entries(catalog)) {
    ...
    for (const tool of tools) {
      fs.writeFileSync(path.join(categoryDir, `${tool.name}.ts`), generateToolFile(tool));
    }

    // Generate index.ts
    fs.writeFileSync(path.join(categoryDir, "index.ts"), generateIndexFile(category, tools));
  }
...
${Object.entries(catalog)
  .map(
    ([cat, tools]) =>
      `- **${cat}/** — ${tools.length} tools (${tools.map((t) => t.name).join(", ")})`
  )
  .join("\n")}
  ...
  return {
    workspace: {
      ...
      createWorkspace: async (input) =>
        client.createWorkspace(
          input as { projectPath: string; branchName: string; trunkBranch?: string; title?: string }
        ),
      removeWorkspace: async (input) =>
        client.removeWorkspace(input.workspaceId as string, input.force as boolean | undefined),
      sendMessage: async (input) => {
        ...
        let baseline = 0;
        try {
          baseline = ((await client.getFullReplay(wsId)) as unknown[]).length;
        } catch {}
      ...
      forkWorkspace: async (input) =>
        client.forkWorkspace(
          input.workspaceId as string,
          input.branchName as string,
          input.title as string | undefined
        ),
      renameWorkspace: async (input) =>
        client.renameWorkspace(input.workspaceId as string, input.title as string),
    ...
    system: {
      ...
      listDirectory: async (input) =>
        fs
          .readdirSync(input.path as string, { withFileTypes: true })
      ...
      gitClone: async (input) => {
        ...
        const repoName =
          url
            ...
            ?.replace(/\.git$/, "") ?? "repo";
        const cloneDir =
          (input.targetDir as string) ?? path.join(os.homedir(), "projects", repoName);
...
async function executeCode(
  ...
  const mcpCall = async (server: string, tool: string, input?: unknown): Promise<unknown> => {
    ...
    if (!handler) {
      throw new Error(
        `Unknown tool: ${server}.${tool}. Available: ${Object.keys(bridge)
          .map((s) => `${s}.[${Object.keys(bridge[s]!).join(",")}]`)
          .join(", ")}`
      );
    }
...
function formatExecutionResult(result: ExecutionResult): string {
  ...
  if (result.toolCalls.length > 0) {
    ...
    for (const call of result.toolCalls) {
      ...
      lines.push(
        `  ${call.server}.${call.tool}(${JSON.stringify(call.input)?.slice(0, 80) ?? ""})`
      );
      lines.push(
        `    → ${resultPreview}${resultPreview.length >= 150 ? "…" : ""} (${call.durationMs}ms)`
      );
    }
  }
...
- "full" — complete definition with params, types, examples`,
    {
      query: z
        .string()
        .optional()
        .describe("Search query (keyword, category name, or action verb). Omit for overview."),
      category: z
        .string()
        .optional()
        .describe(
          "Filter by category: workspace, project, channel, system, swarm, bootstrap, cron, health"
        ),
      detail: z
        .enum(["names", "brief", "full"])
        .optional()
        .describe("Detail level (default: brief)"),
    },
    async ({ query, category, detail }) => {
      ...
      const lines: string[] = [];
      lines.push(
        `🔍 Found ${results.length} tools${query ? ` matching "${query}"` : ""}${category ? ` in ${category}` : ""}\n`
      );
        ...
        for (const r of results) {
          ...
          if (r.tool.params.length > 0) {
            ...
            for (const p of r.tool.params) {
              lines.push(
                `  ${p.name}${p.required ? " (required)" : " (optional)"}: ${p.type} — ${p.description}`
              );
            }
          } else {
      ...
      if (!tools) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Category "${category}" not found. Available: ${Object.keys(catalog).join(", ")}`,
            },
          ],
          isError: true,
        };
      ...
      if (!tool) {
        return {
          content: [
            {
              type: "text" as const,
              text: `Tool "${toolName}" not found in ${category}. Available: ${tools.map((t) => t.name).join(", ")}`,
            },
          ],
          isError: true,
        };
  ...
  return active.length;`,
    {
      code: z
        .string()
        .describe("TypeScript/JavaScript code to execute. Has access to all tool APIs."),
...
inspect intermediate results.`,
      ...
      steps: z
        .array(
          z.object({
            ...
            code: z
              .string()
              .describe("Code to execute. Previous step's return is available as 'input'."),
          })
        )
    ...
    async ({ steps, timeoutMs }) => {
      ...
      for (let i = 0; i < steps.length; i++) {
        ...
        if (result.stdout.length > 0)
          lines.push(`   stdout: ${result.stdout.join("; ").slice(0, 200)}`);
...
then persist what works.`,
    {
      ...
      inputSchema: z
        ...
        .optional()
        .describe("Input params: { paramName: 'type description' }"),
    },
      ...
      if (isUpdate) {
        try {
          const existing = JSON.parse(
            fs.readFileSync(path.join(skillDir, "metadata.json"), "utf-8")
          );
