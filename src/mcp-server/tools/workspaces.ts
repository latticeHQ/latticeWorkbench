  ...
  server.tool(
    ...
    "Create a new workspace in a project. Returns the workspace ID.",
    {
      ...
      trunkBranch: z
        .string()
        .optional()
        .describe("Base branch to create from (default: main/master)"),
    },
    async ({ projectPath, branchName, title, trunkBranch }) => {
      try {
        ...
        if (!result.success) {
          return {
            content: [
              { type: "text" as const, text: `Failed to create workspace: ${result.error}` },
            ],
            isError: true,
          };
        }
  ...
  server.tool(
    ...
    async ({ workspaceId }) => {
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: `Stream interrupted in workspace ${workspaceId}.` },
          ],
        };
      } catch (error) {
        return {
