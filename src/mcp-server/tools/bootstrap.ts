...
export function registerBootstrapTools(server: McpServer, client: WorkbenchClient): void {
  ...
  server.tool(
    ...
    "All-in-one: clone a git repo, register it as a project in the workbench, and create a workspace. Returns the workspace ID ready to use with workbench_send_message.",
    {
      ...
      targetDir: z.string().optional().describe("Where to clone (default: ~/projects/<repo-name>)"),
      ...
      try {
        ...
        try {
          ...
          if (createResult.success) {
            ...
            steps.push(`📋 Headquarter registration: ${createResult.error ?? "already exists"}`);
          }
        } catch (error) {
          steps.push(
            `⚠️ Headquarter registration: ${error instanceof Error ? error.message : String(error)}`
          );
        }
        ...
        const branchName = branch ?? `agent-${Date.now().toString(36)}`;
    ...
    async ({ projectPath, workspaceTitle }) => {
      try {
        ...
        if (!fs.existsSync(path.join(projectPath, ".git"))) {
          ...
          await shellExec('git commit --allow-empty -m "Initial commit"', projectPath);
        ...
        try {
          ...
          steps.push("📋 Headquarter already registered");
        }
