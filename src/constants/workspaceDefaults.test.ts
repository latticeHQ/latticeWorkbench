...
describe("WORKSPACE_DEFAULTS", () => {
  ...
  test("model should follow provider:model format", () => {
    // Provider is a CLI agent slug (e.g. "claude-code"),
    // model part may contain hyphens and dots (e.g. "claude-sonnet-4-5").
    expect(WORKSPACE_DEFAULTS.model).toMatch(/^[a-z][a-z0-9-]*:[a-z0-9][a-z0-9.-]*$/);
  });
