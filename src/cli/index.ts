 ...
 * In packaged apps (`./lattice.AppImage`), argv = [app, ...args] so first arg is at index 1.
...
if (subcommand === "run") {
  if (!isCommandAvailable("run", env)) {
    console.error("The 'run' command is only available via the CLI (bun lattice run).");
    console.error("It is not bundled in Electron.");
    process.exit(1);
  }
  process.argv.splice(env.firstArgIndex, 1); // Remove "run" since run.ts defines .name("lattice run")
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  require("./run");
  ...
  // Explicit `lattice desktop`, or Electron runtime with no subcommand / Electron launch args
  if (!isCommandAvailable("desktop", env)) {
    ...
    console.error("Download from: https://github.com/latticeHQ/latticeWorkbench/releases");
    process.exit(1);
  }
  launchDesktop();
} else {
  ...
  program
    .name("lattice")
    .description("Lattice - AI agent orchestration")
  ...
  program.command("api").description("Interact with the lattice API via a running server");
  if (isCommandAvailable("desktop", env)) {
    program
      .command("desktop")
      .description(
