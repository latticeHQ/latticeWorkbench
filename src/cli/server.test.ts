...
async function createTestServer(): Promise<TestServerHandle> {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-server-test-"));
  ...
  const context: ORPCContext = {
    ...
    latticeInferenceClusterService: services.latticeInferenceClusterService,
    ...
    openbbService: services.openbbService,
...
describe("oRPC Server Endpoints", () => {
  ...
  describe("HTTP endpoint (/orpc)", () => {
    ...
    test("agentSkills.list and agentSkills.get work with projectPath", async () => {
      ...
      const projectPath = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-agent-skills-project-"));
