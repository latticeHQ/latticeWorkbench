#!/usr/bin/env bun
/**
 * `lattice run` - First-class CLI for running agent sessions
 *
 * Usage:
 *   lattice run "Fix the failing tests"
 *   lattice run --dir /path/to/project "Add authentication"
 *   lattice run --runtime "ssh user@host" "Deploy changes"
 */
...
function parseRuntimeConfig(value: string | undefined, srcBaseDir: string): RuntimeConfig {
  if (!value) {
    // Default to local for `lattice run` (no worktree isolation needed for one-off)
    return { type: "local" };
  }
...
Examples:
  ...
  $ lattice run --dir /path/to/project "Add authentication"
  $ lattice run --runtime "ssh user@host" "Deploy changes"
  $ lattice run --mode plan "Refactor the auth module"
  $ lattice run --budget 1.50 "Quick code review"
  $ echo "Add logging" | lattice run
  $ lattice run --json "List all files" | jq '.type'
  $ lattice run --mcp "memory=npx -y @modelcontextprotocol/server-memory" "Remember this"
  $ lattice run --mcp "chrome=npx chrome-devtools-mcp" --mcp "fs=npx @anthropic/mcp-fs" "Take a screenshot"
`
  );
...
async function main(): Promise<number> {
  ...
  if (!message) {
    ...
    console.error('Usage: lattice run "Your instruction here"');
    process.exit(1);
  }

  // Create ephemeral temp dir for session data (auto-cleaned on exit)
  using tempDir = new DisposableTempDir("lattice-run");
  ...
  const writeThinking = (text: string) => {
    ...
    // Purple color matching Lattice UI thinking blocks (hsl(271, 76%, 53%) = #A855F7)
  ...
  const setExitCodeTool = tool({
    description:
      ...
      "Only available in `lattice run` CLI mode.",
