/**
 * Integration tests for `lattice run` CLI command.
 *
...
describe("lattice CLI", () => {
  ...
  describe("top-level", () => {
    test("--help shows usage", async () => {
      ...
      expect(result.stdout).toContain("Usage: lattice");
      expect(result.stdout).toContain("Lattice - AI agent orchestration");
  ...
  describe("lattice run", () => {
    test("--help shows all options", async () => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain("Usage: lattice run");
    ...
    test("shows default model in help output", async () => {
      const result = await runCli(["run", "--help"]);
      expect(result.exitCode).toBe(0);
      // Default model is derived from DEFAULT_MODEL in knownModels.ts
      expect(result.stdout).toContain("claude-code:claude-sonnet-4-5");
    });
  ...
  describe("lattice server", () => {
    test("--help shows all options", async () => {
      const result = await runCli(["server", "--help"]);
      expect(result.exitCode).toBe(0);
      expect(result.stdout).toContain("Usage: lattice server");
