...
program
  ...
  .option("--server-url <url>", "URL of a running lattice server")
  ...
  .action(async (options: Record<string, unknown>) => {
    ...
    console.error("[acp] Connecting to lattice server…");
    const connection = await connectToServer({
      serverUrl: serverUrl ?? process.env.LATTICE_SERVER_URL,
      authToken: authToken ?? process.env.LATTICE_SERVER_AUTH_TOKEN,
    });
