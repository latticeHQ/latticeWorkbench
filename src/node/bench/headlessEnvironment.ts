...
async function establishRootDir(providedRootDir?: string): Promise<{
  ...
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-headless-"));
  return {
    rootDir: tempRoot,
    dispose: async () => {
...
export async function createHeadlessEnvironment(
  ...
  return {
    config,
    services,
    mockIpcMain: mockIpcMainModule as unknown as ElectronIpcMain,
    mockIpcRenderer: mockIpcRendererModule as unknown as Electron.IpcRenderer,
    mockWindow,
    sentEvents,
    rootDir,
    dispose,
