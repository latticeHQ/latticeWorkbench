...
describe("resolveSSHConfig", () => {
  test("applies Host + Match host proxy rules", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-ssh-config-"));
    ...
    try {
      ...
      const config = [
        ...
        "  ProxyCommand /usr/local/bin/lattice --stdio %h",
        "",
      ].join("\n");
      ...
      expect(resolved.user).toBe("lattice-user");
      expect(resolved.hostName).toBe("pog2.lattice");
      expect(resolved.proxyCommand).toBe("/usr/local/bin/lattice --stdio %h");
    } finally {
