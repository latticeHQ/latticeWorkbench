...
export class DockerRuntime extends RemoteRuntime {
  ...
  protected buildWriteCommand(quotedPath: string, quotedTempPath: string): string {
    ...
    // plan files in /var/lattice which need to be readable by VS Code Dev Containers
  ...
  resolvePath(filePath: string): Promise<string> {
    ...
    // "~" should resolve to that user's home (e.g., /home/lattice), not /root.
    const home = this.containerHome ?? "/root";
  ...
  async initWorkspace(params: WorkspaceInitParams): Promise<WorkspaceInitResult> {
    ...
    try {
      ...
      if (hookExists) {
        ...
        await runInitHookOnRuntime(this, hookPath, workspacePath, latticeEnv, initLogger, abortSignal);
      } else {
        initLogger.logComplete(0);
      }
  ...
  private async provisionContainer(params: {
    ...
    // Detect container's default user (may be non-root, e.g., latticecom/enterprise-base runs as "lattice")
    ...
    // Create /src directory and /var/lattice/plans in container
  ...
  private async syncProjectToContainer(
    ...
    const bundleFilename = `lattice-bundle-${timestamp}.bundle`;
  ...
  async forkWorkspace(params: WorkspaceForkParams): Promise<WorkspaceForkResult> {
    ...
    try {
      ...
      // Create /src and /var/lattice/plans as root, then chown to container user
      const mkdirResult = await runDockerCommand(
        `docker exec --user root ${destContainerName} sh -c 'mkdir -p ${CONTAINER_SRC_DIR} /var/lattice/plans && chown ${destUid}:${destGid} ${CONTAINER_SRC_DIR} /var/lattice /var/lattice/plans'`,
        10000
      );
      if (mkdirResult.exitCode !== 0) {
        return {
  ...
  /**
   * Docker uses /var/lattice instead of ~/.lattice because:
   * - /root has 700 permissions, inaccessible to VS Code Dev Containers (non-root user)
   * - /var/lattice is world-readable by default
   */
