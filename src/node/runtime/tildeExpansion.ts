...
export function expandTilde(filePath: string): string {
  // Special-case lattice's own default src dir path so it respects LATTICE_ROOT + NODE_ENV.
  //
  // DEFAULT_RUNTIME_CONFIG uses "~/.lattice/src"; if we expand "~" to the OS home directory,
  // we lose test isolation when LATTICE_ROOT is set.
  const latticePrefixes = ["~/.lattice", "~\\.lattice", "~/.clattice", "~\\.clattice"] as const;
  for (const prefix of latticePrefixes) {
    ...
    return normalizedSuffix ? path.join(latticeHome, normalizedSuffix) : latticeHome;
  }
