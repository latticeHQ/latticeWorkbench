...
describe("backgroundCommands", () => {
  ...
  describe("buildWrapperScript", () => {
    it("builds script with trap, cd, and user script joined by &&", () => {
      ...
      expect(result).toBe(
        `__LATTICE_EXIT_CODE_PATH='/tmp/exit_code' && trap 'echo $? > "$__LATTICE_EXIT_CODE_PATH"' EXIT && cd '/home/user/project' && echo hello`
      );
    });
  ...
  describe("buildTerminateCommand", () => {
    ...
    it("uses custom quotePath function for SSH tilde expansion", () => {
      ...
      const result = buildTerminateCommand(1234, "~/lattice/exit_code", expandTilde);

      expect(result).toContain('"$HOME/lattice/exit_code"');
    });
  });
