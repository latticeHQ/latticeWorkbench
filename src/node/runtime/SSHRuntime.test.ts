 ...
 *
 * Integration tests for minion operations (renameMinion, deleteMinion, forkMinion,
...
describe("SSHRuntime constructor", () => {
  it("should accept tilde in srcBaseDir", () => {
    ...
    expect(() => {
      const config = { host: "example.com", srcBaseDir: "~/lattice" };
  ...
  it("should accept absolute paths in srcBaseDir", () => {
    expect(() => {
      const config = { host: "example.com", srcBaseDir: "/home/user/lattice" };
      new SSHRuntime(config, createSSHTransport(config, false));
    }).not.toThrow();
  });
});
...
describe("computeBaseRepoPath", () => {
  it("computes the correct bare repo path", () => {
    ...
    // <srcBaseDir>/<projectName>/.lattice-base.git
    const result = computeBaseRepoPath("~/lattice", "/Users/me/code/my-project");
    expect(result).toBe("~/lattice/my-project/.lattice-base.git");
  });

  it("handles absolute srcBaseDir", () => {
    const result = computeBaseRepoPath("/home/user/src", "/code/repo");
    expect(result).toBe("/home/user/src/repo/.lattice-base.git");
  });
});
