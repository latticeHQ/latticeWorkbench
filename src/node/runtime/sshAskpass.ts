...
# lattice-askpass — SSH_ASKPASS helper for Lattice
...
req_id="$$.$(date +%s%N)"
prompt_file="$LATTICE_ASKPASS_DIR/prompt.$req_id.txt"
response_file="$LATTICE_ASKPASS_DIR/response.$req_id.txt"
...
async function ensureAskpassScript(): Promise<string> {
  ...
  const dir = path.join(os.homedir(), ".lattice", "bin");
  await fs.promises.mkdir(dir, { recursive: true });
  askpassPath = path.join(dir, "lattice-askpass");
...
export async function createAskpassSession(
  ...
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "lattice-askpass-"));
  ...
  return {
    env: {
      ...
      DISPLAY: process.env.DISPLAY ?? "lattice",
      LATTICE_ASKPASS_DIR: dir,
    },
    cleanup() {
      closed = true;
      watcher.close();
