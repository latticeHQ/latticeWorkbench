...
export interface SSHRuntimeConfig extends SSHConnectionConfig {
  ...
  srcBaseDir: string;
  /** Directory on remote for background process output (default: /tmp/lattice-bashes) */
  bgOutputDir?: string;
}
...
export interface AcquireConnectionOptions {
   ...
   *
   * Useful for user-facing progress logs (e.g. minion init).
   */
...
export class SSHConnectionPool {
  ...
  private async probeConnection(
    ...
    // passphrase-protected keys must be agent-unlocked before Lattice can use them.
    const askpass =
...
export function getControlPath(config: SSHConnectionConfig): string {
  ...
  return path.join(os.tmpdir(), `lattice-ssh-${hash}`);
}
...
function makeConnectionKey(config: SSHConnectionConfig): string {
  ...
  // to be shared across minions on the same host.
  const parts = [
