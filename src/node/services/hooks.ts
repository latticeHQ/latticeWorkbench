 ...
 *
 * Hook Location:
 *   1. .lattice/tool_hook (project-level, committed)
 *   2. ~/.lattice/tool_hook (user-level, personal)
 *
 * Protocol:
 *   1. Hook receives LATTICE_TOOL, LATTICE_TOOL_INPUT, LATTICE_EXEC, etc. as env vars
 *   2. Hook runs pre-logic
 *   3. Hook prints $LATTICE_EXEC (the unique marker) to signal readiness
 *   4. Lattice executes the tool, sends result JSON to hook's stdin
...
export interface HookResult {
  ...
  success: boolean;
  /** Stdout output from hook before the __LATTICE_EXEC__ marker */
  stdoutBeforeExec: string;
  /** Stdout output from hook (after __LATTICE_EXEC__ marker) */
  stdout: string;
  /** Stderr output from hook */
  stderr: string;
  /** Hook process exit code */
  exitCode: number;
  /** Whether the tool was executed (hook printed __LATTICE_EXEC__) */
...
export interface HookTimingOptions {
  ...
  /** Maximum time allowed for hook pre-logic (until __LATTICE_EXEC__). Defaults to 10 seconds. */
 ...
 * @param executeTool Callback to execute the actual tool (called when hook signals __LATTICE_EXEC__)
...
export async function runWithHook<T>(
  ...
  if (context.toolInput.length > TOOL_INPUT_ENV_LIMIT) {
    ...
    try {
      ...
      toolInputPath = joinPathLike(
        tempDir,
        `lattice-tool-input-${Date.now()}-${crypto.randomUUID()}.json`
      );
      await writeFileString(runtime, toolInputPath, context.toolInput);
      toolInputEnv = "__LATTICE_TOOL_INPUT_FILE__";
    } catch (err) {
  ...
  // Read stdout, watching for __LATTICE_EXEC__ marker
  ...
  try {
    while (true) {
      ...
      // continue draining stdout (hooks may log after __LATTICE_EXEC__).
      toolPromise = (async () => {
        try {
          try {
  ...
  // If hook exited before __LATTICE_EXEC__, close stdin
  if (!toolExecuted) {
  ...
  if (timeoutPhase === "pre") {
    stderrOutput += `\nHook timed out before $LATTICE_EXEC marker (${preHookTimeoutMs}ms)`;
