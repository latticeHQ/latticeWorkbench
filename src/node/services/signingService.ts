...
interface AgentKeyCandidate {
  publicKeyOpenSSH: string;
  fingerprint: string;
  latticeKeyType: LatticeMdKeyType;
}
...
export class SigningService {
  ...
  private tryLoadDiskKeyPair(keyPaths: string[]): KeyPair | null {
    for (const keyPath of keyPaths) {
      ...
      try {
        ...
        // Extract raw private key bytes for use with lattice-md-client signing
    ...
    if (!sshAuthSock) {
      if (hasOverride) {
        return {
          selection: null,
          error:
            "LATTICE_SIGNING_PUBLIC_KEY/LATTICE_SIGNING_KEY_FINGERPRINT is set but SSH_AUTH_SOCK is not set",
        };
      }
    ...
    if (hasOverride) {
      ...
      if (desiredPublicKeyRaw) {
        ...
        if (!normalized) {
          return {
            selection: null,
            error:
              "LATTICE_SIGNING_PUBLIC_KEY must be an OpenSSH public key string (e.g. 'ssh-ed25519 AAAA...')",
          };
        }
        ...
        try {
          ...
          return {
            selection: null,
            error: "LATTICE_SIGNING_PUBLIC_KEY is not a supported SSH public key",
          };
        }
      ...
      if (filtered.length === 0) {
        return {
          selection: null,
          error:
            "No SSH agent key matched LATTICE_SIGNING_PUBLIC_KEY/LATTICE_SIGNING_KEY_FINGERPRINT (check your SSH agent)",
        };
      }
  ...
  private async doLoadSigningKey(): Promise<SigningKeySelection | null> {
    ...
    if (this.hasEncryptedKey) {
      ...
      this.keyLoadError =
        "No signing key found. Create ~/.lattice/message_signing_key or ensure your SSH agent is running (SSH_AUTH_SOCK).";
    }
  ...
  /**
   * Extract raw private key bytes from sshpk key for use with lattice-md-client.
   */
