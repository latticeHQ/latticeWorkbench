...
function wrapFetchWithAnthropicCacheControl(
  ...
  const cachingFetch = async (
    ...
    try {
      ...
      // - AI SDK internal format: json.prompt
      ...
      if (messages && messages.length >= 1) {
        ...
        // For AI SDK internal format: add providerOptions.anthropic.cacheControl at message level
...
export function buildAppAttributionHeaders(
  ...
  if (!existingLowercaseKeys.has("http-referer")) {
    headers["HTTP-Referer"] = LATTICE_APP_ATTRIBUTION_URL;
  }

  if (!existingLowercaseKeys.has("x-title")) {
    headers["X-Title"] = LATTICE_APP_ATTRIBUTION_TITLE;
  }
...
const LATTICE_MODEL_COSTS_INCLUDED = Symbol("lattice:modelCostsIncluded");

type LanguageModelWithLatticeCostsIncluded = LanguageModel & {
  [LATTICE_MODEL_COSTS_INCLUDED]?: true;
};

function markModelCostsIncluded(model: LanguageModel): void {
  (model as LanguageModelWithLatticeCostsIncluded)[LATTICE_MODEL_COSTS_INCLUDED] = true;
}

export function modelCostsIncluded(model: LanguageModel): boolean {
  return (model as LanguageModelWithLatticeCostsIncluded)[LATTICE_MODEL_COSTS_INCLUDED] === true;
}
...
export class ProviderModelFactory {
  ...
  async createModel(
    ...
    try {
      ...
      // We still allow request-level values when config is unset for backward
      ...
      if (isAnthropicRoutedModel && configAnthropicCacheTtl && latticeProviderOptions) {
      ...
      const effectiveAnthropicCacheTtl =
        latticeProviderOptions?.anthropic?.cacheTtl ?? configAnthropicCacheTtl;
      ...
      if (providerName === "anthropic") {
        // Check for stored Anthropic OAuth (Claude Pro/Max subscription auth).
        // The OAuth access token doubles as the Anthropic API key.
        ...
        // OAuth routing: use OAuth when connected and no API key is configured,
        // or when both are present (user explicitly chose to connect OAuth).
        const shouldRouteViaOauth = (() => {
          if (!storedAnthropicOauth) return false;
          if (!creds.isConfigured) return true;
          // When both API key and OAuth are available, prefer OAuth
          // (user connected their Pro/Max plan intentionally).
          return true;
        })();

        if (!shouldRouteViaOauth && !creds.isConfigured) {
          return Err({ type: "api_key_not_found", provider: providerName });
        }

        // Build config with resolved credentials.
        // When using OAuth, pass a placeholder so the SDK doesn't read env vars;
        // the real token is injected per-request via the fetch wrapper.
        const effectiveApiKey = shouldRouteViaOauth
        ...
        // header logic across different provider handlers.
        ...
        // When routing via OAuth, wrap fetch to swap the API key with a fresh
        // OAuth access token on every request. The access token is used as the
        // x-api-key value (Anthropic's API accepts OAuth tokens this way).
        const anthropicOauthService = this.anthropicOauthService;
        // When routing through Anthropic OAuth, wrap fetch to inject a fresh
        // OAuth token as the x-api-key header on every request.
        const finalFetch: typeof fetch = shouldRouteViaOauth
          ? (Object.assign(
              async (
                input: Parameters<typeof fetch>[0],
                init?: Parameters<typeof fetch>[1]
                ...
                const headers = new Headers(init?.headers);
                // Replace the placeholder API key with the real OAuth token.
                headers.set("x-api-key", authResult.data.access);
                return fetchWithCacheControl(input, { ...init, headers });
              },
              // Preserve preconnect from the base fetch if present.
      ...
      if (providerName === "openai") {
        ...
        if (configServiceTier && latticeProviderOptions) {
          latticeProviderOptions.openai = {
            ...latticeProviderOptions.openai,
      ...
      if (providerName === "xai") {
        ...
        if (searchParameters && latticeProviderOptions) {
          const existingXaiOverrides = latticeProviderOptions.xai ?? {};
          latticeProviderOptions.xai = {
      ...
      if (providerName === "bedrock") {
        ...
        // Useful for SSO profiles when Lattice isn't launched with AWS_PROFILE set.
        const profile =
      ...
      if (providerName === "github-copilot") {
        ...
        const copilotFetchFn = async (
          ...
          // Copilot API gates model access (especially non-GPT models like Claude)
          // behind the integration-id header. Simulate a VS Code chat client.
        ...
        // Copilot API uses dots in Claude version numbers (e.g. claude-sonnet-4.6)
        // while Lattice internal model IDs use hyphens (e.g. claude-sonnet-4-6).
        // Normalize to dot-separated format so both conventions resolve correctly.
        const copilotModelId = modelId.replace(
          /^(claude-(?:sonnet|opus|haiku)-\d+)-(\d+)$/,
          "$1.$2"
        );
        return Ok(provider.chatModel(copilotModelId));
      }
   ...
   *
   * Combines the xAI thinking-level variant swap and model creation into a
   * single call. Previously this logic was inlined in `AIService.streamMessage()`.
   *
  ...
  async resolveAndCreateModel(
    ...
    Result<
      {
        model: LanguageModel;
        /** Model string after variant resolution. */
    ...
    >
  > {
    const canonicalModelString = modelString;
    ...
    const modelResult = await this.createModel(effectiveModelString, latticeProviderOptions);
