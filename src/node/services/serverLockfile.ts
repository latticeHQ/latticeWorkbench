...
export const ServerLockDataSchema = z.object({
  pid: z.number(),
  /** Base URL for HTTP API (e.g., "http://localhost:3000" or "https://my.box.com/lattice") */
  baseUrl: z.url(),
  token: z.string(),
  startedAt: z.string(),
...
/**
 * Manages the server lockfile at ~/.lattice/server.lock
 *
 * The lockfile enables CLI tools to discover a running lattice server
 * (either Electron app or standalone lattice server) and connect to it.
 */
export class ServerLockfile {
  private readonly lockPath: string;

  constructor(latticeHome: string) {
    this.lockPath = path.join(latticeHome, "server.lock");
  }
