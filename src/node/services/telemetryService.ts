 ...
 * (NODE_ENV=test, CI, LATTICE_E2E=1, JEST_WORKER_ID, etc.).
 * Users can manually disable telemetry by setting LATTICE_DISABLE_TELEMETRY=1.
 *
...
async function getElectronIsPackaged(isElectron: boolean): Promise<boolean | null> {
  ...
  try {
    // eslint-disable-next-line no-restricted-syntax -- Electron is unavailable in `lattice server`; avoid top-level import
...
export class TelemetryService {
  ...
  constructor(latticeHome?: string) {
    this.latticeHome = latticeHome ?? getLatticeHome();
  }
   ...
   * Persisted in ~/.lattice/telemetry_id for cross-session identity.
   */
  private async loadOrCreateDistinctId(): Promise<string> {
    const idPath = path.join(this.latticeHome, TELEMETRY_ID_FILE);
    ...
    try {
      ...
      await fs.mkdir(this.latticeHome, { recursive: true });
