...
describe("ProjectService", () => {
  ...
  describe("clone", () => {
    it("clones a local repository and registers it as a project", async () => {
      ...
      expect(result.data.projectConfig).toEqual({ minions: [] });
...
`,
      ...
      try {
        ...
        expect(path.basename(loggedArgs[4])).toMatch(/^repo\.lattice-clone-[a-f0-9]{12}$/);
      } finally {
...
`,
      ...
      expect(
        cloneEntries.filter((entry) => entry.startsWith("source-repo-stop.lattice-clone-"))
      ).toEqual([]);
...
`,
      ...
      try {
        ...
        const tempCloneEntries = cloneParentEntries.filter((entry) =>
          entry.startsWith("source-repo-race.lattice-clone-")
        );
...
`,
      ...
      try {
        ...
        expect(env.SSH_ASKPASS).toContain("lattice-askpass");
