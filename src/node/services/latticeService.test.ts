  ...
  describe("createWorkspace", () => {
    ...
    function mockPrefetchCalls(options?: { presetParamNames?: string[] }) {
      ...
      execAsyncSpy?.mockImplementation((cmd: string) => {
        ...
        if (cmd === "lattice templates list --output=json") {
          return createMockExecResult(
            Promise.resolve({
              stdout: JSON.stringify([
        ...
        if (cmd.startsWith("lattice templates presets list")) {
        ...
        if (cmd.startsWith("lattice tokens delete")) {
    ...
    it("streams stdout/stderr lines and passes expected args", async () => {
      ...
      expect(lines[0]).toBe("$ lattice create my-workspace -t my-template --yes");
