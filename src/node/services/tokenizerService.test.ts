...
describe("TokenizerService", () => {
  ...
  describe("calculateStats", () => {
    test("delegates to underlying function and persists token stats cache", async () => {
      const messages = [
        createLatticeMessage("msg1", "user", "Hello", { historySequence: 1 }),
        createLatticeMessage("msg2", "assistant", "World", { historySequence: 2 }),
      ];
    ...
    test("skips persisting stale token stats cache when calculations overlap", async () => {
      ...
      const messagesV2 = [
        ...messagesV1,
        createLatticeMessage("msg3", "assistant", "!!!", { historySequence: 3 }),
      ];
