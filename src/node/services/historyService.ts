...
export class HistoryService {
   ...
   */
  private async readChatHistory(workspaceId: string): Promise<LatticeMessage[]> {
    try {
  ...
  async getHistory(workspaceId: string): Promise<Result<LatticeMessage[]>> {
    try {
  ...
  async appendToHistory(workspaceId: string, message: LatticeMessage): Promise<Result<void>> {
   ...
   */
  async updateHistory(workspaceId: string, message: LatticeMessage): Promise<Result<void>> {
    return this.fileLocks.withLock(workspaceId, async () => {
      try {
        const historyPath = this.getChatHistoryPath(workspaceId);
