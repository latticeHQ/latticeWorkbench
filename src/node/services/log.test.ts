...
describe("log file sink state machine", () => {
  let tempLatticeRoot: string;
  let originalLatticeRoot: string | undefined;

  beforeAll(async () => {
    originalLatticeRoot = process.env.LATTICE_ROOT;
    tempLatticeRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "lattice-log-test-"));
    process.env.LATTICE_ROOT = tempLatticeRoot;
  });
  ...
  afterAll(async () => {
    ...
    await fsPromises.rm(tempLatticeRoot, { recursive: true, force: true });
  });
