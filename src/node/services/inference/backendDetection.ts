 ...
 */
export function detectDistributedBackend(_modelPath: string, strategy?: string): string {
...
export function findWorkerScript(appResourcesPath?: string): string {
  ...
  candidates.push(path.join(projectRoot, "vendor", "latticeInference", "python", "worker.py"));
  // Legacy path (resources/inference/python/) — backwards compat
  candidates.push(path.join(projectRoot, "resources", "inference", "python", "worker.py"));
  // Built dist path (after build-static)
  candidates.push(path.join(projectRoot, "dist", "inference", "python", "worker.py"));

  // Development: relative to cwd
  candidates.push(path.join(process.cwd(), "vendor", "latticeInference", "python", "worker.py"));

  // Standard install location
  candidates.push(path.join(os.homedir(), ".lattice", "python", "worker.py"));
