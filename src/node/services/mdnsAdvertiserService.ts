...
// NOTE: Avoid "lattice" here: it's an IANA-registered service name ("Multiplexing Protocol"),
// and some discovery tools will display/handle it specially.
export const LATTICE_MDNS_SERVICE_TYPE = "lattice-api";
...
export interface BuildLatticeMdnsServiceOptions {
  bindHost: string;
  port: number;
  instanceName: string;
  version: string;
...
export function buildLatticeMdnsServiceOptions(options: BuildLatticeMdnsServiceOptions): ServiceOptions {
  ...
  //   lattice-host.home._lattice-api._tcp.local.
  ...
  const serviceOptions: ServiceOptions = {
    name: instanceName,
    type: LATTICE_MDNS_SERVICE_TYPE,
  ...
  // If lattice is bound to IPv4 wildcard only, don't advertise IPv6 addresses.
  if (bindHost === "0.0.0.0") {
    ...
    // If lattice is bound to a specific IP, only advertise that address (otherwise clients may
