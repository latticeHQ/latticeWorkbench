...
import { LATTICE_HELP_CHAT_AGENT_ID, LATTICE_HELP_CHAT_WORKSPACE_ID } from "@/common/constants/latticeChat";
...
export class AIService extends EventEmitter {
    ...
    try {
      ...
      if (providerName === "anthropic") {
        ...
        const headers = buildAnthropicHeaders(
          ...
          latticeProviderOptions?.anthropic?.use1MContext
        );
  ...
  async streamMessage(
    ...
    try {
      ...
      const isLatticeGatewayModel = providerName === "lattice-gateway";
      if (normalizedProviderName === "xai" && normalizedModelId === "grok-4-1-fast") {
        ...
        effectiveModelString = isLatticeGatewayModel ? `lattice-gateway:xai/${variant}` : `xai:${variant}`;
      ...
      const modelResult = await this.createModel(effectiveModelString, effectiveLatticeProviderOptions);
      ...
      // Normalize provider for provider-specific handling (Lattice Gateway models should behave
