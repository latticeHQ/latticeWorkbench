 ...
 * - tool_post: runs after tool with result in LATTICE_TOOL_RESULT/LATTICE_TOOL_RESULT_PATH
 *
...
async function executeWithLegacyHook<TResult>(
  ...
  // Hook blocked tool execution (exited before $LATTICE_EXEC)
  if (!hook.toolExecuted) {
    ...
    const errorResult: { error: string } = {
      error: blockOutput || "Tool blocked by hook (exited before $LATTICE_EXEC)",
    };
    return errorResult as TResult;
  }
