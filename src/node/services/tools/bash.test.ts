...
describe("latticeEnv environment variables", () => {
  it("should inject LATTICE_ environment variables when latticeEnv is provided", async () => {
    using tempDir = new TestTempDir("test-lattice-env");
    const config = createTestToolConfig(process.cwd());
    config.runtimeTempDir = tempDir.path;
    config.latticeEnv = {
    ...
    const args: BashToolArgs = {
      script: 'echo "PROJECT:$LATTICE_PROJECT_PATH RUNTIME:$LATTICE_RUNTIME WORKSPACE:$LATTICE_WORKSPACE_NAME"',
      timeout_secs: 5,
      display_name: "test",
    };
  ...
  it("should allow secrets to override latticeEnv", async () => {
    using tempDir = new TestTempDir("test-lattice-env-override");
    const config = createTestToolConfig(process.cwd());
    config.runtimeTempDir = tempDir.path;
    config.latticeEnv = {
      LATTICE_PROJECT_PATH: "/lattice/path",
      CUSTOM_VAR: "from-lattice",
    };
    config.secrets = {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      // LATTICE_PROJECT_PATH from latticeEnv should be present
      expect(result.output).toContain("LATTICE:/lattice/path");
      // Secrets should override latticeEnv when there's a conflict
...
describe("bash tool - tool_env", () => {
  it("should source .lattice/tool_env before running script", async () => {
    using tempDir = new TestTempDir("test-bash-tool-env");
    const latticeDir = `${tempDir.path}/.lattice`;
    fs.mkdirSync(latticeDir, { recursive: true });
    fs.writeFileSync(`${latticeDir}/tool_env`, "export LATTICE_TEST_VAR=from_tool_env");
  ...
  it("should fail with clear error if tool_env sourcing fails", async () => {
    ...
    fs.writeFileSync(`${latticeDir}/tool_env`, "return 1");
...
describe("bash tool - background execution", () => {
  ...
  it("should accept timeout with background mode for auto-termination", async () => {
    const manager = new BackgroundProcessManager("/tmp/lattice-test-bg");
  ...
  it("should inject latticeEnv environment variables in background mode", async () => {
    const manager = new BackgroundProcessManager("/tmp/lattice-test-bg");

    const tempDir = new TestTempDir("test-bash-bg-lattice-env");
    const config = createTestToolConfig(tempDir.path);
    config.backgroundProcessManager = manager;
    config.latticeEnv = {
      LATTICE_MODEL_STRING: "openai:gpt-5.2",
      LATTICE_THINKING_LEVEL: "medium",
    };

    const tool = createBashTool(config);
    const args: BashToolArgs = {
      script: 'echo "MODEL:$LATTICE_MODEL_STRING THINKING:$LATTICE_THINKING_LEVEL"',
      timeout_secs: 5,
      run_in_background: true,
      display_name: "test-lattice-env-bg",
    };
