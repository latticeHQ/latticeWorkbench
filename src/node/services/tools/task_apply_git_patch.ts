...
import { parseToolResult, requireMinionId } from "./toolUtils";
...
export const createTaskApplyGitPatchTool: ToolFactory = (config: ToolConfiguration) => {
  ...
  const MAX_PARENT_MINION_DEPTH = 32;

  function inferLatticeRootFromMinionSessionDir(minionSessionDir: string): string | undefined {
    assert(
      minionSessionDir.length > 0,
      "inferLatticeRootFromMinionSessionDir: minionSessionDir must be non-empty"
    );

    const sessionsDir = path.dirname(minionSessionDir);
  ...
  async function findGitPatchArtifactInMinionOrAncestors(params: {
    minionId: string;
    minionSessionDir: string;
    childTaskId: string;
  }): Promise<{
    artifact: NonNullable<Awaited<ReturnType<typeof readSidekickGitPatchArtifact>>>;
    artifactMinionId: string;
    artifactSessionDir: string;
    note?: string;
  } | null> {
    assert(
      params.minionId.length > 0,
      "findGitPatchArtifactInMinionOrAncestors: minionId must be non-empty"
    );
    assert(
      params.minionSessionDir.length > 0,
      "findGitPatchArtifactInMinionOrAncestors: minionSessionDir must be non-empty"
    );
    assert(
      params.childTaskId.length > 0,
      "findGitPatchArtifactInMinionOrAncestors: childTaskId must be non-empty"
    );

    const direct = await readSidekickGitPatchArtifact(
    ...
    if (direct) {
      return {
        artifact: direct,
        artifactMinionId: params.minionId,
        artifactSessionDir: params.minionSessionDir,
      };
    }

    const latticeRootDir = inferLatticeRootFromMinionSessionDir(params.minionSessionDir);
    if (!latticeRootDir) {
      log.debug(
        "task_apply_git_patch: minionSessionDir not under sessions/; skipping ancestor lookup",
        {
    ...
    const configService = new Config(latticeRootDir);

    let cfg: ReturnType<Config["loadConfigOrDefault"]>;
    try {
      cfg = configService.loadConfigOrDefault();
    } catch (error) {
      log.debug("task_apply_git_patch: failed to load lattice config for ancestor lookup", {
        minionId: params.minionId,
        latticeRootDir,
        error,
      });
    ...
    visited.add(params.minionId);

    let current = params.minionId;
    for (let i = 0; i < MAX_PARENT_MINION_DEPTH; i++) {
      ...
      if (visited.has(parent)) {
        log.warn("task_apply_git_patch: possible parentMinionId cycle during ancestor lookup", {
          minionId: params.minionId,
          childTaskId: params.childTaskId,
          current,
          parent,
      ...
      const artifact = await readSidekickGitPatchArtifact(parentSessionDir, params.childTaskId);
      if (artifact) {
        return {
          artifact,
          artifactMinionId: parent,
          artifactSessionDir: parentSessionDir,
          note: `Patch artifact loaded from ancestor minion ${parent}.`,
        };
      }
    ...
    log.warn("task_apply_git_patch: exceeded parentMinionId depth during ancestor lookup", {
      minionId: params.minionId,
      childTaskId: params.childTaskId,
    });
  ...
  return tool({
    ...
    execute: async (args, { abortSignal }): Promise<unknown> => {
      const minionId = requireMinionId(config, "task_apply_git_patch");
      ...
      const minionSessionDir = config.minionSessionDir;
      assert(minionSessionDir, "task_apply_git_patch requires minionSessionDir");
      ...
      const artifactLookup = await findGitPatchArtifactInMinionOrAncestors({
        minionId,
        minionSessionDir,
        childTaskId: taskId,
      });
      ...
      const artifactMinionId = artifactLookup.artifactMinionId;
      const artifactSessionDir = artifactLookup.artifactSessionDir;
      const isReplay = artifactMinionId !== minionId;
      const artifactLookupNote = artifactLookup.note;

      if (artifact.parentMinionId !== artifactMinionId) {
        return parseToolResult(
          TaskApplyGitPatchToolResultSchema,
          {
            success: false as const,
            taskId,
            dryRun,
            error: "This patch artifact belongs to a different parent minion.",
            note: mergeNotes(
              artifactLookupNote,
              `Expected parent minion ${artifactMinionId} but artifact metadata says ${artifact.parentMinionId}.`
            ),
          },
          "task_apply_git_patch"
        );
      ...
      const expectedPatchPath = getSidekickGitPatchMboxPath(artifactSessionDir, taskId);
      ...
      const remotePatchPath = path.posix.join(
        ...
        `lattice-task-${taskId}-series.mbox`
      );
      ...
      if (dryRun) {
        ...
        const dryRunWorktreePath = path.posix.join(
          config.runtimeTempDir,
          `lattice-git-am-dry-run-${taskId}-${dryRunId}`
        );
      ...
      if (!dryRun && !isReplay) {
        await markSidekickGitPatchArtifactApplied({
          minionId: artifactMinionId,
          minionSessionDir: artifactSessionDir,
          childTaskId: taskId,
