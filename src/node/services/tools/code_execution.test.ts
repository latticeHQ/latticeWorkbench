...
describe("createCodeExecutionTool", () => {
  ...
  describe("tool bridge integration", () => {
    it("calls bridged tools and returns results", async () => {
      ...
      const result = (await tool.execute!(
        { code: 'return lattice.file_read({ filePath: "test.txt" })' },
        mockToolCallOptions
      )) as PTCExecutionResult;
    ...
    it("records tool calls in result", async () => {
      ...
      const result = (await tool.execute!(
        { code: 'lattice.file_read({ filePath: "a.txt" }); return "done"' },
        mockToolCallOptions
      )) as PTCExecutionResult;
    ...
    it("validates tool arguments against schema at runtime", async () => {
      ...
      const result = (await tool.execute!(
        { code: "return lattice.file_read({ wrongField: 123 })" },
        mockToolCallOptions
      )) as PTCExecutionResult;
    ...
    it("handles tool execution errors gracefully", async () => {
      ...
      const result = (await tool.execute!(
        { code: "return lattice.failing_tool({})" },
        mockToolCallOptions
      )) as PTCExecutionResult;
      ...
      const result = (await tool.execute!(
        {
          code: `
            lattice.counter({});
            lattice.counter({}); // This one fails
            lattice.counter({}); // Never reached
            return "done";
          `,
        },
        mockToolCallOptions
