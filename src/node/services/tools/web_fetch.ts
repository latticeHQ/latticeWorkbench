...
import { parseLatticeMdUrl, downloadFromLatticeMd, LATTICE_MD_BASE_URL, LATTICE_MD_HOST } from "@/common/lib/latticeMd";
  ...
  return tool({
    ...
    execute: async ({ url }, { abortSignal }): Promise<WebFetchToolResult> => {
      try {
        ...
        // Parse HTML with JSDOM (runs locally in Lattice, not over SSH)
        const dom = new JSDOM(body, { url });
