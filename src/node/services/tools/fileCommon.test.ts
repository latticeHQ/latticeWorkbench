...
describe("fileCommon", () => {
  ...
  describe("validateNoRedundantPrefix", () => {
    ...
    it("should work with SSH runtime", () => {
      const sshRuntime = createRuntime({
        type: "ssh",
        host: "user@localhost",
        srcBaseDir: "/home/user/lattice",
        identityFile: "/tmp/fake-key",
      });
      const sshCwd = "/home/user/lattice/project/branch";

      // Should auto-correct absolute paths with redundant prefix on SSH too
      const result = validateNoRedundantPrefix(
        "/home/user/lattice/project/branch/src/file.ts",
        sshCwd,
        sshRuntime
      );
      expect(result).not.toBeNull();
  ...
  describe("validatePlanModeAccess", () => {
    const planFilePath = "~/.lattice/plans/plan.md";
    const resolvedPlanFilePath = "/home/user/.lattice/plans/plan.md";
