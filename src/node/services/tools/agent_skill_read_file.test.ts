...
describe("agent_skill_read_file (Chat with Lattice sandbox)", () => {
  it("allows reading built-in skill files", async () => {
    ...
    const raw: unknown = await Promise.resolve(
      tool.execute!(
        { name: "lattice-docs", filePath: "SKILL.md", offset: 1, limit: 25 },
        mockToolCallOptions
      )
    );
    ...
    if (result.success) {
      expect(result.content).toMatch(/name:\s*lattice-docs/i);
    }
  });

  it("rejects project/global skill file reads on disk", async () => {
    using tempDir = new TestTempDir("test-agent-skill-read-file-lattice-chat-reject");
