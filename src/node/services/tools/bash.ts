...
/**
 * Build script prelude that sources .lattice/tool_env if present.
...
function buildToolEnvPrelude(toolEnvPath: string | null): string {
  ...
  echo "lattice: failed to source ${toolEnvPath}" >&2
  exit 1
fi
`;
}
    ...
    execute: async (
      ...
      if (run_in_background) {
        ...
        const spawnResult = await config.backgroundProcessManager.spawn(
          ...
          {
            cwd: config.cwd,
            // Match foreground bash behavior: latticeEnv is present and secrets override it.
            env: { ...(config.latticeEnv ?? {}), ...(config.secrets ?? {}) },
...
${scriptWithEnv}`;
      const execStream = await config.runtime.exec(scriptWithClosedStdin, {
        cwd: config.cwd,
        env: { ...config.latticeEnv, ...config.secrets, ...NON_INTERACTIVE_ENV_VARS },
