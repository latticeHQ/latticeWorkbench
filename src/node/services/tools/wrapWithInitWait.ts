...
/**
 * Wraps a tool to wait for minion initialization before execution.
 *
 ...
 * @param minionId Minion ID for init state tracking
...
export function wrapWithInitWait<TParameters, TResult>(
  ...
  return {
    ...
    execute: async (args: TParameters, options) => {
      ...
      // Wait for minion initialization to complete (no-op if not needed)
      ...
      await initStateManager.waitForInit(minionId, abortSignal);
