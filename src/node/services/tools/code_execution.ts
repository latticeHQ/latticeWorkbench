 ...
 * Lattice tools via the `lattice.*` namespace. Enables multi-tool workflows in a single
...
import { getCachedLatticeTypes, clearTypeCache } from "@/node/services/ptc/typeGenerator";
...
export async function preGenerateLatticeTypes(tools: Record<string, Tool>): Promise<void> {
  ...
  await getCachedLatticeTypes(toolBridge.getBridgeableTools());
}
...
export async function createCodeExecutionTool(
  ...
  // Generate lattice types for type validation and documentation (cached by tool set hash)
  const latticeTypes = await getCachedLatticeTypes(bridgeableTools);
...
**Usage notes:**
- \`lattice.*\` functions are synchronous—do not use \`await\`
...
**Security:** The sandbox has no access to \`require\`, \`import\`, \`process\`, \`fetch\`, or filesystem outside of \`lattice.*\` tools.`,
