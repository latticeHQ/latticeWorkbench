...
import { compareVersions } from "@/node/services/latticeService";
...
async function loadGovernorPolicyText(input: {
  ...
  try {
    const response = await fetch(policyUrl, {
      ...
      headers: {
        accept: "application/json",
        "Lattice-Governor-Session-Token": input.token,
      },
    });
...
export class PolicyService {
  ...
  getPolicyRuntimeId(runtimeConfig: RuntimeConfig | undefined): PolicyRuntimeId | null {
    if (!runtimeConfig) {
      // This matches the server default in minionService.create().
      return "worktree";
    }
    ...
    if (runtimeConfig.type === "ssh") {
      return runtimeConfig.lattice ? "ssh+lattice" : "ssh";
    }
  ...
  private getActivePolicySource(): ActivePolicySource {
    const filePath = process.env.LATTICE_POLICY_FILE?.trim();
    if (filePath) {
      return { kind: "env", value: filePath };
    }

    const config = this.config.loadConfigOrDefault();
    const governorOrigin = config.latticeGovernorUrl?.trim();
    const governorToken = config.latticeGovernorToken?.trim();
      ...
      if (parsed.minimum_client_version) {
        ...
        if (compareVersions(clientVersion, min) < 0) {
          this.updateState({
            ...
            status: {
              state: "blocked",
              reason: `Lattice ${clientVersion} is below required minimum_client_version ${min}`,
            },
            policy: null,
          });
          return Ok(undefined);
