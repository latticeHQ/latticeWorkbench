...
import type { SendMessageOptions, MinionChatMessage } from "@/common/orpc/types";
...
function createAiService(
  projectPath: string,
  aiEmitter: EventEmitter,
  metadataOverrides?: Partial<MinionMetadata>
): AIService {
  const minionMetadata: MinionMetadata = {
    id: "minion-switch",
    name: "minion-switch-name",
    projectName: "minion-switch-project",
    projectPath,
...
): Extract<MinionChatMessage, { type: "stream-error" }> | undefined {
...
describe("AgentSession switch_agent target validation", () => {
  ...
  test("inherits model/thinking from outgoing stream when target has no aiSettingsByAgent entry", async () => {
    ...
    const session = createSession(historyService, projectDir.path, projectDir.path, {
      // Legacy minion aiSettings should not override the active stream
