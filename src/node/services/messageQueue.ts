 ...
 * - First latticeMetadata (preserved - never overwritten by subsequent adds)
 * - Latest options (model, etc. - updated on each add)
 * - File parts (accumulated across all messages)
 *
 * IMPORTANT:
 * - Compaction requests must preserve their latticeMetadata even when follow-up messages are queued.
...
export class MessageQueue {
  ...
  private firstLatticeMetadata?: unknown;
  ...
  hasCompactionRequest(): boolean {
    return isCompactionMetadata(this.firstLatticeMetadata);
  }

  /**
   * Add a message to the queue.
   * Preserves latticeMetadata from first message, updates other options.
  ...
  private addInternal(
    ...
    const incomingIsCompaction = isCompactionMetadata(options?.latticeMetadata);
    const incomingIsAgentSkill = isAgentSkillMetadata(options?.latticeMetadata);
    const queueHasMessages = !this.isEmpty();
    const queueHasAgentSkill = isAgentSkillMetadata(this.firstLatticeMetadata);
    ...
    if (options) {
      ...
      // Preserve first latticeMetadata (see class docblock for rationale)
      if (options.latticeMetadata !== undefined && this.firstLatticeMetadata === undefined) {
        this.firstLatticeMetadata = options.latticeMetadata;
      }
  ...
  getDisplayText(): string {
    ...
    if (this.messages.length === 1 && isCompactionMetadata(this.firstLatticeMetadata)) {
      return this.firstLatticeMetadata.rawCommand;
    }

    // Only show rawCommand for a single agent-skill invocation.
    // (Batching agent-skill with other messages is disallowed.)
    if (this.messages.length <= 1 && isAgentSkillMetadata(this.firstLatticeMetadata)) {
      return this.firstLatticeMetadata.rawCommand;
    }
  ...
  getReviews(): ReviewNoteData[] | undefined {
    if (hasReviews(this.firstLatticeMetadata) && this.firstLatticeMetadata.reviews?.length) {
      return this.firstLatticeMetadata.reviews;
    }
    return undefined;
  }
  ...
  produceMessage(): {
    ...
    const latticeMetadata =
      this.firstLatticeMetadata !== undefined
        ? this.firstLatticeMetadata
        : (this.latestOptions?.latticeMetadata as unknown);
