...
describe("ServerService.startServer", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-server-test-"));
  ...
  test("cleans up server when lockfile acquisition fails", async () => {
    ...
    // Make latticeHome a *file* (not a directory) so lockfile.acquire() fails reliably,
    // even when tests run as root (chmod-based tests don't fail for root).
    const latticeHomeFile = path.join(tempDir, "latticeHome-not-a-dir");
    await fs.writeFile(latticeHomeFile, "not a directory");
...
test("supports non-CLI allow-http-origin opt-in via LATTICE_SERVER_ALLOW_HTTP_ORIGIN", async () => {
  const service = new ServerService();
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-server-env-test-"));
  const config = new Config(tempDir);
  const stubContext = { config } as unknown as ORPCContext;

  const previousAllowHttpOriginEnv = process.env.LATTICE_SERVER_ALLOW_HTTP_ORIGIN;
  process.env.LATTICE_SERVER_ALLOW_HTTP_ORIGIN = "1";
  ...
  try {
    ...
    if (previousAllowHttpOriginEnv === undefined) {
      delete process.env.LATTICE_SERVER_ALLOW_HTTP_ORIGIN;
    } else {
      process.env.LATTICE_SERVER_ALLOW_HTTP_ORIGIN = previousAllowHttpOriginEnv;
    }
