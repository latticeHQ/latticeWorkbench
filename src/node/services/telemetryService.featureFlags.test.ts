...
describe("TelemetryService feature flag properties", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-telemetry-test-"));
  });
  ...
  test("capture includes $feature/<flagKey> properties when set", () => {
    ...
    const savedEnv = {
      ...
      LATTICE_DISABLE_TELEMETRY: process.env.LATTICE_DISABLE_TELEMETRY,
      LATTICE_E2E: process.env.LATTICE_E2E,
    ...
    delete process.env.LATTICE_DISABLE_TELEMETRY;
    delete process.env.LATTICE_E2E;
