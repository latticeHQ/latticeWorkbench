...
describe("injectFileAtMentions", () => {
  it("expands @file mentions from earlier user messages even when the latest has none", async () => {
    const tmpDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "lattice-file-at-mentions-"));
    ...
    try {
      ...
      const messages = [
        createLatticeMessage("u1", "user", "Please check @src/foo.ts"),
        createLatticeMessage("a1", "assistant", "Sure."),
        createLatticeMessage("u2", "user", "Now do X (no mentions)."),
      ];
      ...
      expect(injectedText).toContain('<lattice-file path="src/foo.ts"');
  ...
  it("prioritizes the latest @file mention when the global cap is hit", async () => {
    ...
    try {
      ...
      const messages = Array.from({ length: 11 }, (_, idx) => {
        const i = idx + 1;
        return createLatticeMessage(`u${i}`, "user", `Please check @src/f${i}.ts`);
      });
      ...
      const injectedText = syntheticMessages
        ...
        .join("\n\n");
      expect(injectedText).toContain('<lattice-file path="src/f11.ts"');
    } finally {
      await fsPromises.rm(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  it("injects a synthetic user message with file contents before the prompt", async () => {
    ...
    try {
      ...
      const runtime = createRuntime({ type: "local" }, { projectPath: tmpDir });
      const messages = [createLatticeMessage("u1", "user", "Please check @src/foo.ts#L2-3")];
      ...
      const injectedText = result[0]?.parts.find((p) => p.type === "text")?.text ?? "";
      expect(injectedText).toContain('<lattice-file path="src/foo.ts" range="L2-L3"');
  ...
  it("ignores non-existent file mentions", async () => {
    ...
    try {
      const runtime = createRuntime({ type: "local" }, { projectPath: tmpDir });
      const messages = [createLatticeMessage("u1", "user", "Please check @src/missing.ts")];
  ...
  it("injects root files like @Makefile", async () => {
    ...
    try {
      ...
      const runtime = createRuntime({ type: "local" }, { projectPath: tmpDir });
      const messages = [createLatticeMessage("u1", "user", "Please check @Makefile")];
      ...
      const injectedText = result[0]?.parts.find((p) => p.type === "text")?.text ?? "";
      expect(injectedText).toContain('<lattice-file path="Makefile" range="L1-L2"');
      expect(injectedText).toContain("line1");
      expect(injectedText).toContain("line2");
    } finally {
      await fsPromises.rm(tmpDir, { recursive: true, force: true });
  ...
  it("ignores non-file @mentions with # fragments", async () => {
    ...
    try {
      const runtime = createRuntime({ type: "local" }, { projectPath: tmpDir });
      const messages = [createLatticeMessage("u1", "user", "Ping @alice#123")];
  ...
  it("skips tokens that already have persisted snapshots (fileAtMentionSnapshot metadata)", async () => {
    ...
    try {
      ...
      const snapshotMessage = createLatticeMessage(
        "snapshot-1",
        "user",
        '<lattice-file path="src/foo.ts" range="L1-L2">\n```ts\nold line1\nold line2\n```\n</lattice-file>',
        {
          timestamp: Date.now(),
          synthetic: true,
      ...
      const userMessage = createLatticeMessage("u1", "user", "Please check @src/foo.ts");
...
describe("materializeFileAtMentions", () => {
  it("materializes @file mentions into snapshot blocks", async () => {
    const tmpDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), "lattice-materialize-"));
    ...
    try {
      ...
      expect(result[0]?.block).toContain('<lattice-file path="src/foo.ts"');
