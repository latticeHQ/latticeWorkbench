...
describe("hooks", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-hooks-test-"));
  ...
  describe("getToolEnvPath", () => {
    ...
    test("finds project-level tool_env", async () => {
      const envDir = path.join(tempDir, ".lattice");
  ...
  describe("runPreHook", () => {
    ...
    test("receives LATTICE_TOOL env var", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_pre");
      await fs.mkdir(hookDir, { recursive: true });
      await fs.writeFile(hookPath, '#!/bin/bash\necho "tool=$LATTICE_TOOL"');
      await fs.chmod(hookPath, 0o755);
  ...
  describe("runPostHook", () => {
    ...
    test("receives LATTICE_TOOL_RESULT env var", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_post");
      await fs.mkdir(hookDir, { recursive: true });
      await fs.writeFile(hookPath, '#!/bin/bash\necho "result=$LATTICE_TOOL_RESULT"');
      await fs.chmod(hookPath, 0o755);
  ...
  describe("runWithHook", () => {
    test("executes tool when hook prints $LATTICE_EXEC", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_hook");
      await fs.mkdir(hookDir, { recursive: true });
...
`
    ...
    test("blocks tool when hook exits before $LATTICE_EXEC", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_hook");
      await fs.mkdir(hookDir, { recursive: true });
...
`
    ...
    test("receives tool input via LATTICE_TOOL_INPUT env var", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_hook");
      await fs.mkdir(hookDir, { recursive: true });
...
`
    ...
    test("handles hook paths with spaces", async () => {
      ...
      const hookDir = path.join(spacedDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_hook");
      await fs.mkdir(hookDir, { recursive: true });
...
`
    ...
    test("succeeds when hook exits without reading LATTICE_RESULT", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_hook");
      await fs.mkdir(hookDir, { recursive: true });
...
`
      ...
      expect(hook.toolExecuted).toBe(false);
      expect(hook.success).toBe(false);
      expect(hook.stderr).toContain("Hook timed out before $LATTICE_EXEC");
      expect(result).toBeUndefined();
    });
...
`
    ...
    test("writes large tool input to LATTICE_TOOL_INPUT_PATH", async () => {
      const hookDir = path.join(tempDir, ".lattice");
      const hookPath = path.join(hookDir, "tool_hook");
      await fs.mkdir(hookDir, { recursive: true });
      ...
      await fs.writeFile(
        hookPath,
        `#!/bin/bash
echo "ENV_INPUT=$LATTICE_TOOL_INPUT" >&2

if [ -z "$LATTICE_TOOL_INPUT_PATH" ]; then
  echo "NO_PATH" >&2
  exit 1
fi

len=$(wc -c < "$LATTICE_TOOL_INPUT_PATH")
echo "LEN=$len" >&2
...
`
      ...
      expect(hook.success).toBe(true);
      expect(hook.stderr).toContain("ENV_INPUT=__LATTICE_TOOL_INPUT_FILE__");
