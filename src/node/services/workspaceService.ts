...
export class WorkspaceService extends EventEmitter {
  ...
  async remove(workspaceId: string, force = false): Promise<Result<void>> {
    if (workspaceId === LATTICE_HELP_CHAT_WORKSPACE_ID) {
    ...
    try {
      ...
      if (metadataResult.success) {
        ...
        // deleting ~/.lattice/sessions/<workspaceId>/session-timing.json.
        ...
        // deleting ~/.lattice/sessions/<workspaceId>/session-usage.json.
  ...
  async fork(
    ...
    try {
      if (sourceWorkspaceId === LATTICE_HELP_CHAT_WORKSPACE_ID) {
      ...
      // Apply runtime-provided config updates (e.g., Lattice marks shared workspaces)
  ...
  async sendMessage(
    ...
    try {
      ...
      const latticeMeta = options?.latticeMetadata as
  ...
  async getChatHistory(workspaceId: string): Promise<LatticeMessage[]> {
    try {
