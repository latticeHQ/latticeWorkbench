...
function buildEnvironmentContext(workspacePath: string, runtimeType: RuntimeMode): string {
  ...
  // Remote runtimes: clarify that LATTICE_PROJECT_PATH is the user's local path
  const isRemote =
  ...
  if (isRemote) {
    lines = [
      ...lines,
      "- $LATTICE_PROJECT_PATH refers to the user's local machine, not this environment",
    ];
  }
...
const CORE_SKILL_NAMES = new Set(["docx", "init", "pdf", "pptx", "skill-creator", "lattice-docs", "xlsx"]);
...
/**
 * Get the system directory where global lattice configuration lives.
 * Users can place global AGENTS.md and .lattice/PLAN.md files here.
 */
function getSystemDirectory(): string {
  return getLatticeHome();
}
...
export function extractToolInstructions(
  ...
  options?: {
    ...
    enableLatticeGlobalAgentsTools?: boolean;
