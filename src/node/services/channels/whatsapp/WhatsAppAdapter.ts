...
export class WhatsAppAdapter extends EventEmitter implements ChannelAdapter {
  ...
  private get webhookVerifyToken(): string {
    return (
      (this.config?.settings?.webhookVerifyToken as string) ??
      this.config?.credentials.webhookVerifyToken ??
      `lattice-wa-${this.accountId}`
    );
  }
  ...
  async connect(config: ChannelConfig): Promise<void> {
    ...
    try {
      ...
      this._botUsername =
        phoneInfo.verified_name ?? phoneInfo.display_phone_number ?? this.phoneNumberId;
  ...
  async downloadFile(mediaId: string): Promise<{ dataUrl: string; mimeType: string } | null> {
    ...
    try {
      ...
      if (!mediaRes.ok) {
        log.warn("[WhatsAppAdapter] Media info request failed", {
          mediaId,
      ...
      const mediaInfo = (await mediaRes.json()) as {
        url?: string;
        mime_type?: string;
        file_size?: number;
      };
