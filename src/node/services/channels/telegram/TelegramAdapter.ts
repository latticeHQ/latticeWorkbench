...
export class TelegramAdapter extends EventEmitter implements ChannelAdapter {
      ...
      try {
        ...
        if (!response.ok) {
          if (response.status === 409) {
            ...
            log.error(
              "[TelegramAdapter] 409 Conflict — another process is polling this bot token. Stopping.",
              {
                accountId: this.accountId,
              }
            );
          ...
          log.error("[TelegramAdapter] Polling HTTP error", {
  ...
  private handleTelegramMessage(msg: TelegramMessage): void {
    ...
    const hasBotMention = this._botUsername
      ? allEntities.some(
          (e) =>
            e.type === "mention" &&
            (msg.text ?? msg.caption ?? "").slice(e.offset, e.offset + e.length).toLowerCase() ===
  ...
  async sendMessage(message: OutboundChannelMessage): Promise<ChannelSendResult> {
    ...
    try {
      ...
      if (
        !data.ok &&
        (data.description?.includes("parse") || data.description?.includes("entity"))
      ) {
   ...
   */
  async sendPhoto(chatId: string, photoUrl: string, caption?: string): Promise<ChannelSendResult> {
    if (!this.config) {
      return { success: false, error: "Not connected" };
    }
