...
describe("ServerLockfile", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-lock-test-"));
  ...
  test("acquire overwrites existing lockfile", async () => {
    ...
    await lockfile.acquire("https://my.machine.local/lattice", "second-token");

    const data = await lockfile.read();
    expect(data).not.toBeNull();
    expect(data!.baseUrl).toBe("https://my.machine.local/lattice");
  ...
  test("supports URLs with path prefixes", async () => {
    await lockfile.acquire("https://my.machine.local/lattice/", "path-token");
