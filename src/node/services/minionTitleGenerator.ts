...
/** Schema for AI-generated minion identity (area name + descriptive title) */
...
export interface MinionIdentity {
...
export interface GenerateMinionIdentityResult extends MinionIdentity {
...
export function mapNameGenerationError(error: unknown, modelString: string): NameGenerationError {
  ...
  if (NoObjectGeneratedError.isInstance(error)) {
    return {
      type: "unknown",
      raw: "The model returned an unexpected format while generating a minion name.",
    };
  }
...
/**
 * Generate minion identity (name + title) using AI.
 ...
 */
export function buildMinionIdentityPrompt(
  message: string,
  ...
  return [
    "Generate a minion name and title for this development task:\n\n",
    `${promptSections.join("\n\n")}\n\n`,
    "Requirements:\n",
...
export async function generateMinionIdentity(
  message: string,
  candidates: string[],
  aiService: AIService,
  ...
  latestUserMessage?: string
): Promise<Result<GenerateMinionIdentityResult, NameGenerationError>> {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    try {
      ...
      const currentStream = streamText({
        ...
        output: Output.object({ schema: minionIdentitySchema }),
        prompt: buildMinionIdentityPrompt(message, conversationContext, latestUserMessage),
      });
