...
const PREFIX = "lattice-policy-service-test-";
...
describe("PolicyService", () => {
  ...
  beforeEach(async () => {
    ...
    prevPolicyFileEnv = process.env.LATTICE_POLICY_FILE;
  });

  afterEach(async () => {
    if (prevPolicyFileEnv === undefined) {
      delete process.env.LATTICE_POLICY_FILE;
    } else {
      process.env.LATTICE_POLICY_FILE = prevPolicyFileEnv;
    }

    await rm(tempDir, { recursive: true, force: true });
  });

  test("disabled when LATTICE_POLICY_FILE is unset", async () => {
    delete process.env.LATTICE_POLICY_FILE;
  ...
  test("blocks startup when policy file fails to parse", async () => {
    ...
    process.env.LATTICE_POLICY_FILE = policyPath;
  ...
  test("loads policy from a remote URI", async () => {
    ...
    try {
      ...
      process.env.LATTICE_POLICY_FILE = `http://127.0.0.1:${address.port}/policy.json`;
  ...
  test("loads policy from Governor when enrolled", async () => {
    ...
    const server = createServer((req, res) => {
      receivedAuth = req.headers["lattice-governor-session-token"] as string | undefined;
      ...
      if (req.headers["lattice-governor-session-token"] !== token) {
        res.writeHead(401);
    ...
    try {
      ...
      await config.editConfig((existing) => ({
        ...existing,
        latticeGovernorUrl: governorOrigin,
        latticeGovernorToken: token,
      }));
  ...
  test("LATTICE_POLICY_FILE takes precedence over Governor enrollment", async () => {
    const token = "governor-test-token";
    const policy = {
      policy_format_version: "0.1",
