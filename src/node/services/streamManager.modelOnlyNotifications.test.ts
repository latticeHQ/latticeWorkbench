...
describe("StreamManager - model-only tool notifications", () => {
  ...
  test("strips __lattice_notifications before emitting tool-call-end", async () => {
    ...
    const mockStreamResult = {
      ...
      fullStream: (async function* () {
        ...
        yield {
          ...
          output: {
            ok: true,
            __lattice_notifications: ["<notification>hello</notification>"],
          },
        };
      })(),
    ...
    await (
      method as (minionId: string, streamInfo: unknown, historySequence: number) => Promise<void>
    ).call(streamManager, "test-minion", streamInfo, 1);

    const toolEnd = events.find((e) => e.toolName === "bash");
    expect(toolEnd).toBeDefined();

    expect(toolEnd?.result && typeof toolEnd.result === "object").toBe(true);
    expect("__lattice_notifications" in (toolEnd!.result as Record<string, unknown>)).toBe(false);
  });
