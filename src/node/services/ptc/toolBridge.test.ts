...
describe("ToolBridge", () => {
  ...
  describe("register", () => {
    it("registers tools under lattice namespace", () => {
    ...
    it("validates arguments before executing tool", async () => {
      ...
      let registeredLattice: Record<string, (...args: unknown[]) => Promise<unknown>> = {};
      const mockRegisterObject = mock(
        (name: string, obj: Record<string, (...args: unknown[]) => Promise<unknown>>) => {
          if (name === "lattice") registeredLattice = obj;
          return undefined;
        }
      );
      const mockRuntime = createMockRuntime({ registerObject: mockRegisterObject });
      ...
      const fileRead = registeredLattice.file_read as (...args: unknown[]) => Promise<unknown>;
      try {
    ...
    it("serializes non-JSON values", async () => {
      ...
      const result = await registeredLattice.circular({});
    ...
    it("uses runtime abort signal for tool cancellation", async () => {
      ...
      await registeredLattice.file_read({ filePath: "test.txt" });
      expect(mockExecute).toHaveBeenCalledTimes(1);
    });
