...
/**
 * Get cached lattice types or generate new ones if tool definitions changed.
 */
export async function getCachedLatticeTypes(tools: Record<string, Tool>): Promise<string> {
 ...
 */
export async function generateLatticeTypes(tools: Record<string, Tool>): Promise<string> {
  const lines: string[] = ["declare namespace lattice {"];
