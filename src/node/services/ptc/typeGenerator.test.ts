...
import { generateLatticeTypes, getCachedLatticeTypes, clearTypeCache } from "./typeGenerator";
...
describe("generateLatticeTypes", () => {
  test("generates interface from tool input schema", async () => {
    ...
    const types = await generateLatticeTypes({ file_read: fileReadTool });
  ...
  test("generates optional properties for fields with .default()", async () => {
    ...
    const types = await generateLatticeTypes({ my_tool: tool });
  ...
  test("generates discriminated union result types with success: true/false", async () => {
    ...
    const types = await generateLatticeTypes({ bash: bashTool });
  ...
  test("handles MCP tools with MCPCallToolResult", async () => {
    ...
    const types = await generateLatticeTypes({ mcp__github__create_issue: mcpTool });
  ...
  test("handles tools without known result type (returns unknown)", async () => {
    ...
    const types = await generateLatticeTypes({ custom_tool: customTool });
  ...
  test("declares console global", async () => {
    const types = await generateLatticeTypes({});
  ...
  test("converts snake_case tool names to PascalCase for types", async () => {
    ...
    const types = await generateLatticeTypes({ file_edit_replace_string: tool });
  ...
  test("sorts tools alphabetically for deterministic output", async () => {
    ...
    const types = await generateLatticeTypes(tools);
  ...
  test("handles JSON Schema input (MCP tools)", async () => {
    ...
    const types = await generateLatticeTypes({ mcp__github__list_repos: mcpTool });
  ...
  test("handles empty tool set", async () => {
    ...
    expect(types).toContain("declare namespace lattice {");
    expect(types).toContain("}");
    expect(types).toContain("declare var console");
  });
});

describe("getCachedLatticeTypes", () => {
  beforeEach(() => {
    clearTypeCache();
  });
  ...
  test("invalidates cache when tool description changes", async () => {
    ...
    const types1 = await getCachedLatticeTypes({ my_tool: tool1 });
    expect(types1).toContain("Version 1");

    const types2 = await getCachedLatticeTypes({ my_tool: tool2 });
  ...
  test("returns cached types when tools are identical", async () => {
    ...
    const types1 = await getCachedLatticeTypes({ my_tool: tool });
    const types2 = await getCachedLatticeTypes({ my_tool: tool });
