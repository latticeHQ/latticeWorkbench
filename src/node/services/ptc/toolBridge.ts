 ...
 * Tool Bridge for PTC
 *
 * Bridges Lattice tools into the QuickJS sandbox, making them callable via `lattice.*` namespace.
...
/**
 * Bridge that exposes Lattice tools in the QuickJS sandbox under `lattice.*` namespace.
 */
export class ToolBridge {
  ...
  /**
   * Register all bridgeable tools on the runtime under `lattice` namespace.
   *
  ...
  register(runtime: IJSRuntime): void {
    const latticeObj: Record<string, (...args: unknown[]) => Promise<unknown>> = {};
    ...
    for (const [name, tool] of this.bridgeableTools) {
      ...
      latticeObj[name] = async (args: unknown) => {
    ...
    runtime.registerObject("lattice", latticeObj);
  }
