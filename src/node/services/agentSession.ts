...
function extractAcpPromptId(latticeMetadata: unknown): string | undefined {
  if (typeof latticeMetadata !== "object" || latticeMetadata == null || Array.isArray(latticeMetadata)) {
    return undefined;
  }

  return normalizeAcpPromptId((latticeMetadata as Record<string, unknown>)[ACP_PROMPT_ID_METADATA_KEY]);
}

function extractAcpDelegatedTools(latticeMetadata: unknown): string[] | undefined {
  if (typeof latticeMetadata !== "object" || latticeMetadata == null || Array.isArray(latticeMetadata)) {
    return undefined;
  }

  return normalizeDelegatedToolNames(
    (latticeMetadata as Record<string, unknown>)[ACP_DELEGATED_TOOLS_METADATA_KEY]
  );
}
...
export class AgentSession {
  ...
  /** Track user message ids that already hard-restarted for exec-like sidekicks. */
  private readonly execSidekickHardRestartAttempts = new Set<string>();
  ...
  constructor(options: AgentSessionOptions) {
    ...
    assert(trimmedMinionId.length > 0, "minionId must not be empty");

    this.minionId = trimmedMinionId;
    this.config = config;
    ...
    this.compactionHandler = new CompactionHandler({
      ...
      sessionDir: this.config.getSessionDir(this.minionId),
      telemetryService,
  ...
  dispose(): void {
    ...
    void this.aiService.stopStream(this.minionId, { abandonPartial: true });
    // Terminate background processes for this minion (skip when flagged for bench/CI)
    if (!this.keepBackgroundProcesses) {
      void this.backgroundProcessManager.cleanup(this.minionId);
    }
  ...
  emitMetadata(metadata: FrontendMinionMetadata | null): void {
  ...
  private getAutoRetryPreferencePath(): string {
    return path.join(this.config.getSessionDir(this.minionId), AUTO_RETRY_PREFERENCE_FILE);
  }
  ...
  private isAiStreaming(): boolean {
    ...
    return aiService.isStreaming(this.minionId);
  }
  ...
  private isPendingAskUserQuestion(message: LatticeMessage | null | undefined): boolean {
  ...
  private isSyntheticSnapshotUserMessage(message: LatticeMessage): boolean {
    return (
      message.role === "user" &&
  ...
  private getLastNonSystemHistoryMessage(historyTail: LatticeMessage[]): LatticeMessage | undefined {
  ...
  private async getMinionMetadataForRetry(): Promise<MinionMetadata | undefined> {
    const aiService = this.aiService as Partial<Pick<AIService, "getMinionMetadata">>;
    if (typeof aiService.getMinionMetadata !== "function") {
      return undefined;
    }

    const metadataResult = await aiService.getMinionMetadata(this.minionId);
  ...
  private shouldUseUserMessageForRetry(message: LatticeMessage): boolean {
    ...
    if (message.metadata?.synthetic === true) {
      return (
        ...
        isCompactionRequestMetadata(message.metadata?.latticeMetadata)
      );
    }
  ...
  private async deriveStartupAutoRetryOptions(params: {
    partial: LatticeMessage | null;
    historyTail: LatticeMessage[];
  }): Promise<SendMessageOptions | undefined> {
    const lastUserMessage = [...params.historyTail]
      .reverse()
      .find((message): message is LatticeMessage & { role: "user" } =>
    ...
    const lastAssistantMessage =
      params.partial?.role === "assistant"
        ...
        : [...params.historyTail]
            .reverse()
            .find(
              (message): message is LatticeMessage & { role: "assistant" } =>
                message.role === "assistant"
            );

    const minionMetadata = await this.getMinionMetadataForRetry();

    const persistedRetrySendOptions = lastUserMessage?.metadata?.retrySendOptions;

    const minionAgentId =
      this.normalizeAgentIdForRetry(minionMetadata?.agentId ?? minionMetadata?.agentType) ??
    ...
    const baseAgentId = persistedAgentId ?? assistantAgentId ?? minionAgentId;

    const agentSettings =
      minionMetadata?.aiSettingsByAgent?.[baseAgentId] ??
      minionMetadata?.aiSettingsByAgent?.[minionAgentId] ??
      minionMetadata?.aiSettings;
    const compactSettings = minionMetadata?.aiSettingsByAgent?.compact;
    ...
    const persistedDisableMinionAgents =
      lastUserMessage?.metadata?.disableMinionAgents ??
      persistedRetrySendOptions?.disableMinionAgents;
    ...
    const lastUserLatticeMetadata = lastUserMessage?.metadata?.latticeMetadata;
    if (isCompactionRequestMetadata(lastUserLatticeMetadata)) {
      const compactionModel =
        this.normalizeStartupModel(lastUserLatticeMetadata.parsed.model) ?? baseModel;
      ...
      const compactionOptions: SendMessageOptions = {
        ...
        maxOutputTokens:
          typeof lastUserLatticeMetadata.parsed.maxOutputTokens === "number"
            ? lastUserLatticeMetadata.parsed.maxOutputTokens
            : persistedMaxOutputTokens,
        toolPolicy: [{ regex_match: ".*", action: "disable" }],
        skipAiSettingsPersistence: true,
        disableMinionAgents: persistedDisableMinionAgents,
      };
    ...
    if (typeof persistedDisableMinionAgents === "boolean") {
      retryOptions.disableMinionAgents = persistedDisableMinionAgents;
    }
  ...
  async getStartupAutoRetryModelHint(): Promise<string | null> {
    ...
    const [partial, historyResult] = await Promise.all([
      this.historyService.readPartial(this.minionId),
      this.historyService.getLastMessages(this.minionId, 20),
    ]);
      ...
      await new Promise<void>((resolve) => {
        const maybeResolve = (...args: unknown[]) => {
          ...
          if (
            ...
            payload !== null &&
            "minionId" in payload &&
            (payload as { minionId: unknown }).minionId !== this.minionId
          ) {
            return;
          }
        ...
        maybeResolve({ minionId: this.minionId });
      });
    }
  }
  ...
  private async emitHistoricalEvents(
    ...
    const emitReplayMessage = (message: MinionChatMessage): void => {
      emittedReplayMessages = true;
      listener({ minionId: this.minionId, message });
    };

    let emittedReplayStreamEvents = false;
    const replayStreamEventTracker = (event: AgentSessionChatEvent) => {
      if (event.minionId !== this.minionId) {
        return;
      }
    ...
    // Without caught-up, the frontend stays in "Loading minion..." forever.
    try {
      if (mode?.type === "live") {
        replayMode = "live";
        ...
        const liveStreamInfo = this.aiService.getStreamInfo(this.minionId);
        if (liveStreamInfo) {
          const streamLastTimestamp = this.getStreamLastTimestamp(liveStreamInfo);
          await this.aiService.replayStream(this.minionId, {
            afterTimestamp: streamLastTimestamp,
          });

          // Stream can end while replayStream runs; only expose cursor when still active.
          const liveStreamInfoAfterReplay = this.aiService.getStreamInfo(this.minionId);
        ...
        await this.initStateManager.replayInit(this.minionId);
      ...
      const streamInfo = this.aiService.getStreamInfo(this.minionId);
      const partial = await this.historyService.readPartial(this.minionId);
      ...
      // Older compaction epochs are fetched on demand through minion.history.loadMore.
      ...
      if (streamInfo) {
        await this.aiService.replayStream(this.minionId, { afterTimestamp });
      }
      ...
      const streamInfoAfterReplay = this.aiService.getStreamInfo(this.minionId);
      ...
      log.error("Failed to replay history for minion", {
        minionId: this.minionId,
        error,
      });
  ...
  async ensureMetadata(args: {
    ...
    const { minionPath, projectName, runtimeConfig } = args;

    assert(typeof minionPath === "string", "minionPath must be a string");
    const trimmedMinionPath = minionPath.trim();
    assert(trimmedMinionPath.length > 0, "minionPath must not be empty");

    const normalizedMinionPath = path.resolve(trimmedMinionPath);
    const existing = await this.aiService.getMinionMetadata(this.minionId);

    if (existing.success) {
      // Metadata already exists, verify minion path matches
      const metadata = existing.data;
      // For in-place minions (projectPath === name), use path directly
      ...
      const expectedPath = isInPlace
        ...
        : (() => {
            ...
            return runtime.getMinionPath(metadata.projectPath, metadata.name);
          })();
      assert(
        expectedPath === normalizedMinionPath,
        `Existing metadata minion path mismatch for ${this.minionId}: expected ${expectedPath}, got ${normalizedMinionPath}`
      );
      return;
    }

    // Detect in-place minion: if minionPath is not under srcBaseDir,
    // it's a direct minion (e.g., for CLI/benchmarks) rather than a worktree
    const srcBaseDir = this.config.srcDir;
    const normalizedSrcBaseDir = path.resolve(srcBaseDir);
    const isUnderSrcBaseDir = normalizedMinionPath.startsWith(normalizedSrcBaseDir + path.sep);
    ...
    if (isUnderSrcBaseDir) {
      // Standard worktree mode: minion is under ~/.lattice/src/project/branch
      derivedProjectPath = path.dirname(normalizedMinionPath);
      minionName = PlatformPaths.basename(normalizedMinionPath);
      derivedProjectName =
        projectName && projectName.trim().length > 0
          ? projectName.trim()
          : PlatformPaths.basename(derivedProjectPath) || "unknown";
    } else {
      // In-place mode: minion is a standalone directory
      // Store the minion path directly by setting projectPath === name
      derivedProjectPath = normalizedMinionPath;
      minionName = normalizedMinionPath;
      derivedProjectName =
        projectName && projectName.trim().length > 0
          ? projectName.trim()
          : PlatformPaths.basename(normalizedMinionPath) || "unknown";
    }
  ...
  async sendMessage(
    ...
    let preservedEditFileParts: LatticeFilePart[] | undefined;
    if (editMessageId && fileParts === undefined) {
      const historyResult = await this.historyService.getHistoryFromLatestBoundary(
        this.minionId
      );
      if (historyResult.success) {
        const targetMessage: LatticeMessage | undefined = historyResult.data.find(
          (msg) => msg.id === editMessageId
        );
        const fileParts = targetMessage?.parts.filter(
          (part): part is LatticeFilePart => part.type === "file"
        );
    ...
    if (editMessageId) {
      ...
      if (this.isBusy()) {
        ...
        try {
          ...
          // Minion teardown does not await in-flight async work; bail out if the session was
    ...
    // latticeMetadata is z.any() in schema - cast to proper type
    const typedLatticeMetadata = options?.latticeMetadata as LatticeMessageMetadata | undefined;
    const acpPromptId =
      normalizeAcpPromptId(options?.acpPromptId) ?? extractAcpPromptId(typedLatticeMetadata);
    const delegatedToolNames =
      normalizeDelegatedToolNames(options?.delegatedToolNames) ??
      extractAcpDelegatedTools(typedLatticeMetadata);
    const isCompactionRequest = isCompactionRequestMetadata(typedLatticeMetadata);
    ...
    // Normalize model strings up front so downstream code always sees trimmed IDs.
    const normalizedModel = options.model.trim();
    const normalizedSystem1Model =
      options.system1Model?.trim() || undefined;
    options = { ...options, model: normalizedModel, system1Model: normalizedSystem1Model };
    ...
    const userMessage = createLatticeMessage(
      ...
      message,
      {
        ...
        latticeMetadata: typedLatticeMetadata, // Pass through frontend metadata as black-box
        ...(acpPromptId != null ? { acpPromptId } : {}),
    ...
    let skillSnapshotResult: { snapshotMessage: LatticeMessage } | null = null;
    try {
      skillSnapshotResult = await this.materializeAgentSkillSnapshot(
        typedLatticeMetadata,
        options?.disableMinionAgents
      );
    } catch (error) {
    ...
    let autoCompactionMessage: LatticeMessage | null = null;
    if (!isCompactionRequest && !editMessageId) {
      ...
      if (shouldCompactBeforeSend) {
        ...
        const followUpContent = this.buildAutoCompactionFollowUp({
          ...
          latticeMetadata: typedLatticeMetadata,
        });
        ...
        autoCompactionMessage = createLatticeMessage(
          createUserMessageId(),
          "user",
          autoCompactionRequest.messageText,
          {
            ...
            disableMinionAgents: optionsForStream.disableMinionAgents,
            retrySendOptions: pickStartupRetrySendOptions(autoCompactionRequest.sendOptions),
            latticeMetadata: autoCompactionRequest.metadata,
            synthetic: true,
            uiVisible: true,
          }
        );
    ...
    if (!autoCompactionMessage) {
      const appendResult = await this.historyService.appendToHistory(this.minionId, userMessage);
    ...
    // Minion may be tearing down while we await filesystem IO.
    ...
    try {
      ...
      if (isCompactionStreamRequest && !this.keepBackgroundProcesses) {
        await this.backgroundProcessManager.cleanup(this.minionId);
  ...
  async resumeStream(
    ...
    // Normalize model strings so downstream code always sees trimmed IDs.
    const normalizedModel = options.model.trim();
    const normalizedSystem1Model =
      options.system1Model?.trim() || undefined;
    ...
    const modelForStream = optionsForStream.model;
  ...
  private is1MContextEnabledForModel(modelString: string, options?: SendMessageOptions): boolean {
    if (!supports1MContext(modelString)) {
      return false;
    }
  ...
  private buildAutoCompactionFollowUp(params: {
    ...
    if (params.latticeMetadata) {
      followUp.latticeMetadata = params.latticeMetadata;
    }
  ...
  private getPreferredCompactionModel(): string | null {
    try {
      ...
      const normalized = preferredCompactionModel.trim();
  ...
  private async interruptForCompaction(): Promise<void> {
    ...
    try {
      const stopResult = await this.aiService.stopStream(this.minionId, {
        abortReason: "system",
      });
      if (!stopResult.success) {
  ...
  async interruptStream(options?: {
    ...
    if (options?.abandonPartial && !options?.soft) {
      const deleteResult = await this.historyService.deletePartial(this.minionId);
  ...
  private async syncAgentSwitchingForResolvedAgent(
    ...
    // Guard for test mocks that may not implement getMinionMetadata.
    if (typeof this.aiService.getMinionMetadata !== "function") {
      return;
    }
    ...
    try {
      ...
      if (!metadataResult.success) {
        log.warn("Failed to read minion metadata while syncing agent switching", {
          minionId: this.minionId,
      ...
      await this.config.updateMinionMetadata(this.minionId, {
  ...
  private async streamWithHistory(
    ...
    const commitResult = await this.historyService.commitPartial(this.minionId);
    if (!commitResult.success) {
      return Err(createUnknownSendMessageError(commitResult.error));
    }

    let historyResult = await this.historyService.getHistoryFromLatestBoundary(this.minionId);
    if (!historyResult.success) {
    ...
    if (historyResult.data.length === 0) {
      return Err(
        createUnknownSendMessageError(
          "Cannot resume stream: minion history is empty. Send a new message instead."
        )
      );
    }
    ...
    if (lastMsg?.role === "assistant" && !lastMsg.metadata?.partial) {
      ...
      const sentinelMessage = createLatticeMessage(createUserMessageId(), "user", "[CONTINUE]", {
        timestamp: Date.now(),
        synthetic: true,
      });
      await this.historyService.appendToHistory(this.minionId, sentinelMessage);
      const refreshed = await this.historyService.getHistoryFromLatestBoundary(this.minionId);
    ...
    const acpPromptId =
      normalizeAcpPromptId(options?.acpPromptId) ?? extractAcpPromptId(options?.latticeMetadata);
    const delegatedToolNames =
      normalizeDelegatedToolNames(options?.delegatedToolNames) ??
  ...
  private resolveCompactionRequest(
    ...
    for (let index = history.length - 1; index >= 0; index -= 1) {
      ...
      const latticeMetadata = message.metadata?.latticeMetadata;
      if (!isCompactionRequestMetadata(latticeMetadata)) {
        return undefined;
      }
      return {
        id: message.id,
  ...
  private async clearFailedAssistantMessage(messageId: string, reason: string): Promise<void> {
    const [partialResult, deleteMessageResult] = await Promise.all([
      this.historyService.deletePartial(this.minionId),
      this.historyService.deleteMessage(this.minionId, messageId),
    ]);
  ...
  private withAnthropic1MContext(
    ...
    return {
      model: modelString,
      agentId: MINION_DEFAULTS.agentId,
      providerOptions: {
        anthropic: {
          use1MContext: true,
  ...
  private async maybeHardRestartExecSidekickOnContextExceeded(data: {
    messageId: string;
    errorType?: string;
  }): Promise<boolean> {
    if (data.errorType !== "context_exceeded") {
    ...
    const context = this.activeStreamContext;
    const requestId = this.activeStreamUserMessageId;
    const experimentEnabled = context?.options?.experiments?.execSidekickHardRestart === true;
    ...
    if (this.execSidekickHardRestartAttempts.has(requestId)) {
      return false;
    }
    ...
    if (!metadata.parentMinionId) {
      return false;
    }

    const agentIdRaw = (metadata.agentId ?? metadata.agentType ?? MINION_DEFAULTS.agentId)
      .trim()
      .toLowerCase();
    ...
    try {
      const parentMetadataResult = await this.aiService.getMinionMetadata(
        metadata.parentMinionId
      );
    ...
    for (const agentMetadata of metadataCandidates) {
      try {
        const runtime = createRuntimeForMinion(agentMetadata);
        ...
        const minionPath = isInPlace
          ...
          : runtime.getMinionPath(agentMetadata.projectPath, agentMetadata.name);

        const agentDiscoveryPath =
          context.options?.disableMinionAgents === true
            ? agentMetadata.projectPath
            : minionPath;
    ...
    this.execSidekickHardRestartAttempts.add(requestId);

    const continuationNotice =
      "Context limit reached. Lattice restarted this agent's chat history and will replay your original prompt below. " +
      "Continue using only the current minion state (files, git history, command output); " +
      "re-inspect the repo as needed.";

    log.info("Exec-like sidekick hit context limit; hard-restarting history and retrying", {
      minionId: this.minionId,
      requestId,
      model: context.modelString,
      agentId,
    ...
    const historyResult = await this.historyService.getHistoryFromLatestBoundary(this.minionId);
    if (!historyResult.success) {
      return false;
    }
    ...
    try {
      await this.compactionHandler.discardPendingDiffs("execSidekickHardRestart");
      this.onPostCompactionStateChange?.();
    } catch (error) {
    ...
    const partialDeleteResult = await this.historyService.deletePartial(this.minionId);
    if (!partialDeleteResult.success) {
      log.warn("Failed to delete partial before exec sidekick hard restart", {
        minionId: this.minionId,
        error: partialDeleteResult.error,
      });
    }

    const clearResult = await this.historyService.clearHistory(this.minionId);
    if (!clearResult.success) {
      log.warn("Failed to clear history for exec sidekick hard restart", {
        minionId: this.minionId,
    ...
    const cloneForAppend = (msg: LatticeMessage): LatticeMessage => {
    ...
    const continuationMessage = createLatticeMessage(
      createUserMessageId(),
      "user",
      continuationNotice,
      {
    ...
    for (const message of messagesToAppend) {
      const appendResult = await this.historyService.appendToHistory(this.minionId, message);
      if (!appendResult.success) {
        log.error("Failed to append message during exec sidekick hard restart", {
          minionId: this.minionId,
          messageId: message.id,
    ...
    if (!retryResult.success) {
      log.error("Exec sidekick hard restart retry failed to start", {
        minionId: this.minionId,
        error: retryResult.error,
      });
      return false;
  ...
  private async handleStreamError(data: StreamErrorPayload): Promise<void> {
    ...
    if (
      await this.maybeHardRestartExecSidekickOnContextExceeded({
        messageId: data.messageId,
        errorType: data.errorType,
      })
    ) {
  ...
  private attachAiListeners(): void {
    const forward = (
      event: string,
      handler: (payload: MinionChatMessage) => Promise<void> | void
    ) => {
      const wrapped = (...args: unknown[]) => {
        ...
        void handler(payload as MinionChatMessage);
      };
    ...
    forward("tool-call-end", (payload) => {
      ...
      // Trigger a metadata refresh so the workbench panel updates immediately.
      if (
    ...
    forward("stream-end", async (payload) => {
      ...
      try {
        ...
        if (this.deferQueuedFlushUntilAfterEdit) {
          ...
          this.backgroundProcessManager.setMessageQueued(this.minionId, false);
        } else {
    ...
    const errorHandler = (...args: unknown[]) => {
      ...
      if (
        ...
        raw === null ||
        !("minionId" in raw) ||
        (raw as { minionId: unknown }).minionId !== this.minionId
      ) {
        return;
      }
      const data = raw as StreamErrorPayload & { minionId: string };
  ...
  private attachInitListeners(): void {
    const forward = (event: string, handler: (payload: MinionChatMessage) => void) => {
      const wrapped = (...args: unknown[]) => {
        const [payload] = args;
        if (
          typeof payload === "object" &&
        ...
        // Strip minionId from payload before forwarding (MinionInitEvent doesn't include it)
        const { minionId: _, ...message } = payload as MinionChatMessage & {
          minionId: string;
        };
        handler(message as MinionChatMessage);
      };
  ...
  // Public method to emit chat events (used by init hooks and other minion events)
  emitChatEvent(message: MinionChatMessage): void {
    // NOTE: Minion teardown does not await in-flight async work (sendMessage(), stopStream(), etc).
  ...
  queueMessage(
    ...
    this.backgroundProcessManager.setMessageQueued(this.minionId, true);
  }
  ...
  sendQueuedMessages(): void {
    // sendQueuedMessages can race with teardown (e.g. minion.remove) because we
  ...
  private async isAgentSwitchTargetValid(
    ...
    if (typeof this.aiService.getMinionMetadata !== "function") {
      log.warn("Cannot validate switch_agent target: minion metadata API unavailable", {
        minionId: this.minionId,
    ...
    const metadataResult = await this.aiService.getMinionMetadata(this.minionId);
    if (!metadataResult.success) {
      log.warn("Cannot validate switch_agent target: minion metadata unavailable", {
        minionId: this.minionId,
        targetAgentId: parsedAgentId.data,
        error: metadataResult.error,
      });
    ...
    // In-place minions (CLI/benchmarks) have projectPath === name.
    // Use the path directly instead of reconstructing via getMinionPath.
    const isInPlace = metadata.projectPath === metadata.name;
    const minionPath = isInPlace
      ? metadata.projectPath
      : runtime.getMinionPath(metadata.projectPath, metadata.name);

    // When disableMinionAgents is active, use project path for discovery
    // (only built-in/global agents). Mirrors resolveAgentForStream behavior.
    const discoveryPath = disableMinionAgents ? metadata.projectPath : minionPath;
  ...
  private async resolveAgentSwitchFallbackTarget(
    ...
    const disableMinionAgents = currentOptions?.disableMinionAgents;
    ...
    for (const candidate of candidates) {
      ...
      if (await this.isAgentSwitchTargetValid(candidate, disableMinionAgents)) {
        return candidate;
      }
    }
  ...
  private buildAgentSwitchFallbackFollowUp(switchResult: SwitchAgentResult): string {
    ...
    const lines = [
      `Agent handoff failed: target "${switchResult.agentId}" is unavailable in this minion.`,
  ...
  private async dispatchAgentSwitch(
    ...
    const targetValid = await this.isAgentSwitchTargetValid(
      targetAgentId,
      currentOptions?.disableMinionAgents
    );
    if (!targetValid) {
    ...
    const normalizedMinionModel = minionAiSettings?.model?.trim();
    const effectiveModel =
      ...
      (normalizedOptionModel != null && normalizedOptionModel.length > 0
        ...
        : undefined) ??
      (normalizedMinionModel != null && normalizedMinionModel.length > 0
        ? normalizedMinionModel
        : undefined) ??
      fallbackModel.trim();
    ...
    const followUpOptions: SendMessageOptions = {
      ...
      ...(currentOptions?.disableMinionAgents != null && {
        disableMinionAgents: currentOptions.disableMinionAgents,
      }),
  ...
  private async dispatchPendingFollowUp(): Promise<void> {
    ...
    const historyResult = await this.historyService.getLastMessages(this.minionId, 1);
    if (!historyResult.success) {
      const historyError =
    ...
    const latticeMeta = lastMessage.metadata?.latticeMetadata;

    // Check if it's a compaction summary with a pending follow-up
    if (!isCompactionSummaryMetadata(latticeMeta) || !latticeMeta.pendingFollowUp) {
      return;
    }

    // Handle legacy formats: older persisted requests may have `mode` instead of `agentId`,
    // and `imageParts` instead of `fileParts`.
    const followUp = latticeMeta.pendingFollowUp as typeof latticeMeta.pendingFollowUp & {
      mode?: "exec" | "plan";
      imageParts?: FilePart[];
    };
    ...
    if (metadata) {
      options.latticeMetadata = metadata;
    }
  ...
  private async getPostCompactionAttachmentsIfNeeded(): Promise<PostCompactionAttachment[] | null> {
    ...
    if (pendingDiffs !== null) {
      ...
      // Load exclusions and persistent TODO state (local minion session data)
      ...
      const runtime = createRuntimeForMinion(metadataResult.data);
  ...
  private async materializeFileAtMentionsSnapshot(
    messageText: string
  ): Promise<{ snapshotMessage: LatticeMessage; materializedTokens: string[] } | null> {
    // Guard for test mocks that may not implement getMinionMetadata
    if (typeof this.aiService.getMinionMetadata !== "function") {
      return null;
    }
    ...
    const metadataResult = await this.aiService.getMinionMetadata(this.minionId);
    if (!metadataResult.success) {
      log.debug("Cannot materialize @file mentions: minion metadata not found", {
        minionId: this.minionId,
      });
      return null;
    }
    ...
    const snapshotMessage = createLatticeMessage(snapshotId, "user", blocks, {
      timestamp: Date.now(),
      synthetic: true,
  ...
  private async materializeAgentSkillSnapshot(
    latticeMetadata: LatticeMessageMetadata | undefined,
    disableMinionAgents: boolean | undefined
  ): Promise<{ snapshotMessage: LatticeMessage } | null> {
    if (!latticeMetadata || latticeMetadata.type !== "agent-skill") {
      return null;
    }
    ...
    const parsedName = SkillNameSchema.safeParse(latticeMetadata.skillName);
    if (!parsedName.success) {
      throw new Error(`Invalid agent skill name: ${latticeMetadata.skillName}`);
    }

    const metadataResult = await this.aiService.getMinionMetadata(this.minionId);
    if (!metadataResult.success) {
      throw new Error("Cannot materialize agent skill: minion metadata not found");
    }
    ...
    // When minion agents are disabled, resolve skills from the project path instead of
    // the worktree so skill invocation uses the same precedence/discovery root as the UI.
    const skillDiscoveryPath = disableMinionAgents ? metadata.projectPath : minionPath;
    ...
    const historyResult = await this.historyService.getLastMessages(this.minionId, 5);
    if (historyResult.success) {
    ...
    const snapshotMessage = createLatticeMessage(snapshotId, "user", snapshotText, {
      timestamp: Date.now(),
      synthetic: true,
      agentSkillSnapshot: {
  ...
  private async loadExcludedItems(): Promise<Set<string>> {
    const exclusionsPath = path.join(
      this.config.getSessionDir(this.minionId),
      "exclusions.json"
    );
    try {
  ...
  private async loadTodoListAttachment(
    ...
    const todoPath = path.join(this.config.getSessionDir(this.minionId), "todos.json");
