...
export class AgentSession {
  ...
  async ensureMetadata(args: {
    ...
    if (isUnderSrcBaseDir) {
      // Standard worktree mode: workspace is under ~/.lattice/src/project/branch
  ...
  async sendMessage(
    ...
    const typedLatticeMetadata = options?.latticeMetadata as LatticeFrontendMetadata | undefined;
    ...
    // Add type: "message" for discriminated union (createLatticeMessage doesn't add it)
    ...
    try {
      ...
      // Supports both new `followUpContent` and legacy `continueMessage` for backwards compatibility
      if (isCompactionRequest && typedLatticeMetadata && options) {
        const compactionMeta = typedLatticeMetadata as CompactionRequestMetadata;
        ...
        if (rawFollowUp) {
          ...
          const { finalText, metadata } = prepareUserMessageForSend(
            ...
            followUpContent?.latticeMetadata ?? legacyContinueMessage?.latticeMetadata
          );
          ...
          const sanitizedOptions: Omit<
            SendMessageOptions,
            "latticeMetadata" | "mode" | "editMessageId" | "fileParts" | "maxOutputTokens"
          > & { fileParts?: FilePart[]; latticeMetadata?: LatticeFrontendMetadata } = {
            model: followUpModel,
          ...
          if (metadata) {
            sanitizedOptions.latticeMetadata = metadata;
          }
  ...
  private resolveCompactionRequest(
    ...
    for (let index = history.length - 1; index >= 0; index -= 1) {
      ...
      if (!isCompactionRequestMetadata(message.metadata?.latticeMetadata)) {
        return undefined;
      }
      return {
        id: message.id,
  ...
  private async materializeAgentSkillSnapshot(
    latticeMetadata: LatticeFrontendMetadata | undefined,
