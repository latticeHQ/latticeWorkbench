/**
 * OAuth service for Lattice Governor enrollment.
 *
 * - Takes a user-provided governor origin
 * - Persists credentials to config.json (latticeGovernorUrl + latticeGovernorToken)
 */
...
} from "@/common/constants/latticeGovernorOAuth";
...
export class LatticeGovernorOauthService {
  ...
  async startDesktopFlow(input: {
    ...
    void (async () => {
      ...
      if (callbackOrDone.success) {
        ...
        result = Err(`Lattice Governor OAuth error: ${callbackOrDone.error}`);
      }
    ...
    log.debug(
      `Lattice Governor OAuth desktop flow started (flowId=${flowId}, origin=${governorOrigin})`
    );
  ...
  async cancelDesktopFlow(flowId: string): Promise<void> {
    ...
    log.debug(`Lattice Governor OAuth desktop flow cancelled (flowId=${flowId})`);
  ...
  startServerFlow(input: {
    ...
    log.debug(`Lattice Governor OAuth server flow started (state=${state}, origin=${governorOrigin})`);
  ...
  private async handleCallbackAndExchange(input: {
    ...
    if (input.error) {
      const message = input.errorDescription
        ...
        : input.error;
      return Err(`Lattice Governor OAuth error: ${message}`);
    }
    ...
    try {
      await this.config.editConfig((config) => ({
        ...config,
        latticeGovernorUrl: input.governorOrigin,
        latticeGovernorToken: tokenResult.data,
      }));
    } catch (error) {
      const message = getErrorMessage(error);
    ...
    log.debug(`Lattice Governor OAuth exchange completed (state=${input.state})`);
  ...
  private async exchangeCodeForToken(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const prefix = `Lattice Governor exchange failed (${response.status})`;
      ...
      if (!token) {
        return Err("Lattice Governor exchange response missing access_token");
      }

      return Ok(token);
    } catch (error) {
      const message = getErrorMessage(error);
      return Err(`Lattice Governor exchange failed: ${message}`);
    }
  }
}
