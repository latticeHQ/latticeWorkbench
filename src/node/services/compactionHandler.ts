...
function getNextCompactionEpoch(messages: LatticeMessage[]): number {
  let epochCursor = 0;
...
export class CompactionHandler {
  ...
  private async loadPersistedPendingStateIfNeeded(): Promise<void> {
    ...
    try {
      ...
      log.warn("Invalid post-compaction state JSON; ignoring", { minionId: this.minionId });
    ...
    if (!state) {
      log.warn("Invalid post-compaction state schema; ignoring", { minionId: this.minionId });
      await this.deletePersistedPendingStateBestEffort();
      return;
    }
  ...
  async handleCompletion(event: StreamEndEvent): Promise<boolean> {
    ...
    const historyResult = await this.historyService.getLastMessages(this.minionId, 10);
    ...
    const latticeMeta = lastUserMsg?.metadata?.latticeMetadata;
    const isCompaction = latticeMeta?.type === "compaction-request";
    ...
    const pendingFollowUp = getCompactionFollowUpContent(latticeMeta);
  ...
  private sanitizeCompactionStreamEndEvent(event: StreamEndEvent): StreamEndEvent {
    ...
    // usage meter shows "near empty" after minion switches instead of vanishing.
  ...
  private async performCompaction(
    ...
    const deletePartialResult = await this.historyService.deletePartial(this.minionId);
    ...
    // For idle compaction, preserve the original recency timestamp so the minion
    ...
    //
    // The summary's latticeMetadata stores the pending follow-up (if any) for crash-safe dispatch.
    ...
    const summaryLatticeMetadata: CompactionSummaryMetadata = {
    ...
    const summaryMessage = createLatticeMessage(
      ...
      summary,
      {
        ...
        ...(postCompactionContextEstimate && { contextUsage: postCompactionContextEstimate }),
        latticeMetadata: summaryLatticeMetadata,
      }
    );
    ...
    const persistenceResult = persistedStreamSummary
      ? await this.historyService.updateHistory(this.minionId, summaryMessage)
      : await this.historyService.appendToHistory(this.minionId, summaryMessage);
   ...
   */
  private emitChatEvent(message: MinionChatMessage): void {
