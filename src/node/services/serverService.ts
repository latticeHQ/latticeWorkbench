...
import { buildLatticeMdnsServiceOptions, MdnsAdvertiserService } from "./mdnsAdvertiserService";
...
export interface StartServerOptions {
  /** Path to lattice home directory (for lockfile) */
  latticeHome: string;
...
export class ServerService {
  ...
  async startServer(options: StartServerOptions): Promise<ServerInfo> {
    ...
    const lockfile = new ServerLockfile(options.latticeHome);

    // Check for existing server (another process)
    const existing = await lockfile.read();
    if (existing) {
      throw new Error(
        `Another lattice server is already running at ${existing.baseUrl} (PID: ${existing.pid})`
      );
    }
    ...
    if (mdnsAdvertisementEnabled !== false && !isLoopbackHost(bindHost)) {
      ...
      try {
        ...
        log.warn("Failed to advertise lattice API server via mDNS:", err);
      }
