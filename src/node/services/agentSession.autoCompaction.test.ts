...
import type { ProvidersConfigMap, MinionChatMessage } from "@/common/orpc/types";
...
describe("AgentSession on-send auto-compaction snapshot deferral", () => {
  ...
  test("does not persist or emit snapshots before forced on-send compaction", async () => {
    const minionId = "ws-auto-compaction-snapshot-deferral";

    const { historyService, cleanup } = await createTestHistoryService();
    historyCleanup = cleanup;

    const aiEmitter = new EventEmitter();
    const streamMessage = mock((_history: LatticeMessage[]) => Promise.resolve(Ok(undefined)));
    ...
    const syntheticSnapshot = createLatticeMessage(
      "file-snapshot-1",
      "user",
    ...
    const persistedCompactionMessage = historyResult.data.find(
      (message) => message.metadata?.latticeMetadata?.type === "compaction-request"
    );
    expect(persistedCompactionMessage).toBeDefined();
    expect(persistedCompactionMessage?.metadata?.disableMinionAgents).toBe(true);
  ...
  test("triggers on-send compaction at threshold even before force buffer", async () => {
    const minionId = "ws-auto-compaction-on-send-threshold";
    ...
    const firstRequest = streamRequests[0] as { messages?: LatticeMessage[] } | undefined;
  ...
  test("uses preferred compaction model for on-send auto-compaction requests", async () => {
    const minionId = "ws-auto-compaction-preferred-model";
    ...
    expect(compactionRequestMessage?.metadata?.latticeMetadata?.requestedModel).toBe(
  ...
  test("threads providers config into pre-send and mid-stream compaction checks", async () => {
    const minionId = "ws-auto-compaction-providers-config";
  ...
  test("seeds on-send compaction usage from the active compaction epoch only", async () => {
    const minionId = "ws-auto-compaction-seed-active-epoch";
    ...
    const appendOldUser = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-old-before-boundary", "user", "old prompt", {
    ...
    const appendOldAssistant = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("assistant-old-before-boundary", "assistant", "old reply", {
    ...
    const appendBoundary = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("assistant-compaction-boundary", "assistant", "compacted summary", {
    ...
    const appendCurrentEpochUser = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-after-boundary", "user", "fresh prompt after compaction", {
  ...
  test("surfaces nested dispatch failures after mid-stream compaction interrupt", async () => {
    const minionId = "ws-auto-compaction-mid-stream-dispatch-failure";
    ...
    const stopStream = mock((_minionId: string) => {
    ...
    const streamError = events.find(
      (event): event is Extract<MinionChatMessage, { type: "stream-error" }> =>
  ...
  test("hides default follow-up sentinel in mid-stream auto-compaction prompts", async () => {
    const minionId = "ws-auto-compaction-mid-stream-sentinel";

    const { historyService, cleanup } = await createTestHistoryService();
    historyCleanup = cleanup;

    const aiEmitter = new EventEmitter();
    const streamHistories: LatticeMessage[][] = [];
    let streamCallCount = 0;
    const streamMessage = mock((request: unknown) => {
      const requestMessages =
        typeof request === "object" && request !== null && "messages" in request
          ...
          : undefined;
      streamHistories.push(Array.isArray(requestMessages) ? (requestMessages as LatticeMessage[]) : []);
      streamCallCount += 1;
    ...
    const compactionRequestMessage = [...compactionHistory]
      .reverse()
      .find((message) => message.metadata?.latticeMetadata?.type === "compaction-request");
