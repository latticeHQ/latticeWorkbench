...
function renderLatticeFileBlock(options: {
  filePath: string;
  rangeLabel: string;
  content: string;
  truncated: boolean;
  ...
  return (
    `<lattice-file path="${options.filePath}" range="${options.rangeLabel}"${truncatedAttr}>\n` +
    `${fence}\n` +
    `${options.content}\n` +
    `\`\`\`\n` +
    `</lattice-file>`
...
export interface MaterializedFileMention {
  ...
  resolvedPath: string;
  /** The rendered <lattice-file> block. */
  block: string;
 ...
 *
 * This reads files and produces stable <lattice-file> blocks that can be persisted to history.
...
export async function materializeFileAtMentions(
  ...
  for (const mention of mentions) {
    ...
    const header = renderLatticeFileBlock({
    ...
    const block = renderLatticeFileBlock({
      filePath: displayPath,
