...
import type { MinionChatMessage, SendMessageOptions } from "@/common/orpc/types";
...
async function createSessionBundle(minionId: string): Promise<SessionBundle> {
...
describe("AgentSession startup auto-retry recovery", () => {
  ...
  test("schedules startup auto-retry for interrupted user tail", async () => {
    const minionId = "startup-retry-user-tail";
    const { session, historyService, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Hello from interrupted turn", {
        timestamp: Date.now(),
    ...
    const appendSnapshotResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("snapshot-1", "user", "<snapshot>", {
        timestamp: Date.now(),
        synthetic: true,
    ...
    expect(retryOptions.agentId).toBe(MINION_DEFAULTS.agentId);
    expect(retryOptions.toolPolicy).toEqual([{ regex_match: ".*", action: "disable" }]);
    expect(retryOptions.disableMinionAgents).toBe(true);

    session.dispose();
  });

  test("re-runs startup auto-retry check after busy startup state clears", async () => {
    const minionId = "startup-retry-busy-rerun";
    const { session, historyService, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted while startup was busy", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
  ...
  test("re-runs startup auto-retry check after transient history read failures", async () => {
    const minionId = "startup-retry-history-read-rerun";
    const { session, historyService, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted while history read failed", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
  ...
  test("backs off reruns after repeated startup history read failures", async () => {
    const minionId = "startup-retry-history-read-backoff";
    const { session, historyService, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted while history is unavailable", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
  ...
  test("waits for AI streaming to settle before rerunning deferred startup checks", async () => {
    const minionId = "startup-retry-wait-stream-settle";
    const { historyService, config, cleanup } = await createTestHistoryService();
    cleanups.push(cleanup);
    ...
    const aiService = Object.assign(aiEmitter, {
      isStreaming: mock((_minionId: string) => aiStreaming),
  ...
  test("restores persisted retry send options for startup auto-retry", async () => {
    const minionId = "startup-retry-preserve-options";
    const { session, historyService, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted with custom send options", {
        timestamp: Date.now(),
        retrySendOptions: {
          model: "anthropic:claude-sonnet-4-5",
          agentId: "exec",
  ...
  test("replays pending auto-retry schedule during reconnect catch-up", async () => {
    const minionId = "startup-retry-replay-snapshot";
    const { session, historyService, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted before reconnect", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
    ...
    const replayEvents: MinionChatMessage[] = [];
  ...
  test("respects persisted auto-retry opt-out across restart", async () => {
    const minionId = "startup-retry-opt-out";
    const {
      ...
      backgroundProcessManager,
      cleanup,
    } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted before restart", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
  ...
  test("respects legacy auto-retry opt-out hint when backend preference is missing", async () => {
    const minionId = "startup-retry-legacy-opt-out";
    const { session, historyService, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);

    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted before migration", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
  ...
  test("does not persist temporary auto-retry enable across restart", async () => {
    const minionId = "startup-retry-temporary-enable";
    const {
      session: firstSession,
      config,
      historyService,
  ...
  test("does not reschedule startup retries after persisted non-retryable failure", async () => {
    const minionId = "startup-retry-non-retryable";
    const {
      session: firstSession,
      config,
      historyService,
    ...
    const appendResult = await historyService.appendToHistory(
      minionId,
      createLatticeMessage("user-1", "user", "Interrupted prompt", {
        timestamp: Date.now(),
      })
    );
    expect(appendResult.success).toBe(true);
  ...
  test("clears persisted startup abandon state once retry resumes successfully", async () => {
    const minionId = "startup-retry-clear-abandon-on-resume";
    const { session, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);
  ...
  test("reschedules retry when resumeStream defers without starting a stream", async () => {
    const minionId = "startup-retry-resume-deferred";
    const { session, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);
  ...
  test("does not re-process retry failures already handled by resumeStream", async () => {
    const minionId = "startup-retry-no-double-process-failure";
    const { session, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);
  ...
  test("handles unprocessed resume failures by scheduling the next retry", async () => {
    const minionId = "startup-retry-process-unhandled-failure";
    const { session, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);
  ...
  test("persists startup abandon marker for pre-stream user aborts", async () => {
    const minionId = "startup-retry-pre-stream-abort";
    const { historyService, config, cleanup } = await createTestHistoryService();
    cleanups.push(cleanup);
  ...
  test("skips persisting startup abandon marker for non-user abort reasons", async () => {
    const minionId = "startup-retry-system-abort-skip";
    const { session, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);
  ...
  test("does not schedule startup auto-retry while ask_user_question is waiting", async () => {
    const minionId = "startup-retry-ask-user";
    const { session, historyService, events, cleanup } = await createSessionBundle(minionId);
    cleanups.push(cleanup);
