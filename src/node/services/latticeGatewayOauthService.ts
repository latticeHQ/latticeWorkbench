...
import {
  buildAuthorizeUrl,
  buildExchangeBody,
  LATTICE_GATEWAY_EXCHANGE_URL,
} from "@/common/constants/latticeGatewayOAuth";
...
export class LatticeGatewayOauthService {
  ...
  > {
    ...
    log.debug(`Lattice Gateway OAuth desktop flow started (flowId=${flowId})`);
  ...
  async cancelDesktopFlow(flowId: string): Promise<void> {
    ...
    log.debug(`Lattice Gateway OAuth desktop flow cancelled (flowId=${flowId})`);
  ...
  startServerFlow(input: { redirectUri: string }): { authorizeUrl: string; state: string } {
    ...
    log.debug(`Lattice Gateway OAuth server flow started (state=${state})`);
  ...
  private async handleDesktopCallback(input: {
    ...
    log.debug(`Lattice Gateway OAuth callback received (flowId=${input.flowId})`);
...
<html lang="en">
  ...
  private async handleCallbackAndExchange(input: {
    ...
    if (input.error) {
      const message = input.errorDescription
        ...
        : input.error;
      return Err(`Lattice Gateway OAuth error: ${message}`);
    }
    ...
    log.debug(`Lattice Gateway OAuth exchange completed (state=${input.state})`);
  ...
  private async exchangeCodeForToken(code: string): Promise<Result<string, string>> {
    try {
      const response = await fetch(LATTICE_GATEWAY_EXCHANGE_URL, {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        const prefix = `Lattice Gateway exchange failed (${response.status})`;
      ...
      if (!token) {
        return Err("Lattice Gateway exchange response missing access_token");
      }

      return Ok(token);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      return Err(`Lattice Gateway exchange failed: ${message}`);
    }
  }
