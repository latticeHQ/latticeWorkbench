...
interface RebuildAllData {
  sessionsDir: string;
  minionMetaById: Record<string, IngestMinionMeta>;
}
 ...
 * parser as the rest of the app (handles malformed
...
export class AnalyticsService {
  ...
  private buildRebuildMinionMetaById(): Record<string, IngestMinionMeta> {
    const configSnapshot = this.config.loadConfigOrDefault();
    const minionMetaById: Record<string, IngestMinionMeta> = {};
    ...
    for (const [projectPath, projectConfig] of configSnapshot.projects) {
      ...
      for (const minion of projectConfig.minions) {
        const minionId = toOptionalNonEmptyString(minion.id);
        if (!minionId) {
          continue;
        }

        if (minionMetaById[minionId]) {
          log.warn(
            "[AnalyticsService] Duplicate minion ID in config while building rebuild metadata",
            {
              minionId,
        ...
        minionMetaById[minionId] = {
          projectPath: normalizedProjectPath,
          projectName,
          minionName: toOptionalNonEmptyString(minion.name),
          parentMinionId: toOptionalNonEmptyString(minion.parentMinionId),
        };
      }
    }

    return minionMetaById;
  }
  ...
  private buildRebuildAllData(): RebuildAllData {
    ...
    return {
      ...
      minionMetaById: this.buildRebuildMinionMetaById(),
    };
  }
  ...
  private async startWorker(): Promise<void> {
    ...
    // Backfill existing minion history when analytics initialization is
    // missing or appears partial (for example, when any session minion lacks
    // a matching watermark row, even if stale watermark rows keep counts equal).
    // Once every session minion has a watermark row, routine worker restarts
  ...
  async rebuildAll(): Promise<{ success: boolean; minionsIngested: number }> {
    await this.ensureWorker();
    const result = await this.dispatch<RebuildAllResult>("rebuildAll", this.buildRebuildAllData());

    return {
      success: true,
      minionsIngested: result.minionsIngested,
    };
  }
  ...
  clearMinion(minionId: string): void {
    if (minionId.trim().length === 0) {
      log.warn("[AnalyticsService] Skipping minion clear due to missing minionId", {
        minionId,
      });
      return;
    }
    ...
    const runClear = () => {
      this.ensureWorker()
        .then(() => this.dispatch<void>("clearMinion", { minionId }))
        .catch((error) => {
          log.warn("[AnalyticsService] Failed to clear minion analytics state", {
            minionId,
            error: getErrorMessage(error),
          });
        });
    ...
    // Minion-removal hooks can fire before analytics is ever opened in this
    ...
    // removed immediately after minion deletion.
  ...
  ingestMinion(minionId: string, sessionDir: string, meta: IngestMinionMeta = {}): void {
    if (minionId.trim().length === 0 || sessionDir.trim().length === 0) {
      log.warn("[AnalyticsService] Skipping ingest due to missing minion information", {
        minionId,
        sessionDir,
      });
      return;
    ...
    this.ensureWorker()
      .then(() => this.dispatch("ingest", { minionId, sessionDir, meta }))
      .catch((error) => {
        log.warn("[AnalyticsService] Failed to ingest minion", {
          minionId,
          error: getErrorMessage(error),
        });
      });
