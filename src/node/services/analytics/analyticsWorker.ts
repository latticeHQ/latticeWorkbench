...
import { CHAT_FILE_NAME, clearMinionAnalyticsState, ingestMinion, rebuildAll } from "./etl";
...
interface RebuildAllData {
  sessionsDir: string;
  minionMetaById?: Record<string, MinionMeta>;
}
...
CREATE TABLE IF NOT EXISTS events (
  minion_id VARCHAR NOT NULL,
  project_path VARCHAR,
  project_name VARCHAR,
  minion_name VARCHAR,
  parent_minion_id VARCHAR,
  agent_id VARCHAR,
  timestamp BIGINT,
  date DATE,
  model VARCHAR,
...
async function handleIngest(data: IngestData): Promise<void> {
  assert(data.minionId.trim().length > 0, "ingest requires minionId");
  assert(data.sessionDir.trim().length > 0, "ingest requires sessionDir");

  await ingestMinion(getConn(), data.minionId, data.sessionDir, data.meta ?? {});
}

async function handleRebuildAll(data: RebuildAllData): Promise<{ minionsIngested: number }> {
  assert(data.sessionsDir.trim().length > 0, "rebuildAll requires sessionsDir");
  if (data.minionMetaById != null) {
    assert(
      isRecord(data.minionMetaById) && !Array.isArray(data.minionMetaById),
      "rebuildAll minionMetaById must be an object when provided"
    );
  }

  return rebuildAll(getConn(), data.sessionsDir, data.minionMetaById ?? {});
}

async function handleClearMinion(data: ClearMinionData): Promise<void> {
  assert(data.minionId.trim().length > 0, "clearMinion requires minionId");
  await clearMinionAnalyticsState(getConn(), data.minionId);
}
...
async function listSessionMinionIdsWithHistory(sessionsDir: string): Promise<string[]> {
  let entries: Dirent[];
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      if (chatStat.isFile()) {
        const minionId = parseNonEmptyString(entry.name);
        assert(
          minionId !== null,
          "needsBackfill expected session minion directory names to be non-empty"
        );
        sessionMinionIds.push(minionId);
      }
    } catch (error) {
      if (isRecord(error) && error.code === "ENOENT") {
        continue;
  ...
  return sessionMinionIds;
}

async function listWatermarkMinionIds(): Promise<Set<string>> {
  const result = await getConn().run("SELECT minion_id FROM ingest_watermarks");
  const rows = await result.getRowObjectsJS();

  const watermarkMinionIds = new Set<string>();
  for (const row of rows) {
    const minionId = parseNonEmptyString(row.minion_id);
    assert(
      minionId !== null,
      "needsBackfill expected ingest_watermarks rows to have non-empty minion_id"
    );
    watermarkMinionIds.add(minionId);
  }

  return watermarkMinionIds;
}
...
async function handleNeedsBackfill(data: NeedsBackfillData): Promise<{ needsBackfill: boolean }> {
  ...
  const sessionMinionIds = await listSessionMinionIdsWithHistory(data.sessionsDir);
  const sessionMinionCount = sessionMinionIds.length;
  const sessionMinionIdSet = new Set(sessionMinionIds);

  const watermarkMinionIds = await listWatermarkMinionIds();
  assert(
    watermarkMinionIds.size === watermarkCount,
    "needsBackfill expected watermark_count to match ingest_watermarks minion IDs"
  );

  const hasSessionMinionMissingWatermark = sessionMinionIds.some(
    (minionId) => !watermarkMinionIds.has(minionId)
  );
  const hasWatermarkMissingSessionMinion = [...watermarkMinionIds].some(
    (minionId) => !sessionMinionIdSet.has(minionId)
  );
  ...
  return {
    needsBackfill: shouldRunInitialBackfill({
      ...
      watermarkCount,
      sessionMinionCount,
      hasSessionMinionMissingWatermark,
      hasWatermarkMissingSessionMinion,
...
async function dispatchTask(taskName: string, data: unknown): Promise<unknown> {
  switch (taskName) {
    ...
    case "clearMinion":
      return handleClearMinion(data as ClearMinionData);
    case "query":
