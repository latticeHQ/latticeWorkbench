...
function getStoreFilePath(latticeHome: string): string {
  return path.join(latticeHome, "mcp-oauth.json");
}
...
describe("McpOauthService store", () => {
  ...
  beforeEach(async () => {
    latticeHome = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-oauth-home-"));
    projectPath = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-oauth-project-"));

    config = new Config(latticeHome);
  ...
  afterEach(async () => {
    ...
    await fs.rm(latticeHome, { recursive: true, force: true });
    await fs.rm(projectPath, { recursive: true, force: true });
  });

  async function readStoreFile(): Promise<unknown> {
    const raw = await fs.readFile(getStoreFilePath(latticeHome), "utf-8");
    return JSON.parse(raw) as unknown;
  }

  test("reading corrupt JSON store self-heals to empty", async () => {
    await fs.writeFile(getStoreFilePath(latticeHome), "{ definitely not valid json", "utf-8");
  ...
  test("migrates v1 store to v2 (dedupes by updatedAtMs)", async () => {
    ...
    try {
      ...
      await fs.writeFile(getStoreFilePath(latticeHome), JSON.stringify(v1Store), "utf-8");
  ...
  test("set/get/clear works via hasAuthTokens + logout", async () => {
    ...
    await fs.writeFile(getStoreFilePath(latticeHome), JSON.stringify(populatedStore), "utf-8");
...
describe("McpOauthService.startDesktopFlow", () => {
  ...
  beforeEach(async () => {
    latticeHome = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-oauth-flow-home-"));
  ...
  test("preserves trailing slashes for OAuth discovery under a base path", async () => {
    ...
    try {
      ...
      const storeRaw = await fs.readFile(getStoreFilePath(latticeHome), "utf-8");
