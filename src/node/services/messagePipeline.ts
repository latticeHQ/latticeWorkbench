/**
 * Message pipeline: transforms LatticeMessages into provider-ready ModelMessages.
 *
...
export interface PrepareMessagesOptions {
  ...
  messagesWithSentinel: LatticeMessage[];
  ...
  runtime: Runtime;
  /** Minion path for file resolution. */
  minionPath: string;
  ...
  /** Minion ID (used only for debug logging). */
  minionId: string;
}
 ...
 *
 * Transforms pre-filtered `LatticeMessage[]` into provider-ready `ModelMessage[]` by:
...
export async function prepareMessagesForProvider(
  ...
  // --- LatticeMessage-level transforms ---
  ...
  log.debug_obj(`${minionId}/2a_redacted_messages.json`, redactedForProvider);
  ...
  log.debug_obj(`${minionId}/2b_sanitized_messages.json`, sanitizedMessages);
  ...
  const modelMessages = sanitizeAssistantModelMessages(rawModelMessages, minionId);

  log.debug_obj(`${minionId}/2_model_messages.json`, modelMessages);
  ...
  log.debug_obj(`${minionId}/3_final_messages.json`, finalMessages);
...
export function sanitizeAssistantModelMessages(
  ...
  if (result.length < messages.length) {
    log.debug(
      `Self-healing: Filtered ${messages.length - result.length} empty ModelMessage(s)${minionId ? ` [${minionId}]` : ""}`
    );
  }
