...
describe("MCPConfigService", () => {
  ...
  test("listServers merges repo overrides on top of global (override wins by name)", async () => {
    ...
    await fs.mkdir(path.join(projectPath, ".lattice"), { recursive: true });
    await fs.writeFile(
      path.join(projectPath, ".lattice", "mcp.jsonc"),
...
describe("Minion MCP overrides filtering", () => {
  let tempDir: string;
  let configService: MCPConfigService;
  let serverManager: MCPServerManager;
  ...
  test("listServers with empty overrides returns all enabled servers", async () => {
    const overrides: MinionMCPOverrides = {};
  ...
  test("project-disabled and minion-disabled work together", async () => {
    // Disable server-a at project level
    await configService.setServerEnabled("server-a", false);

    // Disable server-b at minion level
    const overrides: MinionMCPOverrides = {
