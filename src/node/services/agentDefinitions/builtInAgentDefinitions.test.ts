...
describe("built-in agent definitions", () => {
  ...
  test("exec sidekick append_prompt warns about missing task brief context", () => {
    const pkgs = getBuiltInAgentDefinitions();
    const byId = new Map(pkgs.map((pkg) => [pkg.id, pkg] as const));
    ...
    const appendPrompt = exec?.frontmatter.sidekick?.append_prompt;
  ...
  test("includes orchestrator with expected flags", () => {
    ...
    expect(orchestrator?.frontmatter.sidekick?.runnable).toBe(false);
  });
