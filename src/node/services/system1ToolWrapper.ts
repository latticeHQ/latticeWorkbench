...
export function wrapToolsWithSystem1(opts: System1WrapOptions): Record<string, Tool> {
  ...
  > => {
    ...
    const created = await opts.createModel(system1Ctx.modelString, opts.latticeProviderOptions);
    if (!created.success) {
  ...
  const wrappedTools: Record<string, Tool> = {
    ...tools,
    bash: wrapBashTool(baseBashTool, bashExecuteFn, maybeFilter, opts.minionId),
  };
...
interface System1ModelContext {
  /** Raw model string passed to createModel. */
  modelString: string;
...
function buildSystem1ModelContext(opts: System1WrapOptions): System1ModelContext {
  ...
  const effectiveModelForThinking = raw || opts.modelString;
  ...
  try {
    ...
    try {
      ...
      if (keepRangesResult) {
        ...
        // Track System 1 token usage in minion costs.
        // Pass providerMetadata so cache tokens and costsIncluded are honored.
        if (keepRangesResult.usage && opts.sessionUsageService) {
          const normalizedModel = system1.modelString;
