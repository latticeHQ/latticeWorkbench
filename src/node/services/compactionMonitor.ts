...
export class CompactionMonitor {
  ...
  constructor(
    ...
    assert(typeof minionId === "string", "CompactionMonitor requires a string minionId");
    assert(minionId.trim().length > 0, "CompactionMonitor requires a non-empty minionId");
    assert(
  ...
  checkMidStream(params: CheckMidStreamParams): boolean {
    ...
    assert(
      usageTokens >= 0,
      `CompactionMonitor(${this.minionId}): usage tokens must be non-negative`
    );
  ...
  setThreshold(threshold: number): void {
    assert(
      ...
      `CompactionMonitor(${this.minionId}): threshold must be finite`
    );
    assert(
      threshold > 0 && threshold <= 1,
      `CompactionMonitor(${this.minionId}): invalid threshold ${threshold}`
    );
