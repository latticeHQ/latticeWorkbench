...
import { buildLatticeMdnsServiceOptions, LATTICE_MDNS_SERVICE_TYPE } from "./mdnsAdvertiserService";

describe("buildLatticeMdnsServiceOptions", () => {
  test("0.0.0.0 disables IPv6 and avoids advertising loopback addresses", () => {
    ...
    const serviceOptions = buildLatticeMdnsServiceOptions({
      bindHost: "0.0.0.0",
      port: 3000,
      instanceName: "lattice-test",
      version: "0.0.0-test",
      authRequired: true,
      networkInterfaces,
    });

    expect(serviceOptions.type).toBe(LATTICE_MDNS_SERVICE_TYPE);
  ...
  test("sanitizes dots in instanceName so DNS-SD clients can browse/resolve", () => {
    const serviceOptions = buildLatticeMdnsServiceOptions({
      ...
      port: 3000,
      instanceName: "lattice-host.home",
      version: "0.0.0-test",
      authRequired: false,
    });

    expect(serviceOptions.name).toBe("lattice-host-home");
  });
