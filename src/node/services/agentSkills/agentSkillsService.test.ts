...
describe("agentSkillsService", () => {
  test("project skills override global skills", async () => {
    ...
    // Note: deep-review skill is a project skill in the Lattice repo, not a built-in
    expect(skills.map((s) => s.name)).toEqual(["bar", "foo", "init", "lattice-docs"]);
  ...
  test("scans universal root after lattice global root", async () => {
    using project = new DisposableTempDir("agent-skills-project");
    using global = new DisposableTempDir("agent-skills-global");
  ...
  test("discoverAgentSkillsDiagnostics surfaces invalid skills", async () => {
    ...
    expect(diagnostics.skills.map((s) => s.name)).toEqual(["foo", "init", "lattice-docs"]);
  ...
  test("discovers skill directory via relative symlink", async () => {
    ...
    //   <project>/.lattice/skills/kalshi-docs -> ../../.agents/skills/kalshi-docs  (relative symlink)
    ...
    const latticeSkillsRoot = path.join(projectRoot, ".lattice", "skills");
    await fs.mkdir(externalSkillsDir, { recursive: true });
    await fs.mkdir(latticeSkillsRoot, { recursive: true });

    // Write the real skill outside .lattice/skills/
    ...
    await fs.symlink(
      ...
      path.join(latticeSkillsRoot, "kalshi-docs")
    );

    const roots = { projectRoot: latticeSkillsRoot, globalRoot: "/nonexistent" };
