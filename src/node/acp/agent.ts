...
import type { OnChatMode, SendMessageOptions, MinionChatMessage } from "@/common/orpc/types";
...
import { forkSessionFromMinion } from "./experimental/sessionFork";
import { loadSessionFromMinion } from "./experimental/sessionResume";
...
const ACP_PROMPT_CORRELATION_LATTICE_METADATA_KEY = "acpPromptId";
const ACP_DELEGATED_TOOLS_LATTICE_METADATA_KEY = "acpDelegatedTools";
...
interface NewSessionMinionLifecycle {
...
interface TurnCompletion {
  ...
  startedAtMs: number;
  /**
   * Wall-clock when minion.sendMessage was dispatched for this turn.
...
>;
type MinionActivityById = Awaited<
  ReturnType<ServerConnection["client"]["minion"]["activity"]["list"]>
>;

export class LatticeAgent implements Agent {
  ...
  /**
   * Tracks minions created by ACP session/new so disconnect cleanup can remove
   * untouched placeholders (no prompts/messages sent).
   */
  private readonly newSessionMinionLifecycleById = new Map<
    string,
    NewSessionMinionLifecycle
  >();
  ...
  private readonly chatIteratorsBySessionId = new Map<
    string,
    AsyncIterator<MinionChatMessage>
  >();
  ...
  constructor(
    ...
    options?: LatticeAgentOptions
  ) {
    assert(connection != null, "LatticeAgent: connection is required");
    assert(server != null, "LatticeAgent: server connection is required");
    ...
    assert(
      ...
      "LatticeAgent: disconnectCleanupMaxWaitMs must be a finite non-negative number"
    );
    ...
    assert(
      ...
      "LatticeAgent: sessionIdleTtlMs must be a finite positive number"
    );
    ...
    assert(
      ...
      "LatticeAgent: maxTrackedSessions must be a positive integer"
    );
    ...
    assert(
      ...
      "LatticeAgent: turnCorrelationTimeoutMs must be a finite positive number"
    );
  ...
  initialize(params: InitializeRequest): Promise<InitializeResponse> {
    ...
    this.connection.signal.addEventListener(
      "abort",
      () => {
        const disconnectError = new Error("Lattice ACP connection closed");
        ...
        this.disconnectCleanupPromise = (async () => {
          ...
          // session cleanup can remove minion lookups that stream interruption
          ...
          await Promise.all([
            this.cleanupNewSessionMinionsOnDisconnect(),
            this.cleanupTrackedSessionsOnDisconnect(),
          ]);
        })().catch((cleanupError) => {
          console.error("[acp] Failed during disconnect minion cleanup", cleanupError);
        });
      },
      { once: true }
    );
    ...
    return Promise.resolve({
      ...
      agentInfo: {
        name: "lattice",
        version: process.env.LATTICE_VERSION ?? "dev",
      },
      agentCapabilities: {
        loadSession: true,
        sessionCapabilities: {
  ...
  async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
    ...
    try {
      const meta = parseLatticeMeta(params._meta);
      ...
      // send `cwd`, not lattice-specific `_meta`), derive it from the project's git
      // repo.  Worktree/SSH runtimes require a trunk branch for minion creation.
      ...
      const createResult = await this.server.client.minion.create({
        projectPath,
      ...
      if (!createResult.success) {
        throw new Error(`newSession: minion.create failed: ${createResult.error}`);
      }

      const minion = createResult.metadata;
      const sessionId = minion.id;
      const minionId = minion.id;
      this.newSessionMinionLifecycleById.set(minionId, {
        hasPromptActivity: false,
      });

      const runtimeMode = runtimeModeFromConfig(minion.runtimeConfig);
      ...
      const agentId = meta.agentId ?? minion.agentId ?? DEFAULT_AGENT_ID;
      const aiSettings = await resolveAgentAiSettings(this.server.client, agentId, minionId);
      await this.persistAiSettings(minionId, agentId, aiSettings);
      ...
      await this.ensureChatSubscription(sessionId, minionId, ON_CHAT_MODE_FULL);

      const response = {
        sessionId,
        configOptions: await buildConfigOptions(this.server.client, minionId, {
          activeAgentId: agentId,
        }),
      };

      this.scheduleSessionCommandsRefresh(sessionId, minionId);
  ...
  async loadSession(params: LoadSessionRequest): Promise<LoadSessionResponse> {
    ...
    // in ACP session state, not persisted as the minion's active agent).
    const existingState = this.sessionStateById.get(params.sessionId);
    const resumed = await loadSessionFromMinion(params, {
      server: this.server,
    ...
    await this.ensureChatSubscription(resumed.sessionId, resumed.minionId, ON_CHAT_MODE_FULL);

    this.scheduleSessionCommandsRefresh(resumed.sessionId, resumed.minionId);
  ...
  async unstable_listSessions(params: ListSessionsRequest): Promise<ListSessionsResponse> {
    ...
    const [activeMinions, archivedMinions, minionActivity] = await Promise.all([
      this.server.client.minion.list({ archived: false }),
      this.server.client.minion.list({ archived: true }),
      this.server.client.minion.activity.list(),
    ]);

    const allMinions = dedupeMinionsById([...activeMinions, ...archivedMinions]);
    const filteredMinions =
      normalizedCwd != null
        ? allMinions.filter((minion) => minionMatchesCwd(minion, normalizedCwd))
        : allMinions;

    const sortedMinions = [...filteredMinions].sort((left, right) =>
      compareSessionRecency(left, right, minionActivity)
    );

    const page = sortedMinions.slice(offset, offset + SESSION_LIST_PAGE_SIZE);
    const nextOffset = offset + page.length;

    return {
      sessions: page.map((minion) => ({
        sessionId: minion.id,
        // Surface projectPath as cwd so session/list filtering matches editor cwd.
        cwd: minion.projectPath,
        title: minion.title ?? minion.name,
        updatedAt: toSessionUpdatedAt(minion, minionActivity),
      })),
      nextCursor: nextOffset < sortedMinions.length ? String(nextOffset) : undefined,
    };
  }
  ...
  async unstable_resumeSession(params: ResumeSessionRequest): Promise<ResumeSessionResponse> {
    ...
    const resumed = await loadSessionFromMinion(
      {
        sessionId,
        cwd,
    ...
    await this.ensureChatSubscription(resumed.sessionId, resumed.minionId, ON_CHAT_MODE_LIVE);
  ...
  async unstable_forkSession(params: ForkSessionRequest): Promise<ForkSessionResponse> {
    ...
    const forked = await forkSessionFromMinion(
      params,
      {
        server: this.server,
        sessionManager: this.sessionManager,
    ...
    await this.persistAiSettings(forked.minionId, forked.agentId, forked.aiSettings);

    this.sessionStateById.set(forked.sessionId, {
      minionId: forked.minionId,
    ...
    await this.ensureChatSubscription(forked.sessionId, forked.minionId, ON_CHAT_MODE_FULL);

    this.scheduleSessionCommandsRefresh(forked.sessionId, forked.minionId);
  ...
  async prompt(params: PromptRequest): Promise<PromptResponse> {
    ...
    this.touchSession(sessionId);
    const minionId = this.sessionManager.getMinionId(sessionId);
    ...
    return this.sendMinionMessageAndAwaitTurn({
      sessionId,
      minionId,
  ...
  async cancel(params: CancelNotification): Promise<void> {
    ...
    this.touchSession(sessionId);
    const minionId = this.sessionManager.getMinionId(sessionId);
    const interruptResult = await this.server.client.minion.interruptStream({ minionId });

    if (!interruptResult.success) {
      throw new Error(`cancel: minion.interruptStream failed: ${interruptResult.error}`);
    }
  ...
  authenticate(_params: AuthenticateRequest): Promise<AuthenticateResponse> {
    ...
    // Local lattice server connections do not currently require ACP-level auth.
    return Promise.resolve({});
  }

  private async sendMinionMessageAndAwaitTurn(args: {
    sessionId: string;
    minionId: string;
    message: string;
    ...
    assert(
      ...
      "sendMinionMessageAndAwaitTurn: sessionId required"
    );
    assert(
      args.minionId.trim().length > 0,
      "sendMinionMessageAndAwaitTurn: minionId required"
    );
    assert(args.message.trim().length > 0, "sendMinionMessageAndAwaitTurn: message required");

    this.markNewSessionMinionPromptActivity(args.minionId);
    ...
    try {
      ...
      const sendResult = await this.server.client.minion.sendMessage({
      ...
      if (!sendResult.success) {
        throw new Error(
          `prompt: minion.sendMessage failed: ${stringifyUnknown(sendResult.error)}`
        );
      }
      ...
      // minion.sendMessage / subscription failures can happen before stream
  ...
  private attachPromptCorrelationToSendOptions(
    ...
    const existingLatticeMetadata = isRecord(options.latticeMetadata) ? options.latticeMetadata : {};
    const latticeMetadata: Record<string, unknown> = {
      ...existingLatticeMetadata,
      [ACP_PROMPT_CORRELATION_LATTICE_METADATA_KEY]: promptCorrelationId,
    };

    if (delegatedToolNames.length > 0) {
      latticeMetadata[ACP_DELEGATED_TOOLS_LATTICE_METADATA_KEY] = [...delegatedToolNames];
    } else {
      delete latticeMetadata[ACP_DELEGATED_TOOLS_LATTICE_METADATA_KEY];
    }
  ...
  private async tryHandleSlashCommand(
    ...
    try {
      skillsByName = await this.getSessionSkills(sessionId, minionId);
    } catch (error) {
  ...
  private async handleSlashCommand(
    ...
    switch (parsedCommand.kind) {
      ...
      case "clear": {
        const clearResult = await this.server.client.minion.truncateHistory({
          minionId,
          percentage: 1.0,
        });
      ...
      case "truncate": {
        const truncateResult = await this.server.client.minion.truncateHistory({
          minionId,
      ...
      case "skill": {
        const options: SendMessageOptions = {
          ...
          thinkingLevel: sessionState.aiSettings.thinkingLevel,
          agentId: sessionState.agentId,
          latticeMetadata: buildAgentSkillMetadata({
      ...
      case "fork": {
        const forkResult = await this.server.client.minion.fork({
          sourceMinionId: minionId,
        });
        if (!forkResult.success) {
          return this.respondToCommand(
            sessionId,
            `Failed to clone minion: ${forkResult.error ?? "unknown error"}`
          );
        }

        const newMinionId = forkResult.metadata.id;
        let response = `Created forked minion \`${newMinionId}\`.`;

        if (parsedCommand.startMessage != null && parsedCommand.startMessage.trim().length > 0) {
          const startMessageResult = await this.server.client.minion.sendMessage({
          ...
          response += startMessageResult.success
            ? " Queued the optional start message in the forked minion."
      ...
      case "new": {
        const minionInfo = await this.server.client.minion.getInfo({ minionId });
        if (minionInfo == null) {
          return this.respondToCommand(
            sessionId,
            "Failed to summon minion: current minion metadata is unavailable."
          );
        }
        ...
        const createResult = await this.server.client.minion.create({
          projectPath: minionInfo.projectPath,
          branchName: parsedCommand.minionName,
          trunkBranch,
        ...
        if (!createResult.success) {
          return this.respondToCommand(
            sessionId,
            `Failed to summon minion: ${createResult.error ?? "unknown error"}`
          );
        }

        const newMinionId = createResult.metadata.id;
        let response = `Created minion \`${parsedCommand.minionName}\` (id: \`${newMinionId}\`).`;
        ...
        if (parsedCommand.startMessage != null && parsedCommand.startMessage.trim().length > 0) {
          ...
          response += startMessageResult.success
            ? " Queued the optional start message in the new minion."
            : ` Could not queue the optional start message: ${stringifyUnknown(startMessageResult.error)}`;
        }
  ...
  private markNewSessionMinionPromptActivity(minionId: string): void {
    const lifecycle = this.newSessionMinionLifecycleById.get(minionId);
    if (lifecycle == null) {
      return;
    }
  ...
  private async interruptActiveTurnStreamsOnDisconnect(
    ...
    for (const sessionId of sessionIds) {
      let minionId: string;
      try {
        minionId = this.sessionManager.getMinionId(sessionId);
      } catch {
        continue;
      }

      try {
        const interruptResult = await this.server.client.minion.interruptStream({
          minionId,
          options: {
            abandonPartial: true,
          },
  ...
  private async cleanupNewSessionMinionsOnDisconnect(): Promise<void> {
    const startedAt = Date.now();

    while (true) {
      const nextEntry = this.newSessionMinionLifecycleById.entries().next();
      if (nextEntry.done) {
        // Disconnect can race in-flight session/new requests that haven't yet
        // registered their minion lifecycle entry. Keep draining until all
      ...
      const [minionId, lifecycle] = nextEntry.value;
      this.newSessionMinionLifecycleById.delete(minionId);

      // If this ACP connection attempted any prompt in the minion, keep it.
      // Users can intentionally prepare a minion and disconnect mid-turn.
      if (lifecycle.hasPromptActivity) {
        continue;
      }

      try {
        const replay = await this.server.client.minion.getFullReplay({ minionId });
        if (!isMinionConversationEmpty(replay)) {
          continue;
        }

        const removeResult = await this.server.client.minion.remove({
          minionId,
          options: { force: false },
        });
        if (!removeResult.success) {
          console.error(
            `[acp] Failed to remove untouched ACP minion ${minionId} on disconnect: ${stringifyUnknown(removeResult.error)}`
          );
        }
      } catch (error) {
        console.error(
          `[acp] Failed to cleanup untouched ACP minion ${minionId} on disconnect`,
          error
        );
      }
    }
  ...
  private scheduleSessionCommandsRefresh(sessionId: string, minionId: string): void {
    ...
    setTimeout(() => {
      ...
      void this.refreshSessionCommands(sessionId, minionId);
    }, 0);
  }

  private async refreshSessionCommands(sessionId: string, minionId: string): Promise<void> {
    let advertisedSkills: AgentSkillDescriptor[];

    try {
      const skills = await this.server.client.agentSkills.list({ minionId });
  ...
  private async evictSessionState(sessionId: string, reason: string): Promise<void> {
    ...
    const minionId =
      this.sessionStateById.get(sessionId)?.minionId ??
      this.sessionManager.getAllSessions().get(sessionId)?.minionId;
  ...
  private async runChatSubscription(
    ...
    const chatStream = await this.server.client.minion.onChat({
      minionId,
      mode: onChatMode,
    });
    onConnected();
    ...
    const isTerminalStreamEvent = (event: MinionChatMessage): boolean =>
      event.type === "stream-end" ||
      event.type === "stream-abort" ||
      event.type === "stream-error" ||
      event.type === "error";

    const isDroppableUnderBackpressure = (event: MinionChatMessage): boolean => {
    ...
    const chatIterable: AsyncIterable<MinionChatMessage> = {
    ...
    const forwardingIterable: AsyncIterable<MinionChatMessage> = {
      async *[Symbol.asyncIterator](): AsyncGenerator<MinionChatMessage> {
  ...
  private handleStreamEvent(sessionId: string, event: MinionChatMessage): void {
  ...
  private async maybeDelegateToolCallToEditor(
    sessionId: string,
    event: Extract<MinionChatMessage, { type: "tool-call-start" }>
  ): Promise<void> {
    ...
    const answerResult = await this.server.client.minion.answerDelegatedToolCall({
      minionId,
   ...
   * in shared minions from cancelling/rejecting the current prompt.
   */
  ...
  private async refreshSessionState(sessionId: string): Promise<SessionState> {
    const minionId = this.sessionManager.getMinionId(sessionId);
    const minion = await this.server.client.minion.getInfo({ minionId });

    if (!minion) {
      throw new Error(`refreshSessionState: minion '${minionId}' was not found`);
    }

    const runtimeMode = runtimeModeFromConfig(minion.runtimeConfig);
    const existing = this.sessionStateById.get(sessionId);
    // Prefer the ACP session's agent selection over minion metadata.
    ...
    // minion.agentId value from the backend.
    const agentId = existing?.agentId ?? minion.agentId ?? DEFAULT_AGENT_ID;
    const aiSettings =
      minion.aiSettingsByAgent?.[agentId] ??
      minion.aiSettings ??
      (await resolveAgentAiSettings(this.server.client, agentId, minionId));
  ...
  private async persistAiSettings(
    ...
    if (agentId === "plan" || agentId === "exec") {
      const updateModeResult = await this.server.client.minion.updateModeAISettings({
        minionId,
        mode: agentId,
        aiSettings,
      });
      ...
      if (!updateModeResult.success) {
        throw new Error(`minion.updateModeAISettings failed: ${updateModeResult.error}`);
      }

      return;
    }

    const updateAgentResult = await this.server.client.minion.updateAgentAISettings({
      minionId,
      agentId,
      aiSettings,
    });
    ...
    if (!updateAgentResult.success) {
      throw new Error(`minion.updateAgentAISettings failed: ${updateAgentResult.error}`);
    }
  }
...
function normalizeOptionalPath(value: string | null | undefined): string | undefined {
  ...
  return normalizePathForMinionMatch(trimmed);
}

function normalizePathForMinionMatch(value: string): string {
...
function isMinionConversationEmpty(events: MinionChatMessage[]): boolean {
  return !events.some(
    (event) => event.type === "message" && (event.role === "assistant" || event.role === "user")
  );
}

function dedupeMinionsById(minions: MinionInfo[]): MinionInfo[] {
  const deduped = new Map<string, MinionInfo>();
  for (const minion of minions) {
    if (!deduped.has(minion.id)) {
      deduped.set(minion.id, minion);
    }
  }
  return Array.from(deduped.values());
}

function minionMatchesCwd(minion: MinionInfo, cwd: string): boolean {
  // Match both projectPath and concrete minion path so clients can filter by
  // either the original project root or the runtime-specific working directory.
  const normalizedProjectPath = normalizePathForMinionMatch(minion.projectPath);
  const normalizedMinionPath = normalizePathForMinionMatch(minion.namedMinionPath);
  return normalizedProjectPath === cwd || normalizedMinionPath === cwd;
}

function compareSessionRecency(
  left: MinionInfo,
  right: MinionInfo,
  activityByMinionId: MinionActivityById
): number {
  const leftRecency = toSessionRecencyTimestamp(left, activityByMinionId);
  const rightRecency = toSessionRecencyTimestamp(right, activityByMinionId);
...
function toSessionUpdatedAt(
  ...
  const recency = toSessionRecencyTimestamp(minion, activityByMinionId);
  if (recency > 0) {
    return new Date(recency).toISOString();
  }

  return minion.createdAt ?? null;
}
...
function toSessionRecencyTimestamp(
  ...
  const minionActivity = activityByMinionId[minion.id];
  if (minionActivity?.recency != null) {
    return minionActivity.recency;
  }

  if (minion.createdAt == null) {
    return 0;
  }

  const createdAtMs = Date.parse(minion.createdAt);
  return Number.isFinite(createdAtMs) ? createdAtMs : 0;
}

function parseLatticeMeta(rawMeta: MetaRecord | null | undefined): ParsedLatticeMeta {
  const source = getLatticeMetaSource(rawMeta);
...
function getLatticeMetaSource(rawMeta: MetaRecord | null | undefined): MetaRecord {
  if (!isRecord(rawMeta)) {
    return {};
  }

  const nestedLatticeMeta = rawMeta.lattice;
  if (isRecord(nestedLatticeMeta)) {
    return nestedLatticeMeta;
  }
...
/**
 * Convert ACP ContentBlock[] to LATTICE sendMessage arguments.
...
function parsePromptBlocks(blocks: ContentBlock[]): ParsedPrompt {
  ...
  for (const block of blocks) {
    ...
    if (block.type === "image") {
      // Convert ACP image (base64 data + mimeType) to a LATTICE file part.
