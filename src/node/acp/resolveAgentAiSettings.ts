...
function buildAgentsListInput(
  ...
  | { minionId: string; includeDisabled: boolean }
  | { projectPath: string; includeDisabled: boolean } {
  const trimmedMinionId = minionId?.trim();
  if (trimmedMinionId) {
    return { minionId: trimmedMinionId, includeDisabled: true };
  }

  // Fallback for callers that do not have minion context yet.
...
export async function resolveAgentAiSettings(
  ...
  const [config, agents] = await Promise.all([
    ...
    client.agents.list(buildAgentsListInput(minionId)),
  ]);
