...
import { LatticeAgent } from "./agent";
import type { ServerConnection } from "./serverConnection";

/**
 * ACP framing is sent over process.stdout.  Any non-protocol output to stdout
 * (e.g., from lattice's logger at info/debug level, or from an in-process server)
...
export async function runAcpAdapter(server: ServerConnection): Promise<void> {
  ...
  const connection = new AgentSideConnection((conn) => {
    const createdAgent = new LatticeAgent(conn, server);
