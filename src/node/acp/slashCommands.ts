...
const SERVER_COMMAND_DEFINITIONS: readonly ServerCommandDefinition[] = [
  {
    ...
    description: "Clear conversation history for this minion.",
  },
  {
  ...
  },
  {
    ...
    description: "Fork the current minion. Optionally include a start message.",
    inputHint: "[start message]",
  },
  {
    name: NEW_COMMAND_NAME,
    description:
      "Create a new minion from the current project. Supports -t <trunk-branch>, -r <runtime>, and multiline start messages.",
...
function parseNewCommand(rawInput: string): ParsedAcpSlashCommand {
  ...
  const minionNameToken = coercePositionalTokenToText(parsed._[0], NEW_USAGE);
  if (minionNameToken.error != null) {
    return {
      kind: "invalid",
      message: minionNameToken.error,
    };
  }

  if (minionNameToken.text == null) {
    return {
      kind: "invalid",
      message: `Missing minion name. Usage: ${NEW_USAGE}`,
    };
  }

  const minionName = minionNameToken.text;
  const trunkBranch =
