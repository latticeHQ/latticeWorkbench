...
export class StreamTranslator {
  ...
  /**
   * Consume the LATTICE chat stream and forward events as ACP session updates.
   * Returns a promise that resolves when the stream ends or errors.
   */
  async consumeAndForward(
    sessionId: string,
    chatStream: AsyncIterable<MinionChatMessage>
  ): Promise<void> {
    assert(
  ...
  private translateReplayMessage(
    sessionId: string,
    event: Extract<MinionChatMessage, { type: "message" }>,
    isReplayPhase: boolean
  ): SessionUpdate[] {
    const updates: SessionUpdate[] = [];
  ...
  private translateTodoWritePlanUpdate(
    ...
    // `todo_write` is Lattice's canonical execution-plan surface. Mirror it into
  ...
  clearSession(sessionId: string): void {
    ...
    // ACP currently has no explicit session/close notification. When Lattice evicts
...
function extractAgentSkillRawCommand(
  ...
  const fromLatticeMetadata = extractRawCommandFromFrontendMetadata(metadata.latticeMetadata);
  if (fromLatticeMetadata != null) {
    return fromLatticeMetadata;
  }

  return extractRawCommandFromFrontendMetadata(metadata.clatticeMetadata);
}
