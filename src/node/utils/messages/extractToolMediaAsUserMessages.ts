import type { LatticeFilePart, LatticeMessage } from "@/common/types/message";
 ...
 *   as proper multimodal file parts (LatticeFilePart)
 *
 ...
 */
export function extractToolMediaAsUserMessages(messages: LatticeMessage[]): LatticeMessage[] {
  const result: LatticeMessage[] = [];
  ...
  for (const msg of messages) {
    ...
    let extractedImages: LatticeFilePart[] = [];
    let changed = false;
    ...
    const rewrittenMsg = changed ? ({ ...msg, parts: newParts } satisfies LatticeMessage) : msg;
...
function extractImagesFromToolOutput(
  output: unknown
): { newOutput: unknown; images: LatticeFilePart[] } | null {
