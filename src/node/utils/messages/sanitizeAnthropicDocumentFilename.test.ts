...
describe("sanitizeAnthropicPdfFilenames", () => {
  const createUserMessageWithPdf = (filename: string): LatticeMessage => ({
    id: "msg-1",
    role: "user",
    parts: [
  ...
  it("sanitizes PDF filenames in user messages", () => {
    const messages: LatticeMessage[] = [createUserMessageWithPdf("report.pdf")];
  ...
  it("does not mutate original messages", () => {
    const messages: LatticeMessage[] = [createUserMessageWithPdf("original.pdf")];
  ...
  it("passes through assistant messages unchanged", () => {
    const assistantMessage: LatticeMessage = {
      id: "msg-2",
      role: "assistant",
  ...
  it("does not sanitize non-PDF files", () => {
    const imageMessage: LatticeMessage = {
      id: "msg-3",
      role: "user",
      parts: [
        {
