...
function shouldUseApproxTokenizer(): boolean {
  // LATTICE_FORCE_REAL_TOKENIZER=1 overrides approx mode (for tests that need real tokenization)
  // LATTICE_APPROX_TOKENIZER=1 enables fast approximate mode (default in Jest)
  if (process.env.LATTICE_FORCE_REAL_TOKENIZER === "1") {
    return false;
  }
  return process.env.LATTICE_APPROX_TOKENIZER === "1";
}
