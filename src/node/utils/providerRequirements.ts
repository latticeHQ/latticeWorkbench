...
export function resolveProviderCredentials(
  ...
  // Subprocess providers (e.g., claude-code): always considered configured when enabled,
  // since they don't need API keys — the CLI binary handles its own auth.
  const def: { requiresApiKey: boolean; isSubprocess?: boolean } = PROVIDER_DEFINITIONS[provider];
  if (def.isSubprocess) {
  ...
  // Anthropic OAuth: treat as configured when OAuth tokens are stored
  // (the actual token injection happens in providerModelFactory via fetch wrapping).
  if (provider === "anthropic") {
    const oauthAuth = parseAnthropicOauthAuth((config as Record<string, unknown>).anthropicOauth);
    if (oauthAuth) {
      return { isConfigured: true, baseUrl };
    }
  }

  // OpenAI Codex OAuth: treat as configured when OAuth tokens are stored
  if (provider === "openai") {
    const codexOauth = parseCodexOauthAuth((config as Record<string, unknown>).codexOauth);
    if (codexOauth) {
      return { isConfigured: true, baseUrl, organization };
    }
  }
