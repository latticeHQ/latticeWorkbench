...
export class PlatformPaths {
  ...
  static expandHome(filePath: string): string {
    ...
    // In tests and other isolated environments, lattice can be configured to store all
    // state under a custom root via LATTICE_ROOT. We also allow runtime config paths
    // like "~/.lattice/src" (portable, works for both local + SSH) to resolve to that
    // root when LATTICE_ROOT is set.
    const latticeRoot = env.LATTICE_ROOT;
    if (latticeRoot) {
      const normalizedLatticeRoot = latticeRoot.replace(/[\\/]+$/g, "");
      const sep = getSeparator();
      const prefixes = ["~/.lattice", "~\\.lattice"] as const;
      for (const prefix of prefixes) {
        if (filePath === prefix) {
          return normalizedLatticeRoot;
        }
        ...
        if (filePath.startsWith(slashPrefix) || filePath.startsWith(backslashPrefix)) {
          ...
          return normalizedLatticeRoot + (normalizedRest ? sep + normalizedRest : "");
        }
      }
    }
