...
describe("pathUtils", () => {
  ...
  describe("validateProjectPath", () => {
    ...
    beforeEach(() => {
      ...
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "lattice-path-test-"));
    });
    ...
    it("should expand tilde and validate", async () => {
      ...
      const testDir = path.join(os.homedir(), `lattice-test-git-${Date.now()}`);
      // eslint-disable-next-line local/no-sync-fs-methods -- Test setup only
    ...
    it("should return error for non-existent path", async () => {
      const nonExistentPath = "/this/path/definitely/does/not/exist/lattice-test-12345";
    ...
    it("should handle tilde path to non-existent directory", async () => {
      const nonExistentTildePath = "~/this-directory-should-not-exist-lattice-test-12345";
  ...
  describe("isGitRepository", () => {
    ...
    beforeEach(() => {
      tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "lattice-git-test-"));
    });
