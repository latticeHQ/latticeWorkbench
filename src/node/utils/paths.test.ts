...
describe("PlatformPaths", () => {
  ...
  describe("parse", () => {
    test("parses absolute path on current platform", () => {
      const testPath = path.join("/", "home", "user", "projects", "lattice");
      ...
      expect(result.basename).toBe("lattice");
    });
  ...
  describe("abbreviate", () => {
    test("abbreviates path", () => {
      ...
      expect(result.endsWith("lattice")).toBe(true);
  ...
  describe("splitAbbreviated", () => {
    test("splits abbreviated path", () => {
      const testPath = path.join("/", "h", "u", "P", "c", "lattice");
  ...
  describe("formatHome", () => {
    test("replaces home directory with tilde", () => {
      ...
      const testPath = path.join(home, "projects", "lattice");
      const result = PlatformPaths.formatHome(testPath);

      const sep = PlatformPaths.separator;
      expect(result).toBe(`~${sep}projects${sep}lattice`);
    });
  ...
  describe("expandHome", () => {
    ...
    test("expands tilde with path", () => {
      const home = os.homedir();
      const sep = path.sep;
      const result = PlatformPaths.expandHome(`~${sep}projects${sep}lattice`);
      expect(result).toBe(path.join(home, "projects", "lattice"));
    });
    ...
    test("expands ~/.lattice to LATTICE_ROOT when set", () => {
      const originalLatticeRoot = process.env.LATTICE_ROOT;
      const testLatticeRoot = path.join(os.tmpdir(), "lattice-root-test");
      process.env.LATTICE_ROOT = testLatticeRoot;
      ...
      try {
        ...
        const home = os.homedir();
        const homePath = `~${sep}projects${sep}lattice`;
        expect(PlatformPaths.expandHome(homePath)).toBe(path.join(home, "projects", "lattice"));
      } finally {
  ...
  describe("getProjectName", () => {
    test("extracts project name from path", () => {
      const testPath = path.join("/", "home", "user", "projects", "lattice");
      expect(PlatformPaths.getProjectName(testPath)).toBe("lattice");
    });

    test("handles relative paths", () => {
      expect(PlatformPaths.getProjectName("projects/lattice")).toBe("lattice");
    });
  ...
  describe("on non-Windows platforms", () => {
    test("returns POSIX paths unchanged", () => {
      if (process.platform !== "win32") {
        ...
        expect(toPosixPath("/tmp/lattice-bashes")).toBe("/tmp/lattice-bashes");
      }
    });
