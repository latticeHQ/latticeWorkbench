...
describe("Config", () => {
  ...
  describe("loadConfigOrDefault with trailing slash migration", () => {
    it("should strip trailing slashes from project paths on load", () => {
      ...
      const corruptedConfig = {
        projects: [
          ["/home/user/project/", { minions: [] }],
          ["/home/user/another//", { minions: [] }],
          ["/home/user/clean", { minions: [] }],
        ],
      };
  ...
  describe("getAllMinionMetadata with migration", () => {
    it("should migrate legacy minion without metadata file", async () => {
      const projectPath = "/fake/project";
      const minionPath = path.join(config.srcDir, "project", "feature-branch");

      // Create minion directory
      fs.mkdirSync(minionPath, { recursive: true });

      // Add minion to config without metadata file
      await config.editConfig((cfg) => {
        cfg.projects.set(projectPath, {
          minions: [{ path: minionPath }],
        });
        return cfg;
      });

      // Get all metadata (should trigger migration)
      const allMetadata = await config.getAllMinionMetadata();
      ...
      expect(projectConfig!.minions).toHaveLength(1);
      const minion = projectConfig!.minions[0];
      expect(minion.id).toBe("project-feature-branch");
      expect(minion.name).toBe("feature-branch");
    });

    it("should use existing metadata file if present (legacy format)", async () => {
      const projectPath = "/fake/project";
      const minionName = "my-feature";
      const minionPath = path.join(config.srcDir, "project", minionName);

      // Create minion directory
      fs.mkdirSync(minionPath, { recursive: true });

      // Test backward compatibility: Create metadata file using legacy ID format.
      // This simulates minions created before stable IDs were introduced.
      const legacyId = config.generateLegacyId(projectPath, minionPath);
      ...
      // Add minion to config (without id/name, simulating legacy format)
      await config.editConfig((cfg) => {
        cfg.projects.set(projectPath, {
          minions: [{ path: minionPath }],
        });
      ...
      expect(metadata.name).toBe(minionName);
      ...
      expect(projectConfig!.minions).toHaveLength(1);
      const minion = projectConfig!.minions[0];
      expect(minion.id).toBe(legacyId);
      expect(minion.name).toBe(minionName);
      expect(minion.createdAt).toBe("2025-01-01T00:00:00.000Z");
    });
  });
