...
describe("createOrpcServer", () => {
  test("serveStatic fallback does not swallow /api routes", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-static-"));
    const indexHtml =
      "<!doctype html><html><head><title>lattice</title></head><body><div>ok</div></body></html>";
    ...
    try {
      ...
      expect(uiText).toContain("lattice");
  ...
  test("does not apply origin validation to static and SPA fallback routes", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "lattice-static-origin-"));
    const indexHtml =
      "<!doctype html><html><head><title>lattice</title></head><body><div>ok</div></body></html>";
  ...
  test("uses HTTPS redirect URIs for OAuth start routes when allowHttpOrigin is enabled", async () => {
    let latticeGovernorRedirectUri = "";

    const stubContext: Partial<ORPCContext> = {
      latticeGovernorOauthService: {
        startServerFlow: (input: { governorOrigin: string; redirectUri: string }) => {
          latticeGovernorRedirectUri = input.redirectUri;
          return {
            success: true,
            data: { authorizeUrl: "https://governor.example.com/auth", state: "state-governor" },
          };
        },
      } as unknown as ORPCContext["latticeGovernorOauthService"],
    };
    ...
    try {
      ...
      const sharedHeaders = {
        ...
        Origin: "https://lattice-public.example.com",
        "X-Forwarded-Host": "lattice-public.example.com:443",
        "X-Forwarded-Proto": "http",
      };

      const latticeGovernorResponse = await fetch(
        `${server.baseUrl}/auth/lattice-governor/start?governorUrl=${encodeURIComponent("https://governor.example.com")}`,
        {
          headers: sharedHeaders,
        }
      );
      expect(latticeGovernorResponse.status).toBe(200);

      expect(latticeGovernorRedirectUri).toBe(
        "https://lattice-public.example.com/auth/lattice-governor/callback"
      );
    } finally {
      await server?.close();
    }
  ...
  test("scopes lattice_session cookie path to forwarded app base path", async () => {
    const stubContext: Partial<ORPCContext> = {
      serverAuthService: {
    ...
    try {
      ...
      const response = await fetch(`${server.baseUrl}/auth/server-login/github/wait`, {
        ...
        headers: {
          ...
          "X-Forwarded-Prefix": "/@test/minion/apps/lattice/",
        },
      ...
      expect(cookieHeader).toContain("lattice_session=session-token-1");
      expect(cookieHeader).toContain("Path=/@test/minion/apps/lattice;");
    } finally {
      await server?.close();
    }
  });

  test("sets Secure lattice_session cookie when allowHttpOrigin is enabled", async () => {
    const stubContext: Partial<ORPCContext> = {
      serverAuthService: {
        waitForGithubDeviceFlow: () =>
          Promise.resolve({
    ...
    try {
      ...
      const cookieHeader = response.headers.get("set-cookie");
      expect(cookieHeader).toBeTruthy();
      expect(cookieHeader).toContain("lattice_session=session-token-compat");
  ...
  test("does not set Secure lattice_session cookie when client-facing proto is HTTP", async () => {
    const stubContext: Partial<ORPCContext> = {
      serverAuthService: {
        waitForGithubDeviceFlow: () =>
          Promise.resolve({
    ...
    try {
      ...
      const cookieHeader = response.headers.get("set-cookie");
      expect(cookieHeader).toBeTruthy();
      expect(cookieHeader).toContain("lattice_session=session-token-http");
  ...
  test("accepts ORPC requests authenticated via lattice_session cookie", async () => {
    const stubContext: Partial<ORPCContext> = {
      serverAuthService: {
    ...
    try {
      ...
      const duplicateCookieClient = createHttpClient(server.baseUrl, {
        Cookie: "lattice_session=invalid-session-token; lattice_session=valid-session-token",
      });
      ...
      const cookieClient = createHttpClient(server.baseUrl, {
        Cookie: "lattice_session=valid-session-token",
      });
  ...
  test("OAuth callback routes accept POST redirects (query + form_post)", async () => {
    ...
    try {
      ...
      const queryRes = await fetch(
        `${server.baseUrl}/auth/lattice-governor/callback?state=test-state&code=test-code`,
        { method: "POST" }
      );
      ...
      const formRes = await fetch(`${server.baseUrl}/auth/lattice-governor/callback`, {
        method: "POST",
  ...
  test("allows cross-origin POST requests on OAuth callback routes", async () => {
    ...
    try {
      ...
      const latticeGovernorResponse = await fetch(`${server.baseUrl}/auth/lattice-governor/callback`, {
        method: "POST",
        headers: callbackHeaders,
        body: "state=test-state&code=test-code",
      });
      expect(latticeGovernorResponse.status).toBe(200);
      expect(latticeGovernorResponse.headers.get("access-control-allow-origin")).toBeNull();
  ...
  test("rejects HTTPS origins when X-Forwarded-Proto is overwritten to http by downstream proxy by default", async () => {
    ...
    try {
      ...
      const response = await fetch(`${server.baseUrl}/api/spec.json`, {
        headers: {
          Origin: "https://lattice-public.example.com",
          "X-Forwarded-Host": "lattice-public.example.com",
          "X-Forwarded-Proto": "http",
        },
      });
  ...
  test("accepts HTTPS origins when allowHttpOrigin is enabled and X-Forwarded-Proto is overwritten to http by downstream proxy", async () => {
    ...
    try {
      ...
      expect(response.headers.get("access-control-allow-origin")).toBe(
        "https://lattice-public.example.com"
      );
      expect(response.headers.get("access-control-allow-credentials")).toBe("true");
    } finally {
      await server?.close();
