...
import type { MinionUsageState } from "@/browser/stores/MinionStore";
...
const buildUsage = (tokens: number, model?: string): MinionUsageState => ({
  totalTokens: tokens,
  lastContextUsage: {
    input: { tokens },
    cached: { tokens: 0 },
...
describe("useContextSwitchWarning", () => {
  ...
  test("does not warn on initial load without a user switch", async () => {
    ...
    const props = {
      ...
      pendingModel: model,
      use1M: false,
      minionUsage: buildUsage(260_000, model),
      api: undefined,
  ...
  test("warns when the user switches to a smaller context model", async () => {
    ...
    const props = {
      ...
      use1M: false,
      minionUsage: buildUsage(260_000, previousModel),
      api: undefined,
    ...
    act(() => {
      recordMinionModelChange(props.minionId, nextModel, "user");
      rerender({
        ...props,
  ...
  test("uses custom model context overrides when evaluating switches", async () => {
    ...
    const props = {
      minionId: "minion-custom-context",
      messages: [buildAssistantMessage(previousModel)],
      pendingModel: previousModel,
      use1M: false,
      minionUsage: buildUsage(95_000, previousModel),
      api: undefined,
      pendingSendOptions: buildSendOptions(previousModel),
      providersConfig,
    };
  ...
  test("re-evaluates explicit switches when providers config loads later", async () => {
    ...
    const props = {
      minionId: "minion-custom-context-late-config",
      messages: [buildAssistantMessage(previousModel)],
      pendingModel: previousModel,
      use1M: false,
      minionUsage: buildUsage(95_000, previousModel),
  ...
  test("re-evaluates explicit switches when providers config updates non-null to non-null", async () => {
    ...
    const props = {
      minionId: "minion-custom-context-config-refresh",
      messages: [buildAssistantMessage(previousModel)],
      pendingModel: previousModel,
      use1M: false,
      minionUsage: buildUsage(95_000, previousModel),
  ...
  test("does not re-show dismissed warnings when config arrives", async () => {
    ...
    const props = {
      minionId: "minion-dismissed-warning",
      messages: [buildAssistantMessage(previousModel)],
      pendingModel: previousModel,
      use1M: false,
      minionUsage: buildUsage(260_000, previousModel),
  ...
  test("does not loop when policy refreshes with identical values", async () => {
    ...
    try {
      ...
      const props = {
        ...
        pendingModel: previousModel,
        use1M: false,
        minionUsage: buildUsage(tokens, previousModel),
        api: undefined,
        pendingSendOptions: buildSendOptions(previousModel),
        providersConfig: null,
      };
      ...
      act(() => {
        setMinionModelWithOrigin(props.minionId, nextModel, "user");
        rerender({
          ...props,
          pendingModel: nextModel,
          pendingSendOptions: buildSendOptions(nextModel),
  ...
  test("warns when an agent-driven model change overflows context", async () => {
    ...
    act(() => {
      setMinionModelWithOrigin(props.minionId, nextModel, "agent");
      rerender({
        ...props,
        pendingModel: nextModel,
        pendingSendOptions: buildSendOptions(nextModel),
  ...
  test("clears stale warning when user switches with zero tokens", async () => {
    ...
    act(() => {
      rerender({
        ...
        pendingSendOptions: buildSendOptions(nextModel),
        providersConfig: null,
        minionUsage: buildUsage(0, nextModel),
      });
    });

    act(() => {
      recordMinionModelChange(props.minionId, finalModel, "user");
      rerender({
        ...props,
        pendingModel: finalModel,
        pendingSendOptions: buildSendOptions(finalModel),
        providersConfig: null,
        minionUsage: buildUsage(0, finalModel),
      });
    });
  ...
  test("warns after deferred switch once usage loads", async () => {
    ...
    const props = {
      ...
      pendingModel: previousModel,
      use1M: false,
      minionUsage: buildUsage(0, previousModel),
      api: undefined,
      pendingSendOptions: buildSendOptions(previousModel),
      providersConfig: null,
    };
  ...
  test("does not warn when deferred switch diverges on sync update", async () => {
    ...
    act(() => {
      rerender({
        ...
        pendingSendOptions: buildSendOptions(nextModel),
        providersConfig: null,
        minionUsage: buildUsage(tokens, nextModel),
      });
    });
  ...
  test("warns when 1M is toggled off and context no longer fits", async () => {
    ...
    const props = {
      ...
      pendingModel: model,
      use1M: true,
      minionUsage: buildUsage(tokens, model),
      api: undefined,
      pendingSendOptions: buildSendOptions(model),
      providersConfig: null,
    };
