...
interface UseContextSwitchWarningProps {
  ...
  pendingModel: string;
  use1M: boolean;
  minionUsage: MinionUsageState | undefined;
...
export function useContextSwitchWarning(
  ...
  // ChatPane is keyed by minionId today; keep a defensive reset to avoid stale warnings
  ...
  }, [minionId, pendingModel, use1M, checkOptions]);

  const getCurrentTokens = useCallback(() => {
    const usage = minionUsage?.liveUsage ?? minionUsage?.lastContextUsage;
  ...
  const handleModelChange = useCallback(
    (newModel: string) => {
      if (newModel.trim() === pendingModel.trim()) {
        return;
      }

      // User request: record explicit model switches so warnings only follow user actions.
      setMinionModelWithOrigin(minionId, newModel, "user");
    },
    [pendingModel, minionId]
  );
  ...
  const handleCompact = useCallback(() => {
    ...
    dispatch({ type: "CLEAR_WARNING" });
  }, [api, minionId, pendingSendOptions, warning]);
  ...
  // Sync with indirect model changes (e.g., MinionModeAISync updating model on mode/agent change).
  ...
  useEffect(() => {
    ...
    dismissedWarningModelRef.current = null;
    const origin = consumeMinionModelChange(minionId, pendingModel);
    // Agent/mode switches call setMinionModelWithOrigin, so they flow through this explicit path.
  ...
  }, [pendingModel, queueExplicitSwitch, switchState.currentModel, minionId]);
