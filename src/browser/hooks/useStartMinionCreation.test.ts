...
import {
  getFirstProjectPath,
  persistMinionCreationPrefill,
  type StartMinionCreationDetail,
} from "./useStartMinionCreation";
import {
  getInputKey,
  getModelKey,
  getPendingScopeId,
...
describe("persistMinionCreationPrefill", () => {
  ...
  test("writes provided values and normalizes whitespace", () => {
    const detail: StartMinionCreationDetail = {
      projectPath,
    ...
    persistMinionCreationPrefill(projectPath, detail, persist);
  ...
  test("no-op when detail is undefined", () => {
    const { persist, calls } = createPersistSpy();
    persistMinionCreationPrefill(projectPath, undefined, persist);
...
describe("getFirstProjectPath", () => {
  test("returns first project path or null", () => {
    ...
    projects.set("/tmp/a", { path: "/tmp/a", minions: [] } as ProjectConfig);
    projects.set("/tmp/b", { path: "/tmp/b", minions: [] } as ProjectConfig);
