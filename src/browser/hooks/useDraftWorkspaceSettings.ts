...
export function useDraftWorkspaceSettings(
  ...
  // Headquarter-scoped model preference (persisted per project)
  const [model] = usePersistedState<string>(getModelKey(projectScopeId), getDefaultModel(), {
    listener: true,
  });

  // Headquarter-scoped default runtime (worktree by default, only changed via checkbox)
  ...
  // Headquarter-scoped trunk branch preference (persisted per project)
  ...
  // Headquarter-scoped last runtime config (persisted per provider, stored as an object)
  ...
  const lastLatticeEnabled = readRuntimeConfig(RUNTIME_MODE.SSH, "latticeEnabled", false);
  const lastLatticeConfig = readRuntimeConfig<LatticeWorkspaceConfig | null>(
    RUNTIME_MODE.SSH,
    "latticeConfig",
    null
  );
  ...
  // Build ParsedRuntime from mode + stored host/image/shareCredentials/lattice
  ...
  const buildRuntimeForMode = (
    ...
    switch (mode) {
      ...
      case RUNTIME_MODE.SSH: {
        ...
        const effectiveHost =
          latticeEnabled && latticeConfig && !sshHost.trim()
            ? LATTICE_RUNTIME_PLACEHOLDER
            : sshHost;
        ...
        return {
          mode: "ssh",
          host: effectiveHost,
          lattice: latticeEnabled && latticeConfig ? latticeConfig : undefined,
        };
      }
  ...
  useEffect(() => {
    ...
    if (prevMode !== selectedRuntime.mode) {
      if (selectedRuntime.mode === RUNTIME_MODE.SSH) {
        ...
        const needsLatticeRestore =
          selectedRuntime.lattice === undefined && lastLatticeEnabled && lastLatticeConfig;
        if (needsHostRestore || needsLatticeRestore) {
          setSelectedRuntimeState({
            mode: RUNTIME_MODE.SSH,
            host: needsHostRestore ? lastSshHost : selectedRuntime.host,
            lattice: needsLatticeRestore ? lastLatticeConfig : selectedRuntime.lattice,
          });
        }
      }
  ...
  const setSelectedRuntime = (runtime: ParsedRuntime) => {
    ...
    if (runtime.mode === RUNTIME_MODE.SSH) {
      ...
      // Persist Lattice enabled state and config
      const latticeEnabled = runtime.lattice !== undefined;
