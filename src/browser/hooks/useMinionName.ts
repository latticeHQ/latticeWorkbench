...
import { getMinionNameStateKey } from "@/common/constants/storage";
...
/** Discriminated error type for minion name operations */
export type MinionNameUIError =
 ...
 * Candidates are sent as canonical model IDs.
 */
...
export interface UseMinionNameOptions {
...
/** Generated minion identity (name + title) */
const MinionIdentitySchema = z.object({
...
export type MinionIdentity = z.infer<typeof MinionIdentitySchema>;

/** State and actions for minion name generation, suitable for passing to components */
export interface MinionNameState {
...
export interface UseMinionNameReturn extends MinionNameState {
  /** Wait for any pending generation to complete, returns both name and title */
  waitForGeneration: () => Promise<MinionIdentity | null>;
}

const MinionNamePersistedStateSchema = z.object({
  generatedIdentity: MinionIdentitySchema.nullable(),
  manualName: z.string(),
  autoGenerate: z.boolean(),
  lastGeneratedFor: z.string(),
});

export type MinionNamePersistedState = z.infer<typeof MinionNamePersistedStateSchema>;

const DEFAULT_PERSISTED_STATE: MinionNamePersistedState = {
...
/**
 * Extract the display title from persisted minion name state.
 * Used by DraftMinionListItem to show the title in the sidebar without
 ...
 * This matches how real minions display: metadata.title ?? metadata.name.
 */
export function getDisplayTitleFromPersistedState(state: unknown): string {
  const parsed = MinionNamePersistedStateSchema.partial().safeParse(state);
  if (!parsed.success) {
    return "";
  }
...
/**
 * Hook for managing minion name generation with debouncing.
 *
 ...
 */
export function useMinionName(options: UseMinionNameOptions): UseMinionNameReturn {
  ...
  const anonymousScopeIdRef = useRef(`__minionNameAnon__${Math.random().toString(36).slice(2)}`);
  const persistedKey = getMinionNameStateKey(scopeId ?? anonymousScopeIdRef.current);

  const [persistedState, setPersistedState] = usePersistedState<MinionNamePersistedState>(
    persistedKey,
    DEFAULT_PERSISTED_STATE,
    { listener: true }
  );
  const [localState, setLocalState] =
    useState<MinionNamePersistedState>(DEFAULT_PERSISTED_STATE);
  ...
  const [error, setError] = useState<MinionNameUIError | null>(null);
  ...
  const generationPromiseRef = useRef<{
    promise: Promise<MinionIdentity | null>;
    resolve: (identity: MinionIdentity | null) => void;
    requestId: number;
  } | null>(null);
  ...
  const generateIdentity = useCallback(
    async (forMessage: string): Promise<MinionIdentity | null> => {
      ...
      let resolvePromise: ((identity: MinionIdentity | null) => void) | undefined;
      const promise = new Promise<MinionIdentity | null>((resolve) => {
  ...
  const waitForGeneration = useCallback(async (): Promise<MinionIdentity | null> => {
    // If auto-generate is off, user has provided a manual name
    // Use that name directly with a generated title from the message
    if (!autoGenerate) {
      if (!manualName.trim()) {
        setError({ kind: "validation", message: "Please enter a minion name" });
        return null;
      }
