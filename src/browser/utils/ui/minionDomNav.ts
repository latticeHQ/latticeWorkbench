 ...
 * the exact visual order the user sees (respecting sort, stages, collapsed
 * projects, etc.).
 *
 ...
 * stage is collapsed), returns the first visible minion — matching how
...
export function findAdjacentMinionId(currentMinionId: string): string | null {
  ...
  if (idx === -1) {
    // Current minion not rendered (collapsed project/stage) — pick the
