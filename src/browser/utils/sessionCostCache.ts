/**
 * Persisted LRU cache for minion session costs.
 *
 * Stores only the computed cost number (not the full usage JSON) to keep localStorage small.
 * Used by BenchedMinions to show cached costs immediately and prevent layout flash.
 *
 * TTL: 7 days (costs for archived minions rarely change)
