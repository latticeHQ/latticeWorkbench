 ...
 */
function mergeAdjacentParts(parts: LatticeMessage["parts"]): LatticeMessage["parts"] {
...
export class StreamingMessageAggregator {
   ...
   */
  private extractFinalResponseText(message: LatticeMessage | undefined): string {
    if (!message) return "";
  ...
  private compactMessageParts(message: LatticeMessage): void {
    message.parts = mergeAdjacentParts(message.parts);
  }

  addMessage(message: LatticeMessage): void {
  ...
  private getLatestCompactionRequest(): CompactionRequestData | null {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      if (latticeMetadata?.type === "compaction-request") {
        return latticeMetadata.parsed;
      }
      return null;
    }
  ...
  handleStreamStart(data: StreamStartEvent): void {
    ...
    const streamingMessage = createLatticeMessage(data.messageId, "assistant", "", {
  ...
  handleStreamEnd(data: StreamEndEvent): void {
    ...
    if (activeStream) {
      ...
      if (message?.metadata) {
        ...
        const updatedMetadata: LatticeMetadata = {
          ...message.metadata,
          ...data.metadata,
        };
  ...
  handleStreamError(data: StreamErrorMessage): void {
    ...
    if (activeStream) {
      ...
      const errorMessage: LatticeMessage = {
        id: data.messageId,
        role: "assistant",
        parts: [],
        metadata: {
  ...
  handleMessage(data: MinionChatMessage): void {
    ...
    // Check if it's a LatticeMessage (has role property but no type)
    if (isLatticeMessage(data)) {
      ...
      if (incomingMessage.role === "user") {
        const latticeMeta = incomingMessage.metadata?.latticeMetadata as
        ...
        if (latticeMeta?.displayStatus) {
          ...
          this.agentStatus = latticeMeta.displayStatus;
        } else {
  ...
  private buildDisplayedMessagesForMessage(
    ...
    const latticeMeta = message.metadata?.latticeMetadata;
    if (latticeMeta?.type === "plan-display") {
    ...
    if (message.role === "user") {
      ...
      const fileParts = message.parts
        .filter((p): p is LatticeFilePart => p.type === "file")
        .map((p) => ({
      ...
      const compactionRequest =
        latticeMeta?.type === "compaction-request"
          ? {
              parsed: {
                model: latticeMeta.parsed.model,
                maxOutputTokens: latticeMeta.parsed.maxOutputTokens,
      ...
      const commandPrefix = latticeMeta?.commandPrefix;

      // Extract reviews from latticeMetadata for rich UI display (orthogonal to message type)
      const reviews = latticeMeta?.reviews;
