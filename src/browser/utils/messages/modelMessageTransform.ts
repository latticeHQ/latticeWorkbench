 ...
 * survives filtering.
 */
export function addInterruptedSentinel(messages: LatticeMessage[]): LatticeMessage[] {
...
${planContent}
  ...
  const transitionMessage: LatticeMessage = {
...
export function injectFileChangeNotifications(
  ...
  const syntheticMessage: LatticeMessage = {
  ...
  return messages.map((msg) => {
    ...
    for (const part of assistantMsg.content) {
      ...
      if (part.type === "reasoning" && lastPart?.type === "reasoning") {
        ...
        // but our LatticeReasoningPart (which flows through convertToModelMessages) does.
