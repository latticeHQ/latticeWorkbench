...
import {
  applyMinionChatEventToAggregator,
  type MinionChatEventAggregator,
} from "./applyMinionChatEventToAggregator";

class StubAggregator implements MinionChatEventAggregator {
...
describe("applyMinionChatEventToAggregator", () => {
  test("stream-start routes to handleStreamStart", () => {
    ...
    const hint = applyMinionChatEventToAggregator(aggregator, event);
  ...
  test("runtime-status routes to handleRuntimeStatus", () => {
    ...
    const event: MinionChatMessage = {
      ...
      phase: "starting",
      runtimeType: "ssh",
      detail: "Starting Lattice minion...",
    };
  ...
  test("throws when aggregator is missing", () => {
    ...
    expect(() =>
      applyMinionChatEventToAggregator(null as unknown as MinionChatEventAggregator, event)
    ).toThrow();
  });
});
