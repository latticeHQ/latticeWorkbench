 ...
 * LatticeMessage objects. It's shared between desktop and mobile implementations.
 *
 * Responsibilities:
...
export interface ChatEventProcessor {
   ...
   */
  getMessages(): LatticeMessage[];

  /**
   * Get a specific message by ID.
   */
  getMessageById(id: string): LatticeMessage | undefined;
...
type ExtendedStreamStartEvent = StreamStartEvent & {
  role?: "user" | "assistant";
  metadata?: Partial<LatticeMetadata>;
  timestamp?: number;
};

type ExtendedStreamEndEvent = Omit<StreamEndEvent, "metadata"> & {
  metadata: StreamEndEvent["metadata"] & Partial<LatticeMetadata>;
};

function createLatticeMessage(
  id: string,
  role: "user" | "assistant",
  content: string,
  metadata?: LatticeMetadata
): LatticeMessage {
  const parts: LatticeMessage["parts"] = content ? [{ type: "text" as const, text: content }] : [];
...
export function createChatEventProcessor(): ChatEventProcessor {
  ...
  const handleEvent = (event: MinionChatMessage): void => {
    ...
    if (isStreamStart(event)) {
      ...
      const message = createLatticeMessage(start.messageId, start.role ?? "assistant", "", {
        ...
        latticeMetadata: start.metadata?.latticeMetadata,
        partial: true,
      });
    ...
    if (isStreamEnd(event)) {
      ...
      message.metadata = {
        ...
        latticeMetadata: metadata.latticeMetadata ?? message.metadata?.latticeMetadata,
        historySequence:
  ...
  const getMessages = (): LatticeMessage[] => {
  ...
  const getMessageById = (id: string): LatticeMessage | undefined => {
