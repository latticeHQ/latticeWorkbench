...
function isForceAllRetryableEnabled(): boolean {
  return typeof window !== "undefined" && window.__LATTICE_FORCE_ALL_RETRYABLE === true;
}
...
export function isNonRetryableSendError(error: SendMessageError): boolean {
  ...
  switch (error.type) {
    ...
    case "incompatible_workspace": // Workspace from newer lattice version - user must upgrade
