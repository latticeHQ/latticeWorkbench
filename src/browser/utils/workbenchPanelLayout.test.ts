...
test("splitFocusedTabset moves active tab when possible (no empty tabsets)", () => {
  // Use a multi-tab flat layout so the split produces two tabsets
  const s0 = makeFlatState(["costs", "review", "explorer"], "costs");
...
test("addTabToFocusedTabset can add a tab without stealing focus", () => {
  const s0: WorkbenchPanelLayoutState = {
    version: 1,
    nextId: 2,
    focusedTabsetId: "tabset-1",
...
test("moveTabToTabset moves tab between tabsets", () => {
  // Create a known split layout with two tabsets
  const s0: WorkbenchPanelLayoutState = {
    version: 1,
    nextId: 3,
    focusedTabsetId: "tabset-1",
    root: {
      ...
      sizes: [50, 50],
      children: [
        { type: "tabset", id: "tabset-1", tabs: ["costs", "review"], activeTab: "costs" },
        { type: "tabset", id: "tabset-2", tabs: ["explorer", "terminal"], activeTab: "explorer" },
      ],
    },
  };

  // Move costs from tabset-1 to tabset-2
  const s1 = moveTabToTabset(s0, "costs", "tabset-1", "tabset-2");
  expect(s1.root.type).toBe("split");
  if (s1.root.type !== "split") throw new Error("expected split");
  ...
  expect(right.tabs).toContain("costs");
  expect(right.activeTab).toBe("costs");
});

test("moveTabToTabset removes empty source tabset", () => {
  // Create a split where one tabset has only one tab
  let s: WorkbenchPanelLayoutState = {
    version: 1,
    nextId: 3,
    focusedTabsetId: "tabset-1",
    root: {
...
test("reorderTabInTabset reorders tabs within a tabset", () => {
  // Flat layout with ["costs", "review", "explorer"]; reorder costs from 0 to 1
  const s0 = makeFlatState(["costs", "review", "explorer"], "costs");
...
test("dockTabToEdge splits a tabset and moves the dragged tab into the new pane", () => {
  // Flat layout with ["costs", "review", "explorer"]; drag review into a bottom split
  const s0 = makeFlatState(["costs", "review", "explorer"], "costs");
...
test("closeSplit keeps the specified child", () => {
  const s: WorkbenchPanelLayoutState = {
    version: 1,
    nextId: 3,
    focusedTabsetId: "tabset-1",
    root: {
