...
test("selectTabInFocusedTabset adds missing tool and makes it active", () => {
  ...
  s = selectTabInFocusedTabset(s, "review");
  expect(s.root.type).toBe("tabset");
  if (s.root.type !== "tabset") throw new Error("expected tabset");
  expect(s.root.tabs).toEqual(["costs", "review"]);
  expect(s.root.activeTab).toBe("review");
});

test("splitFocusedTabset moves active tab when possible (no empty tabsets)", () => {
  const s0 = getDefaultRightSidebarLayoutState("review");
...
test("moveTabToTabset removes empty source tabset", () => {
  ...
  let s: RightSidebarLayoutState = {
    ...
    root: {
      ...
      children: [
        ...
        { type: "tabset", id: "tabset-2", tabs: ["review", "explorer"], activeTab: "review" },
      ],
    },
  };
  ...
  expect(s.root.tabs).toContain("explorer");
});

test("reorderTabInTabset reorders tabs within a tabset", () => {
  // Default layout has ["costs", "review", "explorer", "cluster", "models", "browser"];
  // Reorder costs from index 0 to index 1
  const s0 = getDefaultRightSidebarLayoutState("costs");
  ...
  expect(s1.root.tabs).toEqual(["review", "costs", "explorer", "cluster", "models", "browser"]);
