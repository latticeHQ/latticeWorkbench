import { isTabType, isTerminalTab, type TabType } from "@/browser/types/rightSidebar";
...
/**
 * Default layout state for the main area (PM Chat + employee terminal tabs).
 * Always starts with just the "chat" tab — employees are added on demand via the + button.
 */
...
export function getDefaultRightSidebarLayoutState(activeTab: TabType): RightSidebarLayoutState {
  ...
  // Default layout: flat single tabset (no terminal split — terminals live in MainArea)
  return {
    version: 1,
    nextId: 2,
    focusedTabsetId: "tabset-1",
  ...
  if (isRightSidebarLayoutState(raw)) {
    ...
    for (const tab of collectAllTabs(raw.root)) {
      if (isTerminalTab(tab)) {
        migrated = removeTabEverywhere(migrated, tab);
      }
    }
    return migrated;
  }
...
/** Recursively replace oldTab with newTab in every tabset (tabs array + activeTab). */
function replaceTabInNode(
  node: RightSidebarLayoutNode,
  oldTab: TabType,
  newTab: TabType
  ...
  if (node.type === "split") {
    return {
      ...node,
      children: [
        replaceTabInNode(node.children[0], oldTab, newTab),
        replaceTabInNode(node.children[1], oldTab, newTab),
      ],
    };
  }
  // tabset — only touch it if oldTab is actually present
  if (!node.tabs.includes(oldTab)) return node;
  return {
    ...node,
...
/**
 * Replace every occurrence of oldTab with newTab across the entire layout,
 * preserving tab order and active-tab state.
 */
export function replaceTabEverywhere(
  state: RightSidebarLayoutState,
  oldTab: TabType,
  newTab: TabType
): RightSidebarLayoutState {
  return { ...state, root: replaceTabInNode(state.root, oldTab, newTab) };
}
...
export function splitFocusedTabset(
  ...
  const fallbackTab: TabType =
    focused.activeTab === "costs" ? "review" : "costs";
...
/**
 * Find the ID of the tabset that contains terminal tabs (the "terminal pane").
 * Falls back to null if no tabset currently has a terminal tab.
 */
export function findTerminalTabsetId(root: RightSidebarLayoutNode): string | null {
  if (root.type === "tabset") {
    const hasTerminal = root.tabs.some((t) => t.startsWith("terminal:"));
    return hasTerminal ? root.id : null;
  }
  return findTerminalTabsetId(root.children[0]) ?? findTerminalTabsetId(root.children[1]);
}

/**
 * Add a tab to the tabset that contains terminals (or the focused tabset as fallback).
 * Used when restoring terminal sessions so they land in the terminal pane, not the tools pane.
 */
export function addTabToTerminalTabset(
  ...
  const terminalTabsetId = findTerminalTabsetId(state.root);
  if (terminalTabsetId) {
    return {
      ...state,
      root: updateNode(state.root, terminalTabsetId, (ts) => ({
        ...ts,
        tabs: ts.tabs.includes(tab) ? ts.tabs : [...ts.tabs, tab],
        activeTab: activate ? tab : ts.activeTab,
      })),
    };
  }
  // Fallback: add to focused tabset
...
function getFallbackTabForEmptyTabset(movedTab: TabType): TabType {
  return movedTab === "costs" ? "review" : "costs";
}
