...
const LINE_HEIGHT_DEBUG_MINION_ID = "ws-review-immersive-line-height";
...
const HIGHLIGHT_VS_PLAIN_MINION_ID = "ws-review-immersive-highlight-vs-plain";
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        ...
        localStorage.setItem(WORKBENCH_PANEL_WIDTH_KEY, "760");
        localStorage.removeItem(getWorkbenchPanelLayoutKey(LINE_HEIGHT_DEBUG_MINION_ID));
        updatePersistedState(getReviewImmersiveKey(LINE_HEIGHT_DEBUG_MINION_ID), true);

        const client = setupSimpleChatStory({
          minionId: LINE_HEIGHT_DEBUG_MINION_ID,
          minionName: "feature/immersive-line-height",
          projectName: "my-app",
          messages: [
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        localStorage.setItem(WORKBENCH_PANEL_TAB_KEY, JSON.stringify("review"));
        localStorage.setItem(WORKBENCH_PANEL_WIDTH_KEY, "760");
        localStorage.removeItem(getWorkbenchPanelLayoutKey(HIGHLIGHT_VS_PLAIN_MINION_ID));
        updatePersistedState(getReviewImmersiveKey(HIGHLIGHT_VS_PLAIN_MINION_ID), true);

        const diffOutput = buildHighlightVsPlainDiffOutput();
        const client = setupSimpleChatStory({
          minionId: HIGHLIGHT_VS_PLAIN_MINION_ID,
          minionName: "feature/immersive-highlight-vs-plain",
          projectName: "my-app",
          messages: [
