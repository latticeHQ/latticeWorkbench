...
export function createSSHWorkspace(
  ...
  return createWorkspace({
    ...
    runtimeConfig: {
      type: "ssh",
      host: opts.host,
      srcBaseDir: "/home/user/.lattice/src",
    },
  });
}
...
export function createPendingTool(toolCallId: string, toolName: string, args: object): LatticePart {
...
): LatticePart & { nestedCalls?: NestedToolCall[] } {
  return {
    type: "dynamic-tool",
    toolCallId,
...
export function createStaticChatHandler(messages: ChatLatticeMessage[]): ChatHandler {
  return (callback) => {
    setTimeout(() => {
