...
export const TaskForegroundShowsTaskId: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() => {
        const minionId = "ws-main";
        const projectName = "my-app";

        const mainMinion = createMinion({
          id: minionId,
          name: "feature",
          projectName,
        });

        const taskMinionId = "task-foreground-001";
        ...
        const minions = [mainMinion, taskMinion];

        selectMinion(mainMinion);
        collapseWorkbenchPanel();
          ...
          createAssistantMessage("a1", "Spawning… (foreground)", {
            ...
            toolCalls: [
              createPendingTaskTool(toolCallId, {
                sidekick_type: "explore",
                prompt: "Open the child minion as soon as it is created.",
                title: "Foreground task",
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        ...
        const minions = [mainMinion, taskFrontend, taskBackend];

        // Select the main chat minion.
        selectMinion(mainMinion);
        collapseWorkbenchPanel();
        ...
        const chatHandlers = new Map([[minionId, createStaticChatHandler(messages)]]);
 ...
 *
 * Verifies the transcript viewer dialog renders messages loaded via minion.getSidekickTranscript.
 */
export const TaskTranscriptViewer: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() => {
        ...
        const transcriptMessages: LatticeMessage[] = [
          createUserMessage("tu1", "Summarize the minion cleanup flow.", {
            historySequence: 1,
          }),
        ...
        return setupSimpleChatStory({
          minionId: "ws-task-transcript-viewer",
          messages: [
            ...
            createAssistantMessage("a1", "Here's the task result:", {
              ...
              toolCalls: [
                createCompletedTaskTool("tc1", {
                  sidekick_type: "explore",
                  prompt: "Investigate the minion cleanup flow",
  ...
  play: async ({ canvasElement }) => {
    ...
    const taskToolPrompt = "Investigate the minion cleanup flow";
...
export const TaskApplyGitPatchStates: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() =>
        setupSimpleChatStory({
          minionId: "ws-task-apply-git-patch-states",
          messages: [
...
export const TaskApplyGitPatchCommitList: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() =>
        setupSimpleChatStory({
          minionId: "ws-task-apply-git-patch-commit-list",
          messages: [
            createUserMessage("u1", "Apply the patch from task-fe-001", { historySequence: 1 }),
...
export const TaskApplyGitPatchDryRunCommitList: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() =>
        setupSimpleChatStory({
          minionId: "ws-task-apply-git-patch-dry-run-commit-list",
          messages: [
