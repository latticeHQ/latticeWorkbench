...
/** Setup minions across multiple projects for a lived-in sidebar */
function setupGovernorStory(options: GovernorStoryOptions = {}): APIClient {
  const minions = [
    createMinion({ id: "ws-1", name: "main", projectName: "my-app" }),
    createMinion({ id: "ws-2", name: "feat/auth", projectName: "my-app" }),
    createMinion({ id: "ws-3", name: "hotfix/login", projectName: "my-app" }),
    createMinion({ id: "ws-4", name: "main", projectName: "infra-tools" }),
    createMinion({ id: "ws-5", name: "migrate-db", projectName: "infra-tools" }),
  ];

  selectMinion(minions[0]);

  // Expand both projects so sidebar looks populated behind the modal
  const projectPaths = [...new Set(minions.map((w) => w.projectPath))];
...
export const EnrolledWithPolicy: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() =>
        setupGovernorStory({
          latticeGovernorUrl: "https://governor.example.com",
          latticeGovernorEnrolled: true,
