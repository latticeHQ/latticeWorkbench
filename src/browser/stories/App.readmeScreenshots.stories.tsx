 ...
 *
 * Each story here is intended to match a screenshot embedded in README.md (static/img/*.webp).
...
const README_PROJECT_NAME = "lattice";
const README_PROJECT_PATH = "/home/user/projects/lattice";
...
function createReadmeGitDivergenceExecutor(gitStatus: Map<string, GitStatusFixture>) {
  ...
  const buildBranchDetailsOutput = (status: GitStatusFixture): string => {
    ...
    return [
      "__LATTICE_BRANCH_DATA__BEGIN_SHOW_BRANCH__",
      showBranchLines.join("\n"),
      "__LATTICE_BRANCH_DATA__END_SHOW_BRANCH__",
      "__LATTICE_BRANCH_DATA__BEGIN_DATES__",
      dates,
      "__LATTICE_BRANCH_DATA__END_DATES__",
      "__LATTICE_BRANCH_DATA__BEGIN_DIRTY_FILES__",
      dirtyFiles,
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        ...
        window.localStorage.setItem(WORKBENCH_PANEL_TAB_KEY, JSON.stringify("review"));
        window.localStorage.setItem(WORKBENCH_PANEL_WIDTH_KEY, "950");
        window.localStorage.removeItem(getWorkbenchPanelLayoutKey(minionId));

        const REVIEW_DIFF = `diff --git a/src/browser/components/MinionShell.tsx b/src/browser/components/MinionShell.tsx
index aaa1111..bbb2222 100644
--- a/src/browser/components/MinionShell.tsx
+++ b/src/browser/components/MinionShell.tsx
@@ -1,8 +1,18 @@
 import React from 'react';
+import { useWorkbenchPanelLayout } from '../hooks/useWorkbenchPanelLayout';
+import { clamp } from '../utils/layout';
 
-export function MinionShell() {
-  return <div className="shell" />;
+export function MinionShell(props: MinionShellProps) {
+  const layout = useWorkbenchPanelLayout(props.minionId);
+  const sidebarWidth = clamp(layout.width, 200, 800);
+
+  return (
+    <div className="shell">
+      <header className="shell-header" aria-label="Minion">Lattice</header>
...
diff --git a/src/browser/hooks/useWorkbenchPanelLayout.ts b/src/browser/hooks/useWorkbenchPanelLayout.ts
new file mode 100644
index 0000000..def5678
--- /dev/null
+++ b/src/browser/hooks/useWorkbenchPanelLayout.ts
@@ -0,0 +1,18 @@
+import { usePersistedState } from './usePersistedState';
+import { getWorkbenchPanelLayoutKey } from '@/common/constants/storage';
+
+export function useWorkbenchPanelLayout(minionId: string) {
...
+}`;
        ...
        const REVIEW_NUMSTAT = `10\t2\tsrc/browser/components/MinionShell.tsx
12\t0\tsrc/browser/utils/layout.ts
18\t0\tsrc/browser/hooks/useWorkbenchPanelLayout.ts`;

        const client = createMockORPCClient({
          projects: groupMinionsByProject([minion]),
          minions: [minion],
            ...
            new Map([
              [
                ...
                createStaticChatHandler([
                  createUserMessage(
                    "msg-1",
                    "Review this PR — focus on the layout changes and flag anything risky. The PR adds a workbench panel layout system to MinionShell.",
                    {
                      historySequence: 1,
                  ...
                  createAssistantMessage("msg-2", "I'll start by reading the changed files.", {
                    ...
                    toolCalls: [
                      createFileReadTool(
                        "call-read-1",
                        "src/browser/components/MinionShell.tsx",
                        'import React from \'react\';\nimport { useWorkbenchPanelLayout } from \'../hooks/useWorkbenchPanelLayout\';\nimport { clamp } from \'../utils/layout\';\n\nexport function MinionShell(props: MinionShellProps) {\n  const layout = useWorkbenchPanelLayout(props.minionId);\n  const sidebarWidth = clamp(layout.width, 200, 800);\n  return (\n    <div className="shell">\n      <header className="shell-header" aria-label="Minion">Lattice</header>\n      <main className="shell-content" style={{ marginRight: sidebarWidth }} />\n      <aside className="shell-sidebar" style={{ width: sidebarWidth }} />\n    </div>\n  );\n}'
                      ),
                      createFileReadTool(
                        "call-read-2",
                      ...
                      createFileReadTool(
                        "call-read-3",
                        "src/browser/hooks/useWorkbenchPanelLayout.ts",
                        "import { usePersistedState } from './usePersistedState';\nimport { getWorkbenchPanelLayoutKey } from '@/common/constants/storage';\n\nexport function useWorkbenchPanelLayout(minionId: string) {\n  const [layout] = usePersistedState(\n    getWorkbenchPanelLayoutKey(minionId),\n    { width: 400, collapsed: false }\n  );\n  return layout;\n}"
                      ),
                    ],
                  }),
                  createAssistantMessage(
                    ...
                    "The layout hook looks clean. Let me check for type safety and run the tests.",
                    {
                      ...
                      toolCalls: [
                        ...
                        createBashTool(
                          "call-bash-2",
                          "bun test -- MinionShell useWorkbenchPanelLayout layout",
                          "PASS  tests/ui/MinionShell.test.tsx (6 tests)\nPASS  tests/ui/useWorkbenchPanelLayout.test.tsx (4 tests)\nPASS  tests/ui/layout.test.tsx (3 tests)\n\nTest Suites: 3 passed, 3 total\nTests:       13 passed, 13 total\nTime:        1.84s"
                        ),
                      ],
                    }
                  ),
...
**Changes:**
- \`MinionShell.tsx\` — Restructured to accept \`minionId\` prop and integrate the new sidebar layout hook. Added \`aria-label\` for accessibility.
- \`layout.ts\` — New \`clamp()\` and \`px()\` utilities.
- \`useWorkbenchPanelLayout.ts\` — New hook that reads persisted layout state.
...
**Verdict:** Low risk, mostly additive. Fix the NaN guard before merging.`,
                  ...
                  createAssistantMessage("msg-7", "Fixed and pushed.", {
                    ...
                    toolCalls: [
                      ...
                      createStatusTool(
                        ...
                        "🚀",
                        "PR ready",
                        "https://github.com/latticeHQ/latticeWorkbench/pull/2035"
                      ),
                    ],
                  }),
                ]),
          ...
          executeBash: createGitStatusExecutor(
            new Map([
              [minionId, { ahead: 2, dirty: 0, outgoingAdditions: 40, outgoingDeletions: 2 }],
            ]),
            new Map([
              [
                minionId,
...
// README: static/img/agent-status.webp
// This story keeps the left sidebar expanded and seeds varied status_set tool calls
// so minion rows show realistic in-progress agent activity.
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        ...
        const minionFixtures = [
          {
            ...
            statusUrl: "https://github.com/latticeHQ/latticeWorkbench/pull/2035",
          },
          {
            id: "ws-status-2",
            name: "feature/sidebar",
            assistantText: "Profiling minion row rendering and hover interactions.",
            statusEmoji: "🔍",
        ...
        // Seed persisted statusState for every minion so the sidebar can show
        // many status examples without requiring each minion's onChat stream
        // to be actively subscribed.
        for (const fixture of minionFixtures) {
        ...
        const primaryMinion = minionFixtures[0];
        ...
        const chatHandlers = new Map([
              ...
              createAssistantMessage("msg-2", "On it. I am reproducing the visual diffs now.", {
                ...
                toolCalls: [
                  createStatusTool(
                    ...
                    primaryMinion.statusUrl
                  ),
                ],
              }),
              createAssistantMessage("msg-3", primaryMinion.assistantText, {
                historySequence: 3,
                timestamp: STABLE_TIMESTAMP - 100_000,
                toolCalls: [
                  createStatusTool(
                    "call-2",
                    primaryMinion.statusEmoji,
                    primaryMinion.statusMessage,
                    primaryMinion.statusUrl
                  ),
                ],
              }),
          ...
          ...minionFixtures.slice(1).map((fixture, index) => {
            return [
              fixture.id,
              createStaticChatHandler([
...
// README: static/img/git-status.webp
...
// README: static/img/plan-mermaid.webp
export const PlanMermaidWithCosts: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() => {
        const minionId = "ws-plan-with-costs";
        ...
        window.localStorage.setItem(WORKBENCH_PANEL_TAB_KEY, JSON.stringify("costs"));
        window.localStorage.setItem("costsTab:viewMode", JSON.stringify("session"));
        window.localStorage.setItem(WORKBENCH_PANEL_WIDTH_KEY, "500");
        window.localStorage.removeItem(getWorkbenchPanelLayoutKey(minionId));
        ...
        const sessionUsage = new Map([[minionId, createMultiModelSessionUsage(1.84)]]);
...
2. Seed rich fixtures (multi-file diffs, multi-model costs, many minions)
...
// README: static/img/auto-mode.webp
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        ...
        return createMockORPCClient({
          projects: new Map([[README_PROJECT_PATH, { minions: [] }]]),
          minions: [],
          agentDefinitions: [
            {
              id: "exec",
...
// README: static/img/costs-tab.webp
export const CostsTabRich: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() => {
        const minionId = "ws-costs-rich";
        ...
        return createMockORPCClient({
                  ...
                  createAssistantMessage("msg-6", "Done — PR opened.", {
                    ...
                    toolCalls: [
                      createStatusTool(
                        ...
                        "🚀",
                        "PR #427 opened",
                        "https://github.com/latticeHQ/latticeWorkbench/pull/427"
                      ),
                    ],
                  }),
                ]),
          ...
          sessionUsage: new Map([[minionId, createMultiModelSessionUsage(2.31)]]),
        });
      }}
    />
  ),
...
// README: static/img/context-management.webp
...
export const ContextManagementDialog: AppStory = {
  render: () => (
    <AppWithMocks
      setup={() => {
        const minionId = "ws-context-management";
        ...
        return createMockORPCClient({
          ...
          sessionUsage: new Map([[minionId, createMultiModelSessionUsage(1.02)]]),
        });
      }}
    />
  ),
...
// README: static/img/mobile-server-mode.webp
// iPhone 17 Pro Max-sized minion view to document server-mode responsiveness.
export const MobileServerMode: AppStory = {
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        const minionId = "ws-mobile-server";
            ...
            new Map([
              [
                ...
                createStaticChatHandler([
                  createUserMessage(
                    "msg-1",
                    "Can I use lattice server mode from my phone when I’m away from my laptop?",
                    {
                      historySequence: 1,
                      timestamp: STABLE_TIMESTAMP - 45_000,
                    }
                  ...
                  createAssistantMessage(
                    "msg-2",
                    "Yes — the minion UI is responsive, so you can review progress and continue chats from mobile browsers.",
                    {
                      historySequence: 2,
                      timestamp: STABLE_TIMESTAMP - 30_000,
                      toolCalls: [
...
// README: static/img/orchestrate-agents.webp
// Parent minion is selected in plan mode while six running child minions
...
export const OrchestrateAgents: AppStory = {
  ...
  render: () => (
    <AppWithMocks
      setup={() => {
        const minionId = "ws-orchestrator";

        const parentMinion = createMinion({
          id: minionId,
        ...
        const childMinions = subtaskFixtures.map((fixture, index) => ({
          ...createMinion({
            id: fixture.id,
            name: fixture.name,
            projectName: README_PROJECT_NAME,
        ...
        const minions = [parentMinion, ...childMinions];

        window.localStorage.setItem(LEFT_SIDEBAR_COLLAPSED_KEY, JSON.stringify(false));
        expandProjects([README_PROJECT_PATH]);
        selectMinion(parentMinion);
        collapseWorkbenchPanel();
