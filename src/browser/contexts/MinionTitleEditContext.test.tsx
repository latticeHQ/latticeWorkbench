...
import { TitleEditProvider, useTitleEdit } from "./MinionTitleEditContext";
...
function isGeneratingTitle(
  ...
  return [...value.generatingTitleMinionIds].includes(minionId);
}

describe("MinionTitleEditContext", () => {
  afterEach(() => {
    cleanup();
  ...
  test("ignores duplicate regenerate requests while one is already in flight", async () => {
    ...
    await waitFor(() => expect(contextValue?.generatingTitleMinionIds.size).toBe(0));
  ...
  test("keeps loading state for other minions when one regeneration finishes", async () => {
    const testWindow = new GlobalWindow();
    globalThis.window = testWindow as unknown as Window & typeof globalThis;
    globalThis.document = testWindow.document as unknown as Document;
    globalThis.localStorage = testWindow.localStorage as unknown as Storage;
