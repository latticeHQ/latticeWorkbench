...
const LatticeChatHelpButton: React.FC<{
  ...
  const { isUnread: hasUnread } = useWorkspaceUnread(LATTICE_HELP_CHAT_WORKSPACE_ID);
...
const ProjectSidebarInner: React.FC<ProjectSidebarProps> = ({
  ...
  const LatticeLogo =
    theme === "dark" || theme.endsWith("-dark") ? LatticeLogoDark : LatticeLogoLight;
  ...
  const handleOpenLatticeChat = useCallback(() => {
    const meta = workspaceMetadata.get(LATTICE_HELP_CHAT_WORKSPACE_ID);
  ...
  // We still render the lattice-chat workspace as a dedicated pinned row above projects.
  const latticeChatMetadata = workspaceMetadata.get(LATTICE_HELP_CHAT_WORKSPACE_ID);
  const latticeChatProjectPath = latticeChatMetadata?.projectPath ?? null;
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (matchesKeybind(e, KEYBINDS.NEW_WORKSPACE) && selectedWorkspace) {
        e.preventDefault();
        if (selectedWorkspace.workspaceId === LATTICE_HELP_CHAT_WORKSPACE_ID) {
          return;
        }
              ...
              <div className="border-dark flex items-center justify-between border-b py-3 pr-3 pl-4">
                <div className="flex min-w-0 items-center gap-2">
                  <button
                    ...
                    aria-label="Open Chat with Lattice"
                  >
                    <LatticeLogo className="h-6 w-6" aria-hidden="true" />
                  </button>
                  {latticeChatMetadata && (
                    <>
                      <LatticeChatHelpButton
                        onClick={handleOpenLatticeChat}
                        isSelected={
                          selectedWorkspace?.workspaceId === LATTICE_HELP_CHAT_WORKSPACE_ID
                        }
                      />
                      <WorkspaceStatusIndicator workspaceId={LATTICE_HELP_CHAT_WORKSPACE_ID} />
                    </>
                  )}
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto">
                {visibleProjectPaths.length === 0 ? (
                  <div className="px-4 py-8 text-center">
                    <p className="text-muted mb-1 text-[13px]">No Headquarters yet</p>
                    <p className="text-muted mb-4 text-[12px] opacity-60">Add a repo to get started</p>
                    <button
                      onClick={onAddProject}
                    ...
                    return (
                      <div key={projectPath} className="border-hover border-b">
                        <DraggableProjectItem
                          ...
                          aria-label={`New mission in ${projectName}`}
                        ...
                        >
                          <button
                            ...
                            aria-label={`${isExpanded ? "Collapse" : "Expand"} HQ ${projectName}`}
                            data-project-path={projectPath}
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                ...
                                aria-label={`Manage secrets for ${projectName} HQ`}
                                data-project-path={projectPath}
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                onClick={(event) => {
                                  ...
                                  void (async () => {
                                    ...
                                    if (!result.success) {
                                      ...
                                      // If blocked by active workspaces, confirm and retry with force
                                      if (error.includes("active workspaces")) {
                                        const confirmed = window.confirm(
                                          `This HQ has active missions. Remove the HQ and all its missions?`
                                        );
                                        if (confirmed) {
                                          const forceResult = await onRemoveProject(
                                            projectPath,
                                            true
                                          );
                                          if (!forceResult.success) {
                                            const forceError =
                                              forceResult.error ?? "Failed to remove project";
                                            const rect = buttonElement.getBoundingClientRect();
                                            const anchor = {
                                              top: rect.top + window.scrollY,
                                              left: rect.right + 10,
                                ...
                                aria-label={`Remove HQ ${projectName}`}
                                data-project-path={projectPath}
                            ...
                            <TooltipContent align="end">Remove HQ</TooltipContent>
                          </Tooltip>
                          <button
                            onClick={(event) => {
                              event.stopPropagation();
                        ...
                        {isExpanded && (
                          <div
                            id={workspaceListId}
                            role="region"
                            aria-label={`Missions for ${projectName}`}
                            className="pt-1"
                          >
                            {(() => {
