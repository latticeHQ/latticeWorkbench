                  ...
                  {latticeChatMetadata && (
                    <>
                      <LatticeChatHelpButton
                        onClick={handleOpenLatticeChat}
                        isSelected={selectedWorkspace?.workspaceId === LATTICE_HELP_CHAT_WORKSPACE_ID}
                      />
                                    ...
                                    if (!result.success) {
                                      ...
                                      if (error.includes("active workspaces")) {
                                        const confirmed = window.confirm(
                                          `This project has active workspaces. Remove the project and all its workspaces?`
                                        );
                                        if (confirmed) {
                                          const forceResult = await onRemoveProject(projectPath, true);
                                          if (!forceResult.success) {
                                            const forceError = forceResult.error ?? "Failed to remove project";
                                            const rect = buttonElement.getBoundingClientRect();
                                            const anchor = {
                                              top: rect.top + window.scrollY,
                                              left: rect.right + 10,
                                            };
                                            projectRemoveError.showError(projectPath, forceError, anchor);
                                          }
                                          return;
                                        }
                                        return;
