...
import type { DisplayedMessage, LatticeMessage } from "@/common/types/message";
import type { ThinkingLevel } from "@/common/types/thinking";
import type { ChatLatticeMessage } from "@/common/orpc/types";
...
interface SidekickTranscriptDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  /** The minion that owns the transcript artifact index (usually the current minion). */
  minionId?: string;
  /** Child task/minion id whose transcript should be displayed. */
  taskId: string;
}

export const SidekickTranscriptDialog: React.FC<SidekickTranscriptDialogProps> = (props) => {
...
const SidekickTranscriptViewer: React.FC<{
  open: boolean;
  minionId?: string;
  taskId: string;
  ...
  const [messages, setMessages] = useState<LatticeMessage[] | null>(null);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const transcript = await api.minion.getSidekickTranscript({
          taskId,
          minionId,
        });
  ...
  }, [api, open, taskId, minionId, setModel, setThinkingLevel]);
  ...
  const displayedMessages: DisplayedMessage[] | null = useMemo(() => {
    ...
    // We intentionally do not pass minionId to the aggregator: it persists some UI state to localStorage.
    // We DO pass minionId to MessageRenderer so nested "View transcript" tool calls can resolve
    // artifacts from the parent minion that owns the transcript index (important after roll-up).
    const aggregator = new StreamingMessageAggregator(new Date().toISOString());
    aggregator.setShowAllMessages(true);

    for (const msg of messages) {
      const event: ChatLatticeMessage = { ...msg, type: "message" };
          ...
          displayedMessages.length > 0 ? (
            <div className="flex flex-col gap-2">
              {displayedMessages.map((msg) => (
                <MessageRenderer key={msg.id} message={msg} minionId={minionId} />
              ))}
            </div>
          ) : (
