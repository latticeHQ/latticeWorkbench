...
const FALLBACK_AGENTS: AgentDefinitionDescriptor[] = [
  ...
  {
    // Keep Ask visible when minion agent discovery is unavailable.
    id: "ask",
    scope: "built-in",
    name: "Ask",
  ...
  },
  {
    ...
    // get mislabeled as "Unknown agents" when minion discovery is unavailable.
    id: "orchestrator",
    scope: "built-in",
    name: "Orchestrator",
...
function getAgentDefinitionPath(agent: AgentDefinitionDescriptor): string | null {
  switch (agent.scope) {
    case "project":
      return `.lattice/agents/${agent.id}.md`;
    case "global":
      return `~/.lattice/agents/${agent.id}.md`;
    default:
      return null;
...
function areTaskSettingsEqual(a: TaskSettings, b: TaskSettings): boolean {
  return (
    ...
    a.planSidekickExecutorRouting === b.planSidekickExecutorRouting &&
    a.planSidekickDefaultsToOrchestrator === b.planSidekickDefaultsToOrchestrator &&
...
export function TasksSection() {
  ...
  const newMinionDefaultAgentId = coerceAgentId(globalDefaultAgentIdRaw);

  // Resolve the minion's active model so that when a sidekick's model is
  // "Inherit", we show thinking levels for the minion model (falling back to
  ...
  useEffect(() => {
    ...
    const minionId = selectedMinion?.minionId;
    if (!projectPath) {
      setAgents([]);
    ...
    void Promise.all([
      api.agents.list({ projectPath, minionId }),
      api.agents.list({ projectPath, minionId, includeDisabled: true }),
    ])
      .then(([enabled, all]) => {
        if (cancelled) return;
        setAgents(all);
  ...
  }, [api, selectedMinion?.projectPath, selectedMinion?.minionId]);
        ...
        void api.config
          ...
          .then(() => {
            ...
            if (agentDefaultsChanged) {
              ...
              const projectPath = selectedMinionRef.current?.projectPath;
              const minionId = selectedMinionRef.current?.minionId;
              if (!projectPath) {
                return;
              }
  ...
  const setPlanSidekickExecutorRouting = (value: string) => {
    if (!isPlanSidekickExecutorRouting(value)) {
      return;
    }
    ...
    setTaskSettings((prev) =>
      normalizeTaskSettings({
        ...prev,
        planSidekickExecutorRouting: value,
      })
    );
  };
  const setNewMinionDefaultAgentId = (agentId: string) => {
    setGlobalDefaultAgentIdRaw(coerceAgentId(agentId));
  };

  const planSidekickExecutorRouting: PlanSidekickExecutorRouting =
    taskSettings.planSidekickExecutorRouting ?? "exec";
  ...
  const newMinionDefaultAgentOptions = useMemo(() => {
    const options = uiAgents.map((agent) => ({
      id: agent.id,
      label: agent.name,
    }));

    if (!options.some((option) => option.id === newMinionDefaultAgentId)) {
      options.unshift({
        id: newMinionDefaultAgentId,
        label: `${newMinionDefaultAgentId} (unavailable)`,
      });
    }

    return options;
  }, [newMinionDefaultAgentId, uiAgents]);
  ...
  const internalAgents = useMemo(
    () =>
      [...listedAgents]
        .filter((agent) => !agent.uiSelectable && !agent.sidekickRunnable)
        .sort((a, b) => a.name.localeCompare(b.name)),
    [listedAgents]
  );
  ...
  const renderAgentDefaults = (agent: AgentDefinitionDescriptor) => {
    ...
    const enablementLocked =
      agent.id === "exec" || agent.id === "plan" || agent.id === "compact" || agent.id === "lattice";
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            <div className="text-muted text-xs">
              ...
              {agent.uiSelectable && agent.sidekickRunnable ? (
                <>
                  {" "}
                  •{" "}
                  <Tooltip>
  ...
  const renderUnknownAgentDefaults = (agentId: string) => {
    ...
    return (
      ...
      >
        ...
        <div className="text-muted text-xs">Not discovered in the current minion</div>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-4">
            <div className="flex-1">
              <div className="text-foreground text-sm">Default new-minion agent</div>
              <div className="text-muted text-xs">
                Applies when a project does not have its own agent preference yet.
              </div>
            </div>
            <Select
              value={newMinionDefaultAgentId}
              onValueChange={setNewMinionDefaultAgentId}
            >
              <SelectTrigger className="border-border-medium bg-background-secondary h-9 w-56">
                <SelectValue placeholder="Select agent" />
              </SelectTrigger>
              <SelectContent>
                {newMinionDefaultAgentOptions.map((option) => (
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <Select
              value={planSidekickExecutorRouting}
              onValueChange={setPlanSidekickExecutorRouting}
            >
      ...
      <div>
        ...
        <div className="text-muted text-xs">
          Defaults apply globally. Changing model/reasoning in a minion creates a minion
          override.
        </div>
        {agentsLoadFailed ? (
          <div className="text-danger-light mt-3 text-xs">
            Failed to load agent definitions for this minion.
          </div>
        ) : null}
      ...
      {sidekicks.length > 0 ? (
        <div>
          ...
          <div className="space-y-4">{sidekicks.map(renderAgentDefaults)}</div>
        </div>
      ) : null}
