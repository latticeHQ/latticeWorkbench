...
export function GovernorSection() {
  ...
  useEffect(() => {
    ...
    async function loadConfig() {
      try {
        ...
        setEnrolled(config.latticeGovernorEnrolled);
        setGovernorUrl(config.latticeGovernorUrl);
      } catch {
  ...
  useEffect(() => {
    return () => {
      ...
      if (isDesktop && apiRef.current && desktopFlowIdRef.current) {
        void apiRef.current.latticeGovernorOauth.cancelDesktopFlow({
  ...
  const handleStartEnroll = async () => {
    ...
    if (isDesktop) {
      ...
      const startResult = await api.latticeGovernorOauth.startDesktopFlow({ governorOrigin });
      ...
      const waitResult = await api.latticeGovernorOauth.waitForDesktopFlow({ flowId });
      ...
      // Popup must be opened synchronously before async fetch to avoid popup blockers
      const popup = window.open(
        "about:blank",
        "lattice-governor-oauth",
      ...
      const startUrl = new URL(`${backendBaseUrl}/auth/lattice-governor/start`);
      ...
      function isGovernorOAuthMessage(data: unknown): data is GovernorOAuthMessage {
        return (
          typeof data === "object" &&
          data !== null &&
          (data as GovernorOAuthMessage).type === "lattice-governor-oauth"
        );
      }
  ...
  const handleUnenroll = async () => {
    ...
    try {
      await api.config.unenrollLatticeGovernor();
      setEnrolled(false);
      setGovernorUrl(null);
      setRefreshPolicyError(null);
    } catch (error) {
  ...
  if (loadingConfig) {
    return (
      <div className="space-y-4">
        <h2 className="text-lg font-semibold">Lattice Governor</h2>
      ...
      {enrolled ? (
        ...
        <div className="space-y-4">
          <p className="text-muted-foreground text-sm">
            You are enrolled in Lattice Governor for enterprise policy delivery.
          </p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="destructive" size="sm" onClick={() => void handleUnenroll()}>
              Unenroll from Lattice Governor
            </Button>
          </div>
        ...
        <div className="space-y-4">
          <p className="text-muted-foreground text-sm">
            Lattice Governor enables enterprise policy delivery for centralized agent control. Enroll to
            connect to your organization&apos;s Governor server.
          </p>
          <Button onClick={handleOpenUrlDialog}>
            <ExternalLink className="mr-2 h-4 w-4" />
            Enroll in Lattice Governor
          </Button>
        </div>
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (isDesktop && desktopFlowId && api) {
                void api.latticeGovernorOauth.cancelDesktopFlow({ flowId: desktopFlowId });
              }
              setEnrollStatus("idle");
      ...
      <Dialog open={showUrlDialog} onOpenChange={setShowUrlDialog}>
        <DialogContent>
          ...
          <div className="space-y-4 pt-4">
            <p className="text-muted-foreground text-sm">
              Enter the URL of your organization&apos;s Lattice Governor server.
            </p>
            <Input
