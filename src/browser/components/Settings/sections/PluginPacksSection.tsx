    ...
    <div className="border-border border-b last:border-b-0">
      ...
      {expanded && (
        <div className="text-muted mb-3 ml-2 space-y-1 text-xs">
          ...
          {pack.mcpServerCount > 0 && (
            <div>Suggested MCP servers: {pack.mcpServerCount}</div>
          )}
        </div>
      )}
    </div>
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-muted mt-1 text-sm">
          Enable domain-specific skill packs to extend your workspace with
          specialized knowledge and commands.{" "}
          {enabledCount > 0
      ...
      <p className="text-muted text-xs">
        Click a plugin name to see details. Enabled plugins add domain-specific
        skills to the agent context in new conversations.
      </p>
    </div>
  );
}
