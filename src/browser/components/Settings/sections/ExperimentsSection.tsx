  ...
  if (!api) {
    return (
      <div className="bg-background-secondary px-4 py-3">
        <div className="text-muted text-xs">Connect to lattice to configure this setting.</div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-background-secondary space-y-4 px-4 py-3">
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <div className="text-muted text-xs">
              Use a fixed port to avoid changing URLs each time lattice restarts
            </div>
          </div>
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="text-foreground text-sm">Serve lattice web UI</div>
            <div className="text-muted text-xs">
              Serve the lattice web interface at / (browser mode)
            </div>
          </div>
          <Switch
            checked={serveWebUi}
            onCheckedChange={(value) => setServeWebUi(value)}
            aria-label="Toggle serving lattice web UI"
          />
        </div>
      ...
      {status && (
        <div className="space-y-2">
          ...
          {status.configuredServeWebUi ? (
            ...
            <div className="text-muted text-xs">
              Web UI serving is disabled (enable “Serve lattice web UI” and Apply to access /).
            </div>
          )}
