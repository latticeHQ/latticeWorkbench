  ...
  const startLogin = useCallback(async () => {
    ...
    try {
      ...
      if (!api) {
        ...
        setLoginError("Lattice API not connected.");
        return;
      }
          ...
          <>
            ...
            <div className="space-y-2">
              {loading ? (
                ...
                <>
                {/* Built-in servers (read-only) */}
                {Object.entries(servers).some(([, e]) => e.builtin) && (
                  <div className="mb-4 space-y-2">
                    <p className="text-muted text-xs font-medium tracking-wider uppercase">Built-in</p>
                    {Object.entries(servers)
                      ...
                      .map(([name, entry]) => (
                        ...
                        >
                          ...
                          {testCache[name]?.result.success && (
                            <div className="border-border-medium border-t px-3 py-1.5">
                              <span className="text-xs text-green-500">
                                {testCache[name].result.success && testCache[name].result.tools.length} tools available
                              </span>
                            </div>
                          )}
                          {testCache[name] && !testCache[name].result.success && (
                            <div className="border-border-medium border-t px-3 py-2 text-xs">
                              <div className="text-destructive flex items-start gap-1.5">
                                ...
                                <span>{!testCache[name].result.success && testCache[name].result.error}</span>
                              </div>
                            </div>
                          )}
                        </div>
                ...
                {Object.entries(servers).some(([, e]) => !e.builtin) && Object.entries(servers).some(([, e]) => e.builtin) && (
                  <p className="text-muted text-xs font-medium tracking-wider uppercase">User Configured</p>
                )}
                {Object.entries(servers).filter(([, e]) => !e.builtin).map(([name, entry]) => {
