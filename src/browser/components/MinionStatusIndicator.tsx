...
export const MinionStatusIndicator = memo<{
  minionId: string;
  fallbackModel: string;
  /** When true the minion is still being provisioned (show "starting…"). Passed as
   *  a prop so this component doesn't need to subscribe to the full MinionContext. */
  isCreating?: boolean;
}>(({ minionId, fallbackModel, isCreating }) => {
  ...
  if (awaitingUserQuestion) {
    return (
      <div className="bg-plan-mode-alpha text-plan-mode-light flex min-w-0 items-center gap-1.5 rounded px-1.5 py-0.5 text-xs">
        ...
        <span className="min-w-0 truncate font-medium">Lattice has a few questions</span>
      </div>
    );
  }
...
MinionStatusIndicator.displayName = "MinionStatusIndicator";
