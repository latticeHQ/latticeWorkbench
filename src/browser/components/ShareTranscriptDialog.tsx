...
interface ShareTranscriptDialogProps {
  minionId: string;
  minionName: string;
  /** Human-readable minion title shown in the dialog header */
  minionTitle?: string;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

function getTranscriptFileName(minionName: string): string {
  const trimmed = minionName.trim();
  if (!trimmed) {
    return "chat.jsonl";
  }

  // Keep this consistent with existing share filename sanitization.
  // (lattice.md expects `FileInfo.name` to be safe for display and download.)
...
function transcriptContainsProposePlanToolCall(messages: LatticeMessage[]): boolean {
...
export function ShareTranscriptDialog(props: ShareTranscriptDialogProps) {
  ...
  // Guards against cross-minion leakage when users switch minions mid-upload.
  ...
  useEffect(() => {
    ...
    setIsDeleting(false);
  }, [clearSharedTranscriptState, props.minionId]);
  ...
  const handleGenerateLink = useCallback(async () => {
    ...
    try {
      ...
      const minionState = store.getMinionState(minionId);

      let planSnapshot: { path: string; content: string } | undefined;
      if (api && transcriptContainsProposePlanToolCall(minionState.latticeMessages)) {
        try {
      ...
      const chatJsonl = buildChatJsonlForSharing(minionState.latticeMessages, {
        includeToolOutput,
        minionId,
        planSnapshot,
      });
      ...
      const sendOptions = getSendOptionsFromStorage(minionId);

      // Prefer the user-facing minion title for uploaded filename when available.
      const fileInfo: FileInfo = {
        name: getTranscriptFileName(props.minionTitle ?? props.minionName),
        type: "application/x-ndjson",
        size: new TextEncoder().encode(chatJsonl).length,
        model: minionState.currentModel ?? sendOptions.model,
        thinking: minionState.currentThinkingLevel ?? sendOptions.thinkingLevel,
      };

      const result = await uploadToLatticeMd(chatJsonl, fileInfo, {
        expiresAt: (() => {
  ...
  const handleUpdateExpiration = async (value: ExpirationValue) => {
    ...
    // link is generated (minion switch or re-upload) before this resolves.
    ...
    try {
      ...
      const newExpiration = await updateLatticeMdExpiration(shareId, shareMutateKey, expiresAtArg);
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      await deleteFromLatticeMd(shareId, shareMutateKey);
      if (uploadSeqRef.current === seq) {
        clearSharedTranscriptState();
      }
    } catch (err) {
    ...
    <Dialog open={props.open} onOpenChange={handleOpenChange}>
      <DialogContent maxWidth="380px" showCloseButton>
        <DialogHeader>
          ...
          {props.minionTitle && (
            <p className="text-muted truncate text-xs">{props.minionTitle}</p>
          )}
        </DialogHeader>
