/**
 * ProjectHQOverview — 2-D AWS-architecture-diagram canvas with animated flows.
 *
 * Layout: 12-col CSS grid — sections span 3/4/6/8/12 cols → dynamic 2-D mix.
 * Connections: SVG overlay measured via ResizeObserver, orthogonal bezier paths,
 *              animated dashed flow + moving dot + gradient glow on active edges.
 * All nodes clickable (navigate). Section headers collapsible.
 */
import React, {
  useMemo,
  useCallback,
...
// ─────────────────────────────────────────────────────────────────────────────
// SVG flow canvas — animated connections between section zones
// ─────────────────────────────────────────────────────────────────────────────
...
/** Animated SVG overlay drawn after CSS grid layout is measured */
function ConnectionCanvas({
  ...
  return (
    ...
    >
      <defs>
        <style>{`
          @keyframes hqDashFlow {
            to { stroke-dashoffset: -36; }
          }
          @keyframes hqDashFlowActive {
            to { stroke-dashoffset: -28; }
          }
          .hq-flow { animation: hqDashFlow 2.4s linear infinite; }
          .hq-flow-active { animation: hqDashFlowActive 1s linear infinite; }
        `}</style>
        ...
        {edges.filter(e => e.active).map(e => (
          ...
          >
            <stop offset="0%"   stopColor={e.color} stopOpacity="0.25" />
            <stop offset="50%"  stopColor={e.color} stopOpacity="0.9" />
            <stop offset="100%" stopColor="var(--color-exec-mode)" stopOpacity="0.8" />
          </linearGradient>
        ))}
        ...
        {edges.map(e => (
          ...
          >
            <path
              d="M0,0 L6,3 L0,6 Z"
              fill={e.active ? e.color : "hsl(var(--border))"}
              opacity={e.active ? 0.85 : 0.4}
            />
          </marker>
        ))}
      </defs>
      ...
      {edges.map(edge => {
        ...
        // Bezier path — S-curve horizontally, smooth vertically
        ...
        return (
          <g key={edge.id}>
            ...
            {edge.active && (
              <>
                ...
                <path
                  ...
                  strokeLinecap="round"
                  className="hq-flow-active"
                  markerEnd={`url(#arr-${edge.id})`}
                />
              </>
            )}

            {/* Idle animated dashes */}
            {!edge.active && (
              <path
                d={d}
                fill="none"
            ...
            {/* Moving dot (active only) */}
            {edge.active && (
              <circle r="3.5" fill={edge.color} opacity="0.9" filter="none">
            ...
            {/* Faint idle dot */}
            {!edge.active && (
              <circle r="2.5" fill="hsl(var(--border))" opacity="0.5">
...
// ─────────────────────────────────────────────────────────────────────────────
// Zone group (section container) — now accepts a nodeRef for edge measurement
// ─────────────────────────────────────────────────────────────────────────────
function ZoneGroup({
  ...
  return (
    <div
      ref={nodeRef}
      className={cn(colSpanClass(workspaces.length), "relative rounded-xl transition-all duration-200", empty && "opacity-40")}
      style={{
        border: `2px dashed ${color}45`,
        background: `${color}04`,
        boxShadow: active ? `0 0 0 1px ${color}20, 0 8px 24px ${color}08` : undefined,
        zIndex: 1, // above SVG overlay
      }}
    >
      <button
        type="button" onClick={onToggle}
        className="group/zlbl flex w-full items-center gap-2 px-3 py-2 rounded-t-xl hover:bg-white/4 active:bg-white/6 transition-colors cursor-pointer focus-visible:outline-none"
        ...
        <div className="p-2.5">
          {empty ? (
            <div className="flex items-center justify-center py-5">
              <span className="text-[9.5px]" style={{ color: `${color}30` }}>— no missions —</span>
            </div>
          ) : (
...
function MetricsBar({ totalMissions, activeMissions, totalSubAgents, stageCount, cliIds }: {
  ...
  return (
    <div className="flex items-center gap-4 px-4 py-2.5 rounded-xl border border-border/25 bg-background-secondary/50 flex-wrap text-[10.5px]">
      ...
      {totalSubAgents > 0 && (
        <span className="text-muted/45 shrink-0"><strong className="text-foreground/65">{totalSubAgents}</strong> sub-agents</span>
      )}
      <span className="text-muted/45 shrink-0"><strong className="text-foreground/65">{stageCount}</strong> zones</span>
      {cliIds.length > 0 && (
        <>
...
export function ProjectHQOverview({
  ...
  useLayoutEffect(() => {
    ...
    if (!canvas || sections.length < 2) return;
    ...
    const measure = () => {
      ...
      for (let i = 0; i < sections.length - 1; i++) {
        ...
        const fromActive = (workspacesBySection.get(from.id) ?? []).some(w => w.taskStatus === "running");
        const toActive   = (workspacesBySection.get(to.id)   ?? []).some(w => w.taskStatus === "running");
        const active = fromActive || toActive;

        // Same row: right → left, center-to-center vertically
        // Different row: bottom-center → top-center
  ...
  return (
    ...
    >
      ...
      {/* ── 2-D zone grid with SVG flow overlay ── */}
      {sections.length > 0 && (
        <div ref={canvasRef} className="relative">
          {/* Animated SVG connections (below zone cards) */}
          <ConnectionCanvas edges={edges} width={canvasSize.w} height={canvasSize.h} />

          {/* Zone cards grid (above SVG) */}
          <div className="grid grid-cols-12 gap-3 items-start relative" style={{ zIndex: 1 }}>
            {sections.map((sec, i) => (
              <ZoneGroup
                key={sec.id}
                section={sec}
