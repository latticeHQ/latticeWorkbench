...
export const FileViewerTab: React.FC<FileViewerTabProps> = (props) => {
  ...
  const initialCached = React.useMemo(() => {
    ...
    const cached = getCachedFileContent(props.minionId, props.relativePath);
    if (!cached) return null;
    return { data: cacheToResult(cached), diff: cached.diff ?? null };
  }, [props.minionId, props.relativePath]);
    ...
    async function fetchFile() {
      try {
        ...
        if (!bashResult.success && !bashResult.output) {
          ...
          removeCachedFileContent(props.minionId, props.relativePath);
        ...
        setCachedFileContent(props.minionId, props.relativePath, data, diff);
  ...
  }, [api, props.minionId, props.relativePath, refreshCounter]);
