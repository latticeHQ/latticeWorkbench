...
// Cache key builders
const entriesCacheKey = (minionId: string) => `explorer:entries:${minionId}`;
const expandedCacheKey = (minionId: string) => `explorer:expanded:${minionId}`;
...
export const ExplorerTab: React.FC<ExplorerTabProps> = (props) => {
  ...
  const [expandedPaths, setExpandedPaths] = usePersistedState<string[]>(
    expandedCacheKey(props.minionId),
    [],
    { listener: true }
  );
  ...
  const [state, setState] = React.useState<ExplorerState>(() => {
    const cached = readPersistedState<Record<string, FileTreeNode[]>>(
      entriesCacheKey(props.minionId),
      {}
    );
    return {
  ...
  React.useEffect(() => {
    ...
    updatePersistedState(entriesCacheKey(props.minionId), obj, {});
  }, [state.entries, props.minionId]);
  ...
  const fetchDirectory = React.useCallback(
    async (relativePath: string, suppressErrors = false): Promise<FileTreeNode[] | null> => {
      ...
      try {
        ...
        const lsPromise = api.minion.executeBash({
        ...
        if (dirsToCheck.length > 0) {
          const checkIgnoreResult = await api.minion.executeBash({
            minionId: props.minionId,
    ...
    [api, props.minionId, setExpandedPaths]
  );
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = minionStore.subscribeFileModifyingTool(() => {
  ...
  }, [props.minionId, expandedSet, fetchDirectory]);
  ...
  // Shorten minion path for display (replace home dir with ~)
  ...
  const displayPath = shortenPath(props.minionPath);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="border-border-light flex items-center gap-1 border-b px-2 py-1">
        ...
        <Tooltip>
          ...
          <TooltipContent side="bottom">{props.minionPath}</TooltipContent>
        </Tooltip>
