...
export function useReadMore(options: UseReadMoreOptions): UseReadMoreResult {
  const { hunk, hunkId, minionId, diffBase, includeUncommitted } = options;
  const { api } = useAPI();
  ...
  useEffect(() => {
    ...
    void readFileLines(api, minionId, hunk.filePath, startLine, endLine, gitRef).then(
      (lines) => {
        if (cancelled) return;
        setUpLoading(false);
        if (lines) {
  ...
  }, [api, readMore.up, hunk.oldStart, hunk.filePath, minionId, gitRef]);
  ...
  }, [api, readMore.down, hunk.oldStart, hunk.oldLines, hunk.filePath, minionId, gitRef]);
