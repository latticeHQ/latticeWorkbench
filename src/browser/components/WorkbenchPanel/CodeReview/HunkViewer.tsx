...
export const HunkViewer = React.memo<HunkViewerProps>(
  ({
    ...
    // Persist manual expand/collapse state across remounts per minion
    ...
    const [expandStateMap, setExpandStateMap] = usePersistedState<Record<string, boolean>>(
      getReviewExpandStateKey(minionId),
      {},
      { listener: true }
    );
    ...
    const {
      ...
      handleCollapseUp,
      handleCollapseDown,
    } = useReadMore({ hunk, hunkId, minionId, diffBase, includeUncommitted });
