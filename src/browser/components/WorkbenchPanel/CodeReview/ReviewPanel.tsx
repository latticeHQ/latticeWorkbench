 ...
 * Displays diff hunks for viewing changes in the minion
 *
 ...
 *
 * Note: Parent uses key={minionId} so component remounts on minion change,
 * guaranteeing fresh state. No need to track minionId in state.
 */
type DiffState =
  | { status: "loading" }
...
async function ensureOriginFetched(params: {
  ...
  const key = [params.minionId, params.diffBase, String(params.refreshToken)].join("\u0000");
  ...
  const promise = params.api.minion
    .executeBash({
...
function makeReviewPanelCacheKey(params: {
  ...
  return [params.minionId, params.minionPath, params.gitCommand].join("\u0000");
}
...
async function executeMinionBashAndCache<T extends ReviewPanelCacheValue>(params: {
  api: APIClient;
  minionId: string;
  script: string;
  cacheKey: string;
  ...
  const result = await params.api.minion.executeBash({
    minionId: params.minionId,
...
export const ReviewPanel: React.FC<ReviewPanelProps> = ({
  ...
  // Note: Parent renders with key={minionId}, so component remounts on minion change.
  const [diffState, setDiffState] = useState<DiffState>({ status: "loading" });

  // Persist selected hunk per minion so navigation survives tab switches
  const [selectedHunkId, setSelectedHunkId] = usePersistedState<string | null>(
    `review-selected-hunk:${minionId}`,
    null
  );
  ...
  const skipCacheOnMountRef = useRef(
    minionStore.getFileModifyingToolMs(minionId) !== undefined
  );

  // Unified search state (per-minion persistence)
  const [searchState, setSearchState] = usePersistedState<ReviewSearchState>(
    getReviewSearchStateKey(minionId),
    { input: "", useRegex: false, matchCase: false }
  );
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState("");

  // Persist file filter per minion
  const [selectedFilePath, setSelectedFilePath] = usePersistedState<string | null>(
    `review-file-filter:${minionId}`,
    null
  );

  // Per-project default base (shared across minions in the same project)
  ...
  const { isRead, toggleRead, markAsRead, markAsUnread } = useReviewState(minionId);
  ...
  const { recordFirstSeen, firstSeenMap } = useHunkFirstSeen(minionId);
  ...
  // Immersive review mode - persisted so MinionShell overlay can react
  ...
  useEffect(() => {
    ...
    const unsubscribe = minionStore.subscribeFileModifyingTool((wsId) => {
      if (wsId === minionId) {
        controller.schedule();
      }
    });
    ...
    const lastToolMs = minionStore.getFileModifyingToolMs(minionId);
    if (lastToolMs && lastToolMs > lastFetchTimeRef.current) {
      controller.requestImmediate();
      minionStore.clearFileModifyingToolMs(minionId);
    }
  ...
  // Load file tree - when minion, diffBase, or refreshTrigger changes
  useEffect(() => {
    // Skip data loading while minion is being created
    ...
    const numstatMarker = "__LATTICE_REVIEW_FILE_TREE_NUMSTAT__";
    const nameStatusMarker = "__LATTICE_REVIEW_FILE_TREE_NAME_STATUS__";
    ...
    // Fast path: use cached tree when switching minions (unless user explicitly refreshed
    ...
    const loadFileTree = async () => {
      ...
      try {
        ...
        const tree = await executeMinionBashAndCache({
          api,
          minionId,
  ...
  // Load diff hunks - when minion, diffBase, selected path, or refreshTrigger changes
  useEffect(() => {
    // Skip data loading while minion is being created
    if (!api || isCreating) return;
    let cancelled = false;
    ...
    // Fast path: use cached diff when switching minions (unless user explicitly refreshed
    // or tools completed while we were unmounted).
    if (!isManualRefresh && !skipCacheOnMountRef.current) {
    ...
    const loadDiff = async () => {
      try {
        ...
        const data = await executeMinionBashAndCache({
          api,
          minionId,
          script: diffCommand,
          cacheKey,
  ...
  return (
    ...
    >
            ...
            <div className="flex flex-[0_0_auto] flex-col p-3">
              {hunks.length === 0 ? (
                ...
                filteredHunks.map((hunk) => {
                  ...
                  // Default to now for hunks without first-seen (e.g., old lattice versions)
      ...
      {/* Immersive review mode: render into minion overlay */}
      {isImmersive &&
        (() => {
          const root =
