/**
 * CommodityView — Commodity prices dashboard.
 * Futures tickers: Gold, Silver, Oil, Copper, Natural Gas.
...
const COMMODITY_KEYS = Object.keys(COMMODITY_SYMBOLS);
...
export const CommodityView: React.FC<CommodityViewProps> = ({ baseUrl }) => {
  ...
  const quotes = useQuotes(COMMODITY_KEYS, baseUrl);
        ...
        <div className="h-full overflow-hidden">
          {priceHistory.loading && !priceHistory.data ? (
            ...
            <Suspense fallback={<div className="flex h-full items-center justify-center"><RefreshCw className="h-5 w-5 animate-spin text-neutral-600" /></div>}>
              <CandlestickChart data={priceHistory.data} symbol={COMMODITY_SYMBOLS[selected] ?? selected} />
            </Suspense>
          ) : (
        ...
        <div className="h-full overflow-auto">
          ...
          <table className="w-full text-xs">
            <thead className="sticky top-0 z-10 bg-[#0a0a0a]">
              <tr className="border-b border-neutral-800 text-left">
                <th className="px-2 py-1.5 font-mono font-medium text-neutral-400">Commodity</th>
            ...
            <tbody>
              {COMMODITY_KEYS.map((sym) => {
                const name = COMMODITY_SYMBOLS[sym];
          ...
          <div className="border-t border-neutral-800 p-2">
            <div className="mt-1 text-[10px] text-neutral-600">
              Track any commodity symbol via the Equity view or add to your Watchlist.
            </div>
          </div>
        </div>
      </Panel>
