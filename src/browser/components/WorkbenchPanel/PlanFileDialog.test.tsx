...
describe("PlanFileDialog", () => {
  ...
  test("fetches plan content only after the dialog opens", async () => {
    ...
    const view = render(
      <PlanFileDialog open={false} onOpenChange={onOpenChange} minionId="minion-1" />
    );

    expect(getPlanContent).toHaveBeenCalledTimes(0);

    view.rerender(<PlanFileDialog open onOpenChange={onOpenChange} minionId="minion-1" />);
  ...
  test("shows API error responses in the dialog", async () => {
    ...
    const view = render(
      <PlanFileDialog open onOpenChange={() => undefined} minionId="minion-2" />
    );
  ...
  test("renders API-unavailable state when not connected", async () => {
    ...
    const view = render(
      <PlanFileDialog open onOpenChange={() => undefined} minionId="minion-3" />
    );
