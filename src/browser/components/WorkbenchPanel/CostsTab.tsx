...
import { useMinionUsage, useMinionConsumers } from "@/browser/stores/MinionStore";
...
const CostsTabComponent: React.FC<CostsTabProps> = ({ minionId }) => {
  const usage = useMinionUsage(minionId);
  const consumers = useMinionConsumers(minionId);
  ...
  const postCompactionState = usePostCompactionState(minionId);

  // Get runtimeConfig for SSH-aware editor opening
  const minionContext = useOptionalMinionContext();
  const runtimeConfig = minionContext?.minionMetadata.get(minionId)?.runtimeConfig;
  ...
  const { threshold: autoCompactThreshold, setThreshold: setAutoCompactThreshold } =
    useAutoCompactionSettings(minionId, contextDisplayModel);
...
// Only re-renders when minionId changes or internal hook data (usage/consumers) updates
