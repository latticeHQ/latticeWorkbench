...
// Colors for timing components — distinct hues for visual clarity
const TIMING_COLORS = {
  ttft: "#FBBF24", // amber-400 — waiting for first token
  model: "#38BDF8", // sky-400 — model inference
  tools: "#A78BFA", // violet-400 — tool execution
} as const;
...
export function StatsTab(props: StatsTabProps) {
  ...
  const onClearStats =
    ...
    (async (): Promise<void> => {
      ...
      await client.minion.stats.clear({ minionId: props.minionId });
    });
  ...
  const handleClearStats = async (): Promise<void> => {
    ...
    try {
      ...
      console.warn(`[StatsTab] Failed to clear stats for ${props.minionId}:`, error);
