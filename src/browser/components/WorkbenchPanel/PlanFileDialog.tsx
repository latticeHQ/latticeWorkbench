...
export const PlanFileDialog: React.FC<PlanFileDialogProps> = (props) => {
  ...
  useEffect(() => {
    ...
    assert(props.minionId.trim().length > 0, "minionId is required to load plan preview");
    ...
    const run = async () => {
      try {
        const result = await api.minion.getPlanContent({ minionId: props.minionId });
