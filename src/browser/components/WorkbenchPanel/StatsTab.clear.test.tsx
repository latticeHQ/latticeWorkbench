...
describe("StatsTab clear", () => {
  ...
  test("renders inline error when minion.stats.clear rejects", async () => {
    const minionId = "minion-1";

    const snapshot: MinionStatsSnapshot = {
      minionId,
    ...
    const view = render(
      <StatsTab minionId={minionId} _snapshot={snapshot} _clearStats={() => clearPromise} />
    );
