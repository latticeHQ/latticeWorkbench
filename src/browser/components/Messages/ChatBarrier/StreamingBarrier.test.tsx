...
function createMinionState(overrides: Partial<MockMinionState> = {}): MockMinionState {
  return {
    canInterrupt: true,
    isCompacting: false,
...
let currentMinionState = createMinionState();
let hasInterruptingStream = false;
const setInterrupting = mock((_minionId: string) => undefined);
...
void mock.module("@/browser/stores/MinionStore", () => ({
  useMinionState: () => currentMinionState,
  useMinionAggregator: () => ({
...
describe("StreamingBarrier", () => {
  beforeEach(() => {
    ...
    currentMinionState = createMinionState();
  ...
  test("clicking stop during normal streaming interrupts with default options", () => {
    ...
    const view = render(<StreamingBarrier minionId="ws-1" />);

    fireEvent.click(view.getByRole("button", { name: "Stop streaming" }));

    expect(setAutoRetryEnabled).toHaveBeenCalledWith({ minionId: "ws-1", enabled: false });
    expect(setInterrupting).toHaveBeenCalledWith("ws-1");
    expect(interruptStream).toHaveBeenCalledWith({ minionId: "ws-1" });
  });
  ...
  test("shows vim interrupt shortcut when vim mode is enabled", () => {
    ...
    const view = render(<StreamingBarrier minionId="ws-1" vimEnabled />);
  ...
  test("clicking stop during compaction uses onCancelCompaction when provided", () => {
    ...
    const view = render(
      <StreamingBarrier minionId="ws-1" onCancelCompaction={onCancelCompaction} />
    );
