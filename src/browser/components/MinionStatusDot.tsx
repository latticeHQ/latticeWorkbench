...
export const MinionStatusDot = memo<{
  minionId: string;
  lastReadTimestamp?: number;
  onClick?: (e: React.MouseEvent) => void;
  size?: number;
}>(
  ({ minionId, lastReadTimestamp, onClick, size = 8 }) => {
    const {
      canInterrupt,
      isStarting,
  ...
  (prevProps, nextProps) =>
    prevProps.minionId === nextProps.minionId &&
    prevProps.lastReadTimestamp === nextProps.lastReadTimestamp &&
    prevProps.onClick === nextProps.onClick &&
    prevProps.size === nextProps.size
);
MinionStatusDot.displayName = "MinionStatusDot";
