import { useTitleEdit } from "@/browser/contexts/MinionTitleEditContext";
...
import { MinionActionsMenuContent } from "./MinionActionsMenuContent";
import { useAPI } from "@/browser/contexts/API";

export interface MinionSelection {
  projectPath: string;
  projectName: string;
  namedMinionPath: string; // Worktree path (directory uses minion name)
  minionId: string;
}

/** Props for draft minion rendering (UI-only placeholders) */
export interface DraftMinionData {
  draftId: string;
  draftNumber: number;
...
/** Base props shared by both minion and draft items */
interface MinionListItemBaseProps {
  projectPath: string;
  isSelected: boolean;
  depth?: number;
}

/** Props for regular (persisted) minion items */
export interface MinionListItemProps extends MinionListItemBaseProps {
  variant?: "minion";
  ...
  onCancelCreation: (minionId: string) => Promise<void>;
}

/** Props for draft (UI-only placeholder) items */
export interface DraftMinionListItemProps extends MinionListItemBaseProps {
  variant: "draft";
  draft: DraftMinionData;
}
...
/** Container styles shared between minion and draft items */
...
// ─────────────────────────────────────────────────────────────────────────────
// Draft Minion Item (UI-only placeholder)
// ─────────────────────────────────────────────────────────────────────────────

function DraftMinionListItemInner(props: DraftMinionListItemProps) {
...
// ─────────────────────────────────────────────────────────────────────────────
// Regular Minion Item (persisted minion)
// ─────────────────────────────────────────────────────────────────────────────

function RegularMinionListItemInner(props: MinionListItemProps) {
  const {
    metadata,
    projectPath,
    projectName,
  ...
  const { id: minionId, namedMinionPath } = metadata;
  ...
  const { isUnread } = useMinionUnread(minionId);
  ...
  const isGeneratingTitle = generatingTitleMinionIds.has(minionId);
  const { api } = useAPI();
  ...
  // Display title (fallback to name for legacy minions without title)
  const displayTitle = metadata.title ?? metadata.name;
  const isEditing = editingMinionId === minionId;
  ...
  // SHARE_TRANSCRIPT keybind is handled in MinionMenuBar (always mounted),
  // so it works even when the sidebar is collapsed and list items are unmounted.

  const startEditing = () => {
    if (requestEdit(minionId, displayTitle)) {
      setEditingTitle(displayTitle);
      setTitleError(null);
    }
  };
  ...
  const handleConfirmEdit = async () => {
    ...
    const result = await confirmEdit(minionId, editingTitle);
    if (!result.success) {
  ...
  const { canInterrupt, awaitingUserQuestion, isStarting, agentStatus, terminalActiveCount } =
    useMinionSidebarState(minionId);

  const fallbackModel = useMinionFallbackModel(minionId);
  ...
  // Note: we intentionally render the secondary row even while the minion is still
  ...
  const [{ isDragging }, drag, dragPreview] = useDrag(
    () => ({
      ...
      item: (): MinionDragItem & { displayTitle?: string; runtimeConfig?: unknown } => ({
        type: MINION_DRAG_TYPE,
        minionId,
        projectPath,
    ...
    <React.Fragment>
      <div
        ...
        className={cn(
          ...
          // Keep hover styles enabled for initializing minions so the row feels interactive.
      ...
      >
        ...
        {/* Action button: cancel/delete spinner for initializing minions, overflow menu otherwise */}
        {isInitializing ? (
              ...
              <TooltipTrigger asChild>
                <button
                  ...
                  onClick={(e) => {
                    ...
                    void onCancelCreation(minionId);
                  }}
                  aria-label={
                    isRemoving
                      ? `Deleting minion ${displayTitle}`
            ...
            <ActionButtonWrapper hasSubtitle={hasStatusText}>
              ...
              <Popover open={ctxMenu.isOpen} onOpenChange={ctxMenu.onOpenChange}>
                ...
                <PopoverContent
                  ...
                  onClick={(e) => e.stopPropagation()}
                >
                  <MinionActionsMenuContent
                    onEditTitle={startEditing}
                    onForkChat={(anchorEl) => {
                      void onForkMinion(minionId, anchorEl);
                    }}
                    onShareTranscript={() => setShareTranscriptOpen(true)}
                    onArchiveChat={(anchorEl) => {
                      void onArchiveMinion(minionId, anchorEl);
                    }}
                    onCloseMenu={() => ctxMenu.close()}
                    linkSharingEnabled={linkSharingEnabled === true}
                    isLatticeHelpChat={isLatticeHelpChat}
                  />
                  <PositionedMenuItem
                    icon={<Sparkles />}
                    label="Generate new title"
                    shortcut={formatKeybind(KEYBINDS.GENERATE_MINION_TITLE)}
                    onClick={() => {
                      ctxMenu.close();
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="text-muted flex items-center gap-0.5">
                        <MinionTerminalIcon className="h-3 w-3" />
...
type UnifiedMinionListItemProps = MinionListItemProps | DraftMinionListItemProps;

function MinionListItemInner(props: UnifiedMinionListItemProps) {
  if (props.variant === "draft") {
    return <DraftMinionListItemInner {...props} />;
  }
  return <RegularMinionListItemInner {...props} />;
}

export const MinionListItem = React.memo(MinionListItemInner);
