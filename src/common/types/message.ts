...
export function rebuildContinueMessage(
  ...
  return buildContinueMessage({
    ...
    latticeMetadata: persisted.latticeMetadata,
...
export type LatticeToolPart = z.infer<typeof LatticeToolPartSchema>;

// Text part type
export interface LatticeTextPart {
  type: "text";
  text: string;
  timestamp?: number;
}
...
export interface LatticeReasoningPart {
  type: "reasoning";
  text: string;
  timestamp?: number;
  /**
...
export interface LatticeFilePart {
  type: "file";
...
// LatticeMessage extends UIMessage with our metadata and custom parts
// Supports text, reasoning, file, and tool parts (including interrupted tool calls)
export type LatticeMessage = Omit<UIMessage<LatticeMetadata, never, never>, "parts"> & {
  parts: Array<LatticeTextPart | LatticeReasoningPart | LatticeFilePart | LatticeToolPart>;
};
...
export type DisplayedMessage =
  | {
      ...
      historyId: string; // Original LatticeMessage ID for history operations
      content: string;
      /**
      ...
      /** Structured review data for rich UI display (from latticeMetadata) */
      reviews?: ReviewNoteDataForDisplay[];
    }
  | {
      type: "assistant";
  ...
  | {
      type: "plan-display"; // Ephemeral plan display from /plan command
      id: string; // Display ID for UI/React keys
      historyId: string; // Original LatticeMessage ID (same as id for ephemeral messages)
...
export function createLatticeMessage(
  id: string,
  role: "user" | "assistant",
  content: string,
  metadata?: LatticeMetadata,
  additionalParts?: LatticeMessage["parts"]
): LatticeMessage {
