...
export interface FlattenToolHookEnvOptions {
   ...
   * Hooks can use `LATTICE_TOOL_INPUT_PATH`/`LATTICE_TOOL_RESULT_PATH` for full JSON.
   */
 ...
 *
 * Example:
 *   flattenToolHookValueToEnv({ path: "a.ts" }, "LATTICE_TOOL_INPUT")
 *     -> { LATTICE_TOOL_INPUT_PATH: "a.ts", LATTICE_TOOL_INPUT_FILE_PATH: "a.ts" }
 */
export function flattenToolHookValueToEnv(
  ...
  // Guard: Our hooks already have env vars like LATTICE_TOOL_INPUT/LATTICE_TOOL_RESULT.
  ...
  function trySet(key: string, rawValue: string): void {
    ...
    // and let hooks use the JSON file (`LATTICE_TOOL_INPUT_PATH`/`LATTICE_TOOL_RESULT_PATH`).
