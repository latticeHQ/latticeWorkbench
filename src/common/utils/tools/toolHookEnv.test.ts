...
describe("flattenToolHookValueToEnv", () => {
  it("flattens a simple object", () => {
    expect(flattenToolHookValueToEnv({ script: "echo hi" }, "LATTICE_TOOL_INPUT")).toEqual({
      LATTICE_TOOL_INPUT_SCRIPT: "echo hi",
    });
  });
  ...
  it("flattens nested objects", () => {
    expect(
      ...
      LATTICE_TOOL_INPUT_OPTS_TIMEOUT: "30",
    });
  });
  ...
  it("flattens arrays and provides a _COUNT", () => {
    expect(
      ...
      LATTICE_TOOL_INPUT_ITEMS_COUNT: "2",
      LATTICE_TOOL_INPUT_ITEMS_0: "a",
      LATTICE_TOOL_INPUT_ITEMS_1: "b",
    });
  });
  ...
  it("flattens arrays of objects", () => {
    expect(
      ...
      LATTICE_TOOL_INPUT_ITEMS_COUNT: "2",
      LATTICE_TOOL_INPUT_ITEMS_0_NAME: "x",
      LATTICE_TOOL_INPUT_ITEMS_1_NAME: "y",
    });
  });
  ...
  it("does not emit a root env var for non-object values", () => {
    expect(flattenToolHookValueToEnv("hello", "LATTICE_TOOL_INPUT")).toEqual({});
  });
  ...
  it("emits legacy FILE_PATH alias when canonical PATH is present", () => {
    ...
    // LATTICE_TOOL_INPUT_FILE_PATH. The flattener emits it as an alias.
    expect(flattenToolHookValueToEnv({ path: "src/app.ts" }, "LATTICE_TOOL_INPUT")).toEqual({
      LATTICE_TOOL_INPUT_PATH: "src/app.ts",
      LATTICE_TOOL_INPUT_FILE_PATH: "src/app.ts",
    });
  });

  it("does not emit FILE_PATH alias when PATH is absent", () => {
    const result = flattenToolHookValueToEnv({ script: "echo hi" }, "LATTICE_TOOL_INPUT");
    expect(result).not.toHaveProperty("LATTICE_TOOL_INPUT_FILE_PATH");
  });
});
