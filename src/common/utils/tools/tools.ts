...
import { createAllBrowserTools } from "@/node/services/tools/browser";
...
export interface ToolConfiguration {
  ...
  /** LATTICE_ environment variables (LATTICE_PROJECT_PATH, LATTICE_RUNTIME) - set from init hook env */
...
export async function getToolsForModel(
  ...
  const nonRuntimeTools: Record<string, Tool> = {
    lattice_global_agents_read: createLatticeGlobalAgentsReadTool(config),
    lattice_global_agents_write: createLatticeGlobalAgentsWriteTool(config),
