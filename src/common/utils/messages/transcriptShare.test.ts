...
describe("buildChatJsonlForSharing", () => {
  it("strips tool output and sets state to output-redacted when includeToolOutput=false", () => {
    ...
    const parsed = JSON.parse(splitJsonlLines(jsonl)[0]) as LatticeMessage;
  ...
  it("inlines planContent even when propose_plan planPath uses ~ but planSnapshot.path is resolved", () => {
    ...
    const planSnapshot = {
      path: "/home/user/.lattice/plans/p/w.md",
      content: "# My Plan\n\n- Step 1",
    };
  ...
  it("inlines planContent even when propose_plan planPath uses Windows-style slashes", () => {
    ...
    const planSnapshot = {
      path: "C:\\Users\\user\\.lattice\\plans\\p\\w.md",
      content: "# My Plan\n\n- Step 1",
    };
  ...
  it("injects minionId into each message when provided", () => {
    const messages: LatticeMessage[] = [
      {
        id: "user-1",
        role: "user",
    ...
    const jsonl = buildChatJsonlForSharing(messages, { minionId: "ws-123" });
    const parsed = JSON.parse(splitJsonlLines(jsonl)[0]) as LatticeMessage & { minionId?: string };

    expect(parsed.minionId).toBe("ws-123");
    expect((messages[0] as LatticeMessage & { minionId?: string }).minionId).toBeUndefined();
  });
  ...
  it("produces valid JSONL (each line parses, trailing newline)", () => {
    ...
    const parsed = lines.map((line) => JSON.parse(line) as LatticeMessage);
        ...
        parts: [
          {
            ...
            toolName: "code_execution",
            state: "input-available",
            input: { code: "lattice.bash({ script: 'fail' })" },
            nestedCalls: [
              {
                toolCallId: "nested-1",
                toolName: "bash",
        ...
        parts: [
          {
            ...
            toolName: "code_execution",
            state: "input-available",
            input: { code: "lattice.bash({ script: 'echo ok' })" },
            nestedCalls: [
              {
                toolCallId: "nested-1",
                toolName: "bash",
