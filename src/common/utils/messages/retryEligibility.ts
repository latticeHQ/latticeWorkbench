 ...
 * Set in browser console: window.__LATTICE_FORCE_ALL_RETRYABLE = true
 *
 ...
 */
const FORCE_ALL_RETRYABLE_FLAG = "__LATTICE_FORCE_ALL_RETRYABLE";
...
function isForceAllRetryableEnabled(): boolean {
  const runtime = globalThis as typeof globalThis & {
    __LATTICE_FORCE_ALL_RETRYABLE?: boolean;
  };
...
export function isNonRetryableSendError(error: { type: string }): boolean {
  ...
  switch (error.type) {
    ...
    case "incompatible_minion": // Minion from newer lattice version - user must upgrade
    case "runtime_not_ready": // Container doesn't exist - user must recreate minion
...
function computeHasInterruptedStream(
  ...
  // Don't show retry barrier if minion init is still running AND no error has occurred yet.
  ...
  const initMessage = messages.find((m) => m.type === "minion-init");
  if (
    initMessage?.type === "minion-init" &&
  ...
  // Don't show retry barrier if runtime is still starting (e.g., Lattice minion waiting for startup scripts).
  // The backend's ensureReady() is still running - this happens when reconnecting to a stopped minion.
  ...
  // Don't show retry barrier for runtime_not_ready - requires minion recreation.
