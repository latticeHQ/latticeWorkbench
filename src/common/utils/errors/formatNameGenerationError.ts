  ...
  switch (error.type) {
    case "authentication": {
      ...
      if (error.authKind === "oauth_not_connected") {
        return {
          ...
          message: provider
            ? `Minion naming requires an OAuth connection for ${provider}.`
            : "OAuth connection required for minion naming.",
          hint: provider
    ...
    case "policy": {
      ...
      return {
        ...
        message: provider
          ? `An organization policy prevents using ${provider} for minion naming.`
