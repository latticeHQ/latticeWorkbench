...
export const WorkspaceMetadataSchema = z.object({
  ...
  projectName: z
    .string()
    .meta({ description: "Headquarter name extracted from project path (for display)" }),
  projectPath: z
    .string()
...
export const FrontendWorkspaceMetadataSchema = WorkspaceMetadataSchema.extend({
  ...
  incompatibleRuntime: z.string().optional().meta({
    description:
      "If set, this workspace has an incompatible runtime configuration (e.g., from a newer version of lattice). The workspace should be displayed but interactions should show this error message.",
  }),
...
export const WorkspaceActivitySnapshotSchema = z.object({
  recency: z.number().meta({ description: "Lattice ms timestamp of last user interaction" }),
