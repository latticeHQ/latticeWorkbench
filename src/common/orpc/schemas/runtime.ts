...
const bgOutputDirField = z
  .string()
  .optional()
  .meta({ description: "Directory for background process output (e.g., /tmp/lattice-bashes)" });
...
export const RuntimeConfigSchema = z.union([
  ...
  z.object({
    ...
    srcBaseDir: z
      .string()
      .meta({ description: "Base directory where all workspaces are stored (e.g., ~/.lattice/src)" }),
    bgOutputDir: bgOutputDirField,
  }),
  // SSH runtime
  z.object({
