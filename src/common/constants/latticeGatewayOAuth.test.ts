...
import { buildAuthorizeUrl, buildExchangeBody, LATTICE_GATEWAY_ORIGIN } from "./latticeGatewayOAuth";

describe("latticeGatewayOAuth", () => {
  test("buildAuthorizeUrl includes required params", () => {
    ...
    expect(url.origin).toBe(LATTICE_GATEWAY_ORIGIN);
  ...
  test("buildExchangeBody includes required fields", () => {
    ...
    expect(body.get("client_id")).toBe("lattice-client");
    expect(body.get("client_secret")).toBe("lattice-client");
  });
});
