...
/**
 * Preferred expiration for openagent.md shares (global)
...
/**
 * Get the localStorage key for the main area (PM Chat + employee terminals) layout per workspace.
 * Format: "main-area:layout:{workspaceId}"
 */
export function getMainAreaLayoutKey(workspaceId: string): string {
  return `main-area:layout:${workspaceId}`;
}

/**
 * Get the localStorage key for main area employee metadata per workspace.
 * Maps sessionId -> { slug, label } so tab labels survive page reload.
 * Format: "main-area:employee-meta:{workspaceId}"
 */
export function getMainAreaEmployeeMetaKey(workspaceId: string): string {
  return `main-area:employee-meta:${workspaceId}`;
}
