...
export const EXPERIMENT_IDS = {
  ...
  SYSTEM_1: "system-1",
  EXEC_SIDEKICK_HARD_RESTART: "exec-sidekick-hard-restart",
  LATTICE_GOVERNOR: "lattice-governor",
} as const;
...
export const EXPERIMENTS: Record<ExperimentId, ExperimentDefinition> = {
  ...
  [EXPERIMENT_IDS.CONFIGURABLE_BIND_URL]: {
    ...
    description:
      "Allow lattice to listen on a non-localhost address so other devices on your LAN/VPN can connect. Anyone on your network with the auth token can access your lattice API. HTTP only; use only on trusted networks (Tailscale recommended).",
    enabledByDefault: false,
    userOverridable: true,
    showInSettings: true,
  },
  ...
  [EXPERIMENT_IDS.EXEC_SIDEKICK_HARD_RESTART]: {
    id: EXPERIMENT_IDS.EXEC_SIDEKICK_HARD_RESTART,
  ...
  [EXPERIMENT_IDS.LATTICE_GOVERNOR]: {
    id: EXPERIMENT_IDS.LATTICE_GOVERNOR,
    name: "Lattice Governor",
    description: "Remote policy delivery for enterprise Lattice Governor service",
    enabledByDefault: false,
    userOverridable: true,
    showInSettings: true,
  },
